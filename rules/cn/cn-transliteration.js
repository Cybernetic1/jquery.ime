( function ( $ ) {
	'use strict';

	var cn = {
		id: 'cn',
		name: 'YKY Chinese',
		description: 'YKY`s testing.',
		date: '2013-11-15',
		URL: 'http://github.com/Cybernetic1/jquery.ime',
		author: 'YKY',
		license: 'GPLv3',
		version: '1.0',
		patterns: function(input, context) {
			var patterns = [
				['a', ['阿','啊','嗄','锕','吖']],
				['ai','爱'],
				['ai','埃'],
				['ai','艾'],
				['ai','碍'],
				['ai','哀'],
				['ai','癌'],
				['ai','挨'],
				['ai','矮'],
				['ai','唉'],
				['ai','哎'],
				['ai','隘'],
				['ai','蔼'],
				['ai','暧'],
				['ai','嗳'],
				['ai','皑'],
				['ai','霭'],
				['ai','捱'],
				['ai','嗌'],
				['ai','砹'],
				['ai','嫒'],
				['ai','瑷'],
				['ai','锿'],
				['ai','礙'],
				['ai','騃'],
				['ai','鴱'],
				['ai','靉'],
				['ai','賹'],
				['ai','鑀'],
				['ai','薆'],
				['ai','靄'],
				['an','安'],
				['an','案'],
				['an','按'],
				['an','暗'],
				['an','岸'],
				['an','俺'],
				['an','黯'],
				['an','庵'],
				['an','氨'],
				['an','鞍'],
				['an','胺'],
				['an','谙'],
				['an','鹌'],
				['an','铵'],
				['an','桉'],
				['an','犴'],
				['an','鮟'],
				['an','埯'],
				['an','闇'],
				['an','鶕'],
				['an','揞'],
				['an','盦'],
				['an','盫'],
				['an','貋'],
				['an','韽'],
				['an','罯'],
				['an','裺'],
				['an','鴳'],
				['an','鵪'],
				['ang','昂'],
				['ang','肮'],
				['ang','盎'],
				['ang','骯'],
				['ao','奥'],
				['ao','澳'],
				['ao','傲'],
				['ao','熬'],
				['ao','凹'],
				['ao','懊'],
				['ao','鳌'],
				['ao','袄'],
				['ao','敖'],
				['ao','拗'],
				['ao','嗷'],
				['ao','翱'],
				['ao','坳'],
				['ao','鏖'],
				['ao','遨'],
				['ao','媪'],
				['ao','鏊'],
				['ao','骜'],
				['ao','螯'],
				['ao','獒'],
				['ao','岙'],
				['ao','聱'],
				['ao','鰲'],
				['ao','廒'],
				['ao','鼇'],
				['ao','蝹'],
				['ao','鏕'],
				['ao','襖'],
				['ao','謸'],
				['ao','驁'],
				['ao','鴢'],
				['ba','把'],
				['ba','八'],
				['ba','吧'],
				['ba','巴'],
				['ba','爸'],
				['ba','罢'],
				['ba','拔'],
				['ba','霸'],
				['ba','叭'],
				['ba','坝'],
				['ba','跋'],
				['ba','疤'],
				['ba','扒'],
				['ba','芭'],
				['ba','靶'],
				['ba','笆'],
				['ba','耙'],
				['ba','粑'],
				['ba','魃'],
				['ba','鲅'],
				['ba','捌'],
				['ba','灞'],
				['ba','鲃'],
				['ba','钯'],
				['ba','岜'],
				['ba','鲌'],
				['ba','菝'],
				['ba','鮊'],
				['ba','茇'],
				['ba','罷'],
				['ba','釛'],
				['ba','鮁'],
				['ba','釟'],
				['ba','軷'],
				['ba','覇'],
				['ba','跁'],
				['ba','鼥'],
				['bai','白'],
				['bai','百'],
				['bai','败'],
				['bai','摆'],
				['bai','拜'],
				['bai','柏'],
				['bai','呗'],
				['bai','掰'],
				['bai','稗'],
				['bai','佰'],
				['bai','捭'],
				['bai','粺'],
				['bai','襬'],
				['bai','贁'],
				['bai','薭'],
				['ban','办'],
				['ban','半'],
				['ban','般'],
				['ban','版'],
				['ban','班'],
				['ban','板'],
				['ban','伴'],
				['ban','搬'],
				['ban','扮'],
				['ban','斑'],
				['ban','颁'],
				['ban','瓣'],
				['ban','扳'],
				['ban','绊'],
				['ban','拌'],
				['ban','阪'],
				['ban','瘢'],
				['ban','坂'],
				['ban','舨'],
				['ban','钣'],
				['ban','辦'],
				['ban','癍'],
				['ban','籓'],
				['ban','螁'],
				['ban','覂'],
				['ban','跘'],
				['ban','頒'],
				['ban','絆'],
				['bang','帮'],
				['bang','邦'],
				['bang','膀'],
				['bang','棒'],
				['bang','绑'],
				['bang','镑'],
				['bang','榜'],
				['bang','傍'],
				['bang','磅'],
				['bang','谤'],
				['bang','蚌'],
				['bang','梆'],
				['bang','浜'],
				['bang','蒡'],
				['bang','稖'],
				['bang','艕'],
				['bang','蛖'],
				['bang','邫'],
				['bao','报'],
				['bao','保'],
				['bao','包'],
				['bao','宝'],
				['bao','暴'],
				['bao','抱'],
				['bao','爆'],
				['bao','胞'],
				['bao','堡'],
				['bao','饱'],
				['bao','鲍'],
				['bao','豹'],
				['bao','瀑'],
				['bao','褒'],
				['bao','刨'],
				['bao','苞'],
				['bao','葆'],
				['bao','雹'],
				['bao','孢'],
				['bao','煲'],
				['bao','褓'],
				['bao','鸨'],
				['bao','趵'],
				['bao','飽'],
				['bao','菢'],
				['bao','賲'],
				['bao','袌'],
				['bao','窇'],
				['bao','笣'],
				['bao','藵'],
				['bei','被'],
				['bei','北'],
				['bei','备'],
				['bei','背'],
				['bei','贝'],
				['bei','悲'],
				['bei','倍'],
				['bei','杯'],
				['bei','辈'],
				['bei','卑'],
				['bei','碑'],
				['bei','惫'],
				['bei','狈'],
				['bei','悖'],
				['bei','钡'],
				['bei','蓓'],
				['bei','焙'],
				['bei','孛'],
				['bei','碚'],
				['bei','鞴'],
				['bei','鐾'],
				['bei','褙'],
				['bei','貝'],
				['bei','鹎'],
				['bei','誖'],
				['bei','邶'],
				['bei','骳'],
				['bei','輩'],
				['bei','藣'],
				['bei','鐴'],
				['bei','鋇'],
				['ben','本'],
				['ben','奔'],
				['ben','笨'],
				['ben','苯'],
				['ben','贲'],
				['ben','畚'],
				['ben','锛'],
				['ben','坌'],
				['ben','翉'],
				['ben','輽'],
				['ben','賁'],
				['beng','崩'],
				['beng','蹦'],
				['beng','绷'],
				['beng','迸'],
				['beng','甭'],
				['beng','泵'],
				['beng','嘣'],
				['beng','甏'],
				['beng','鏰'],
				['beng','镚'],
				['beng','跰'],
				['beng','繃'],
				['beng','鞛'],
				['beng','錋'],
				['beng','絣'],
				['bi','比'],
				['bi','必'],
				['bi','笔'],
				['bi','避'],
				['bi','毕'],
				['bi','币'],
				['bi','彼'],
				['bi','闭'],
				['bi','鼻'],
				['bi','壁'],
				['bi','逼'],
				['bi','臂'],
				['bi','蔽'],
				['bi','辟'],
				['bi','碧'],
				['bi','弊'],
				['bi','鄙'],
				['bi','毙'],
				['bi','泌'],
				['bi','陛'],
				['bi','庇'],
				['bi','婢'],
				['bi','匕'],
				['bi','敝'],
				['bi','痹'],
				['bi','俾'],
				['bi','璧'],
				['bi','弼'],
				['bi','裨'],
				['bi','愎'],
				['bi','蓖'],
				['bi','跸'],
				['bi','毖'],
				['bi','吡'],
				['bi','哔'],
				['bi','薜'],
				['bi','嬖'],
				['bi','畀'],
				['bi','铋'],
				['bi','祕'],
				['bi','篦'],
				['bi','睥'],
				['bi','髀'],
				['bi','濞'],
				['bi','荸'],
				['bi','閉'],
				['bi','妣'],
				['bi','筆'],
				['bi','秕'],
				['bi','萆'],
				['bi','襞'],
				['bi','荜'],
				['bi','舭'],
				['bi','筚'],
				['bi','狴'],
				['bi','禆'],
				['bi','鸊'],
				['bi','邲'],
				['bi','閟'],
				['bi','聛'],
				['bi','躄'],
				['bi','貏'],
				['bi','滗'],
				['bi','苾'],
				['bi','庳'],
				['bi','觱'],
				['bi','诐'],
				['bi','箅'],
				['bi','粊'],
				['bi','螕'],
				['bi','鶝'],
				['bi','綼'],
				['bi','袐'],
				['bi','蜌'],
				['bi','鲾'],
				['bi','鼊'],
				['bi','鴓'],
				['bi','詖'],
				['bi','髲'],
				['bi','篳'],
				['bi','粃'],
				['bi','罼'],
				['bi','肸'],
				['bi','繴'],
				['bi','赑'],
				['bi','饆'],
				['bi','鉍'],
				['bi','駜'],
				['bi','鷩'],
				['bi','縪'],
				['bi','驆'],
				['bi','鮅'],
				['bian','变'],
				['bian','便'],
				['bian','边'],
				['bian','编'],
				['bian','遍'],
				['bian','辩'],
				['bian','扁'],
				['bian','辨'],
				['bian','鞭'],
				['bian','贬'],
				['bian','辫'],
				['bian','匾'],
				['bian','蝙'],
				['bian','汴'],
				['bian','卞'],
				['bian','弁'],
				['bian','砭'],
				['bian','變'],
				['bian','笾'],
				['bian','苄'],
				['bian','鳊'],
				['bian','煸'],
				['bian','褊'],
				['bian','窆'],
				['bian','碥'],
				['bian','缏'],
				['bian','邊'],
				['bian','辯'],
				['bian','釆'],
				['bian','忭'],
				['bian','編'],
				['bian','邉'],
				['bian','覍'],
				['bian','箯'],
				['bian','緶'],
				['bian','艑'],
				['bian','覵'],
				['biao','表'],
				['biao','标'],
				['biao','彪'],
				['biao','镖'],
				['biao','飙'],
				['biao','婊'],
				['biao','镳'],
				['biao','裱'],
				['biao','膘'],
				['biao','飚'],
				['biao','杓'],
				['biao','鳔'],
				['biao','髟'],
				['biao','贆'],
				['biao','飑'],
				['biao','麃'],
				['biao','飆'],
				['biao','鑣'],
				['biao','鏢'],
				['biao','臕'],
				['biao','脿'],
				['biao','藨'],
				['biao','諘'],
				['bie','别'],
				['bie','憋'],
				['bie','瘪'],
				['bie','鳖'],
				['bie','蹩'],
				['bie','穪'],
				['bie','鼈'],
				['bie','鱉'],
				['bie','莂'],
				['bin','宾'],
				['bin','斌'],
				['bin','滨'],
				['bin','彬'],
				['bin','濒'],
				['bin','鬓'],
				['bin','缤'],
				['bin','殡'],
				['bin','槟'],
				['bin','摈'],
				['bin','膑'],
				['bin','髌'],
				['bin','傧'],
				['bin','豳'],
				['bin','镔'],
				['bin','邠'],
				['bin','鬂'],
				['bin','臏'],
				['bin','蠙'],
				['bin','賓'],
				['bin','髕'],
				['bin','訜'],
				['bin','鬢'],
				['bin','砏'],
				['bing','并'],
				['bing','兵'],
				['bing','病'],
				['bing','冰'],
				['bing','屏'],
				['bing','柄'],
				['bing','饼'],
				['bing','丙'],
				['bing','秉'],
				['bing','禀'],
				['bing','炳'],
				['bing','摒'],
				['bing','邴'],
				['bing','稟'],
				['bing','竝'],
				['bing','餅'],
				['bing','鈵'],
				['bing','鉼'],
				['bo','波'],
				['bo','伯'],
				['bo','博'],
				['bo','播'],
				['bo','薄'],
				['bo','勃'],
				['bo','玻'],
				['bo','拨'],
				['bo','剥'],
				['bo','卜'],
				['bo','驳'],
				['bo','泊'],
				['bo','脖'],
				['bo','舶'],
				['bo','搏'],
				['bo','膊'],
				['bo','钵'],
				['bo','簸'],
				['bo','帛'],
				['bo','渤'],
				['bo','跛'],
				['bo','啵'],
				['bo','箔'],
				['bo','饽'],
				['bo','菠'],
				['bo','铂'],
				['bo','礴'],
				['bo','钹'],
				['bo','擘'],
				['bo','鹁'],
				['bo','亳'],
				['bo','磻'],
				['bo','茀'],
				['bo','檗'],
				['bo','髆'],
				['bo','駮'],
				['bo','缽'],
				['bo','踣'],
				['bo','镈'],
				['bo','礡'],
				['bo','袚'],
				['bo','蔔'],
				['bo','箥'],
				['bo','艊'],
				['bo','鑮'],
				['bo','蚾'],
				['bo','蹳'],
				['bo','鈸'],
				['bo','穛'],
				['bo','髉'],
				['bo','豰'],
				['bu','不'],
				['bu','部'],
				['bu','步'],
				['bu','布'],
				['bu','补'],
				['bu','捕'],
				['bu','怖'],
				['bu','哺'],
				['bu','簿'],
				['bu','埠'],
				['bu','钚'],
				['bu','補'],
				['bu','逋'],
				['bu','卟'],
				['bu','醭'],
				['bu','瓿'],
				['bu','晡'],
				['bu','陠'],
				['bu','钸'],
				['bu','蔀'],
				['bu','鯆'],
				['bu','篰'],
				['bu','餔'],
				['bu','鵏'],
				['bu','鈽'],
				['bu','鳪'],
				['bu','踄'],
				['ca','擦'],
				['ca','礤'],
				['cai','才'],
				['cai','采'],
				['cai','财'],
				['cai','材'],
				['cai','彩'],
				['cai','裁'],
				['cai','菜'],
				['cai','猜'],
				['cai','蔡'],
				['cai','踩'],
				['cai','睬'],
				['cai','財'],
				['cai','綵'],
				['cai','縩'],
				['can','参'],
				['can','残'],
				['can','餐'],
				['can','惨'],
				['can','灿'],
				['can','蚕'],
				['can','惭'],
				['can','璨'],
				['can','粲'],
				['can','孱'],
				['can','骖'],
				['can','黪'],
				['can','蠶'],
				['can','驂'],
				['cang','藏'],
				['cang','苍'],
				['cang','舱'],
				['cang','仓'],
				['cang','沧'],
				['cang','伧'],
				['cang','蒼'],
				['cang','螥'],
				['cang','艙'],
				['cao','草'],
				['cao','操'],
				['cao','曹'],
				['cao','槽'],
				['cao','糙'],
				['cao','嘈'],
				['cao','漕'],
				['cao','螬'],
				['cao','艚'],
				['cao','肏'],
				['cao','蓸'],
				['cao','艸'],
				['ce','策'],
				['ce','测'],
				['ce','侧'],
				['ce','册'],
				['ce','厕'],
				['ce','恻'],
				['ce','粣'],
				['ce','萗'],
				['ce','箣'],
				['ce','蓛'],
				['cen','岑'],
				['cen','涔'],
				['cen','膥'],
				['ceng','曾'],
				['ceng','层'],
				['ceng','蹭'],
				['ceng','噌'],
				['cha','查'],
				['cha','察'],
				['cha','差'],
				['cha','茶'],
				['cha','插'],
				['cha','叉'],
				['cha','刹'],
				['cha','诧'],
				['cha','岔'],
				['cha','喳'],
				['cha','嚓'],
				['cha','茬'],
				['cha','碴'],
				['cha','杈'],
				['cha','搽'],
				['cha','衩'],
				['cha','楂'],
				['cha','槎'],
				['cha','姹'],
				['cha','汊'],
				['cha','锸'],
				['cha','檫'],
				['cha','艖'],
				['cha','猹'],
				['cha','镲'],
				['cha','肞'],
				['cha','臿'],
				['cha','鍤'],
				['cha','褨'],
				['chai','柴'],
				['chai','拆'],
				['chai','钗'],
				['chai','豺'],
				['chai','侪'],
				['chai','虿'],
				['chai','瘥'],
				['chai','袃'],
				['chai','蠆'],
				['chan','产'],
				['chan','单'],
				['chan','缠'],
				['chan','禅'],
				['chan','阐'],
				['chan','蝉'],
				['chan','铲'],
				['chan','忏'],
				['chan','掺'],
				['chan','搀'],
				['chan','馋'],
				['chan','蟾'],
				['chan','婵'],
				['chan','潺'],
				['chan','谄'],
				['chan','谗'],
				['chan','廛'],
				['chan','澶'],
				['chan','羼'],
				['chan','蟬'],
				['chan','躔'],
				['chan','蒇'],
				['chan','酁'],
				['chan','顫'],
				['chan','冁'],
				['chan','繟'],
				['chan','镵'],
				['chan','韂'],
				['chan','鋋'],
				['chan','鑱'],
				['chan','纒'],
				['chan','禪'],
				['chan','諂'],
				['chan','硟'],
				['chan','纏'],
				['chan','鄽'],
				['chan','鏟'],
				['chan','閳'],
				['chan','闡'],
				['chan','骣'],
				['chang','长'],
				['chang','常'],
				['chang','场'],
				['chang','厂'],
				['chang','唱'],
				['chang','偿'],
				['chang','昌'],
				['chang','尝'],
				['chang','肠'],
				['chang','畅'],
				['chang','倡'],
				['chang','敞'],
				['chang','裳'],
				['chang','嫦'],
				['chang','猖'],
				['chang','怅'],
				['chang','娼'],
				['chang','昶'],
				['chang','長'],
				['chang','氅'],
				['chang','苌'],
				['chang','腸'],
				['chang','菖'],
				['chang','鬯'],
				['chang','伥'],
				['chang','徜'],
				['chang','阊'],
				['chang','鲳'],
				['chang','惝'],
				['chang','鲿'],
				['chang','鋹'],
				['chang','閶'],
				['chao','朝'],
				['chao','超'],
				['chao','潮'],
				['chao','吵'],
				['chao','抄'],
				['chao','巢'],
				['chao','嘲'],
				['chao','炒'],
				['chao','剿'],
				['chao','钞'],
				['chao','晁'],
				['chao','焯'],
				['chao','耖'],
				['chao','麨'],
				['chao','鄛'],
				['chao','怊'],
				['che','车'],
				['che','彻'],
				['che','撤'],
				['che','扯'],
				['che','辙'],
				['che','澈'],
				['che','掣'],
				['che','砗'],
				['che','坼'],
				['che','車'],
				['chen','称'],
				['chen','陈'],
				['chen','沉'],
				['chen','臣'],
				['chen','晨'],
				['chen','沈'],
				['chen','尘'],
				['chen','趁'],
				['chen','辰'],
				['chen','衬'],
				['chen','忱'],
				['chen','琛'],
				['chen','嗔'],
				['chen','谌'],
				['chen','宸'],
				['chen','谶'],
				['chen','碜'],
				['chen','抻'],
				['chen','郴'],
				['chen','榇'],
				['chen','龀'],
				['chen','綝'],
				['chen','陳'],
				['chen','蔯'],
				['chen','麎'],
				['chen','贂'],
				['chen','茞'],
				['chen','踸'],
				['chen','磣'],
				['cheng','成'],
				['cheng','程'],
				['cheng','城'],
				['cheng','承'],
				['cheng','盛'],
				['cheng','诚'],
				['cheng','乘'],
				['cheng','呈'],
				['cheng','惩'],
				['cheng','撑'],
				['cheng','澄'],
				['cheng','逞'],
				['cheng','丞'],
				['cheng','橙'],
				['cheng','秤'],
				['cheng','瞠'],
				['cheng','骋'],
				['cheng','铛'],
				['cheng','稱'],
				['cheng','蛏'],
				['cheng','裎'],
				['cheng','枨'],
				['cheng','柽'],
				['cheng','铖'],
				['cheng','塍'],
				['cheng','酲'],
				['cheng','埕'],
				['cheng','郕'],
				['cheng','脭'],
				['cheng','誠'],
				['cheng','赪'],
				['cheng','絾'],
				['cheng','騁'],
				['cheng','頳'],
				['cheng','鏿'],
				['cheng','碀'],
				['chi','持'],
				['chi','吃'],
				['chi','迟'],
				['chi','尺'],
				['chi','赤'],
				['chi','池'],
				['chi','齿'],
				['chi','翅'],
				['chi','驰'],
				['chi','耻'],
				['chi','痴'],
				['chi','匙'],
				['chi','侈'],
				['chi','叱'],
				['chi','嗤'],
				['chi','炽'],
				['chi','弛'],
				['chi','敕'],
				['chi','哧'],
				['chi','饬'],
				['chi','啻'],
				['chi','蚩'],
				['chi','笞'],
				['chi','魑'],
				['chi','媸'],
				['chi','瘛'],
				['chi','踅'],
				['chi','螭'],
				['chi','鸱'],
				['chi','篪'],
				['chi','褫'],
				['chi','墀'],
				['chi','豉'],
				['chi','踟'],
				['chi','傺'],
				['chi','坻'],
				['chi','郗'],
				['chi','茌'],
				['chi','遲'],
				['chi','齒'],
				['chi','眵'],
				['chi','趩'],
				['chi','飭'],
				['chi','鶒'],
				['chi','馳'],
				['chi','鴟'],
				['chi','鵄'],
				['chi','竾'],
				['chi','筂'],
				['chi','鷘'],
				['chong','重'],
				['chong','充'],
				['chong','冲'],
				['chong','虫'],
				['chong','崇'],
				['chong','宠'],
				['chong','忡'],
				['chong','憧'],
				['chong','舂'],
				['chong','铳'],
				['chong','艟'],
				['chong','蟲'],
				['chong','衝'],
				['chong','翀'],
				['chong','茺'],
				['chong','蹖'],
				['chou','抽'],
				['chou','仇'],
				['chou','筹'],
				['chou','丑'],
				['chou','酬'],
				['chou','臭'],
				['chou','愁'],
				['chou','绸'],
				['chou','畴'],
				['chou','瞅'],
				['chou','踌'],
				['chou','稠'],
				['chou','惆'],
				['chou','俦'],
				['chou','雠'],
				['chou','紬'],
				['chou','瘳'],
				['chou','雔'],
				['chou','醜'],
				['chou','帱'],
				['chou','籌'],
				['chou','跾'],
				['chou','絒'],
				['chu','出'],
				['chu','处'],
				['chu','除'],
				['chu','初'],
				['chu','楚'],
				['chu','础'],
				['chu','触'],
				['chu','储'],
				['chu','畜'],
				['chu','厨'],
				['chu','矗'],
				['chu','橱'],
				['chu','躇'],
				['chu','雏'],
				['chu','锄'],
				['chu','搐'],
				['chu','褚'],
				['chu','黜'],
				['chu','刍'],
				['chu','怵'],
				['chu','杵'],
				['chu','绌'],
				['chu','蜍'],
				['chu','處'],
				['chu','樗'],
				['chu','蹰'],
				['chu','滁'],
				['chu','憷'],
				['chu','禇'],
				['chu','楮'],
				['chu','礎'],
				['chu','觸'],
				['chu','臅'],
				['chu','貙'],
				['chu','鉏'],
				['chu','齣'],
				['chu','鋤'],
				['chu','蒭'],
				['chu','豠'],
				['chu','踀'],
				['chu','耡'],
				['chu','芻'],
				['chu','齼'],
				['chu','竌'],
				['chu','篨'],
				['chuai','揣'],
				['chuai','踹'],
				['chuai','嘬'],
				['chuai','搋'],
				['chuai','膗'],
				['chuai','膪'],
				['chuan','传'],
				['chuan','船'],
				['chuan','穿'],
				['chuan','川'],
				['chuan','串'],
				['chuan','喘'],
				['chuan','椽'],
				['chuan','舛'],
				['chuan','氚'],
				['chuan','钏'],
				['chuan','遄'],
				['chuan','荈'],
				['chuan','舡'],
				['chuan','巛'],
				['chuang','创'],
				['chuang','床'],
				['chuang','窗'],
				['chuang','闯'],
				['chuang','幢'],
				['chuang','疮'],
				['chuang','怆'],
				['chui','吹'],
				['chui','垂'],
				['chui','椎'],
				['chui','锤'],
				['chui','炊'],
				['chui','捶'],
				['chui','槌'],
				['chui','陲'],
				['chui','棰'],
				['chui','箠'],
				['chui','顀'],
				['chui','菙'],
				['chun','春'],
				['chun','纯'],
				['chun','唇'],
				['chun','蠢'],
				['chun','醇'],
				['chun','淳'],
				['chun','椿'],
				['chun','鹑'],
				['chun','莼'],
				['chun','蝽'],
				['chun','純'],
				['chun','陙'],
				['chun','鯙'],
				['chun','賰'],
				['chun','輴'],
				['chun','脣'],
				['chun','醕'],
				['chuo','绰'],
				['chuo','戳'],
				['chuo','啜'],
				['chuo','辍'],
				['chuo','龊'],
				['chuo','踔'],
				['chuo','辵'],
				['chuo','磭'],
				['ci','此'],
				['ci','次'],
				['ci','词'],
				['ci','刺'],
				['ci','辞'],
				['ci','慈'],
				['ci','磁'],
				['ci','茨'],
				['ci','赐'],
				['ci','瓷'],
				['ci','雌'],
				['ci','伺'],
				['ci','祠'],
				['ci','疵'],
				['ci','鹚'],
				['ci','呲'],
				['ci','糍'],
				['ci','詞'],
				['ci','茈'],
				['ci','跐'],
				['ci','餈'],
				['ci','鈶'],
				['ci','莿'],
				['ci','賜'],
				['ci','辭'],
				['ci','鴜'],
				['ci','飺'],
				['cong','从'],
				['cong','匆'],
				['cong','聪'],
				['cong','丛'],
				['cong','葱'],
				['cong','囱'],
				['cong','淙'],
				['cong','枞'],
				['cong','苁'],
				['cong','琮'],
				['cong','骢'],
				['cong','璁'],
				['cong','茐'],
				['cong','賨'],
				['cong','聦'],
				['cong','鏦'],
				['cong','聰'],
				['cong','蔥'],
				['cong','驄'],
				['cong','誴'],
				['cong','謥'],
				['cong','聡'],
				['cou','凑'],
				['cou','辏'],
				['cou','腠'],
				['cou','輳'],
				['cou','楱'],
				['cu','促'],
				['cu','粗'],
				['cu','卒'],
				['cu','簇'],
				['cu','醋'],
				['cu','猝'],
				['cu','蹙'],
				['cu','蹴'],
				['cu','徂'],
				['cu','殂'],
				['cu','麄'],
				['cu','顣'],
				['cu','蔟'],
				['cu','麤'],
				['cuan','窜'],
				['cuan','篡'],
				['cuan','攒'],
				['cuan','蹿'],
				['cuan','撺'],
				['cuan','爨'],
				['cuan','氽'],
				['cuan','汆'],
				['cuan','镩'],
				['cuan','竄'],
				['cui','衰'],
				['cui','粹'],
				['cui','脆'],
				['cui','催'],
				['cui','摧'],
				['cui','翠'],
				['cui','崔'],
				['cui','悴'],
				['cui','萃'],
				['cui','瘁'],
				['cui','啐'],
				['cui','璀'],
				['cui','淬'],
				['cui','榱'],
				['cui','粋'],
				['cui','膬'],
				['cui','臎'],
				['cui','膵'],
				['cui','毳'],
				['cui','脺'],
				['cui','翆'],
				['cui','繀'],
				['cui','磪'],
				['cun','存'],
				['cun','村'],
				['cun','寸'],
				['cun','忖'],
				['cun','邨'],
				['cun','皴'],
				['cuo','错'],
				['cuo','措'],
				['cuo','挫'],
				['cuo','搓'],
				['cuo','磋'],
				['cuo','撮'],
				['cuo','厝'],
				['cuo','锉'],
				['cuo','痤'],
				['cuo','矬'],
				['cuo','蹉'],
				['cuo','鹾'],
				['cuo','嵯'],
				['cuo','錯'],
				['cuo','脞'],
				['cuo','虘'],
				['da','大'],
				['da','打'],
				['da','达'],
				['da','答'],
				['da','搭'],
				['da','瘩'],
				['da','哒'],
				['da','耷'],
				['da','鞑'],
				['da','靼'],
				['da','妲'],
				['da','褡'],
				['da','笪'],
				['da','怛'],
				['da','跶'],
				['da','達'],
				['da','荅'],
				['da','迏'],
				['da','繨'],
				['da','矺'],
				['da','鎝'],
				['da','荙'],
				['dai','代'],
				['dai','带'],
				['dai','待'],
				['dai','戴'],
				['dai','袋'],
				['dai','呆'],
				['dai','贷'],
				['dai','逮'],
				['dai','黛'],
				['dai','歹'],
				['dai','怠'],
				['dai','殆'],
				['dai','傣'],
				['dai','岱'],
				['dai','迨'],
				['dai','玳'],
				['dai','甙'],
				['dai','绐'],
				['dai','埭'],
				['dai','轪'],
				['dai','艜'],
				['dai','襶'],
				['dai','簤'],
				['dai','貸'],
				['dai','鮘'],
				['dan','但'],
				['dan','石'],
				['dan','弹'],
				['dan','担'],
				['dan','丹'],
				['dan','淡'],
				['dan','旦'],
				['dan','胆'],
				['dan','蛋'],
				['dan','诞'],
				['dan','耽'],
				['dan','氮'],
				['dan','惮'],
				['dan','眈'],
				['dan','郸'],
				['dan','澹'],
				['dan','掸'],
				['dan','疸'],
				['dan','殚'],
				['dan','啖'],
				['dan','聃'],
				['dan','儋'],
				['dan','箪'],
				['dan','膽'],
				['dan','萏'],
				['dan','赕'],
				['dan','酖'],
				['dan','瘅'],
				['dan','霮'],
				['dan','禫'],
				['dan','蜑'],
				['dan','饏'],
				['dan','駳'],
				['dan','誕'],
				['dan','贉'],
				['dan','聸'],
				['dan','黮'],
				['dan','簞'],
				['dan','髧'],
				['dan','躭'],
				['dan','襌'],
				['dang','当'],
				['dang','党'],
				['dang','荡'],
				['dang','档'],
				['dang','挡'],
				['dang','裆'],
				['dang','宕'],
				['dang','谠'],
				['dang','凼'],
				['dang','砀'],
				['dang','菪'],
				['dang','黨'],
				['dang','逿'],
				['dang','簜'],
				['dang','趤'],
				['dang','碭'],
				['dang','讜'],
				['dang','盪'],
				['dang','簹'],
				['dang','礑'],
				['dao','到'],
				['dao','道'],
				['dao','导'],
				['dao','倒'],
				['dao','岛'],
				['dao','刀'],
				['dao','盗'],
				['dao','祷'],
				['dao','稻'],
				['dao','蹈'],
				['dao','捣'],
				['dao','叨'],
				['dao','悼'],
				['dao','焘'],
				['dao','氘'],
				['dao','忉'],
				['dao','纛'],
				['dao','禱'],
				['dao','盜'],
				['dao','裯'],
				['dao','鱽'],
				['dao','稲'],
				['dao','衟'],
				['dao','隝'],
				['dao','魛'],
				['dao','翢'],
				['dao','軇'],
				['de','的'],
				['de','地'],
				['de','得'],
				['de','德'],
				['de','底'],
				['de','锝'],
				['deng','等'],
				['deng','登'],
				['deng','灯'],
				['deng','邓'],
				['deng','瞪'],
				['deng','凳'],
				['deng','蹬'],
				['deng','簦'],
				['deng','噔'],
				['deng','镫'],
				['deng','嶝'],
				['deng','戥'],
				['deng','磴'],
				['deng','鄧'],
				['deng','隥'],
				['di','第'],
				['di','提'],
				['di','敌'],
				['di','低'],
				['di','帝'],
				['di','弟'],
				['di','抵'],
				['di','蒂'],
				['di','迪'],
				['di','递'],
				['di','滴'],
				['di','狄'],
				['di','堤'],
				['di','笛'],
				['di','缔'],
				['di','邸'],
				['di','涤'],
				['di','嘀'],
				['di','谛'],
				['di','嫡'],
				['di','翟'],
				['di','砥'],
				['di','棣'],
				['di','氐'],
				['di','诋'],
				['di','荻'],
				['di','娣'],
				['di','骶'],
				['di','柢'],
				['di','睇'],
				['di','籴'],
				['di','羝'],
				['di','镝'],
				['di','碲'],
				['di','菧'],
				['di','鞮'],
				['di','磾'],
				['di','袛'],
				['di','觌'],
				['di','遞'],
				['di','禘'],
				['di','隄'],
				['di','踧'],
				['di','菂'],
				['di','篴'],
				['di','締'],
				['di','聜'],
				['di','糴'],
				['di','釱'],
				['di','靮'],
				['di','腣'],
				['di','逓'],
				['di','諦'],
				['di','鬄'],
				['di','祶'],
				['di','軧'],
				['dia','嗲'],
				['dian','点'],
				['dian','电'],
				['dian','店'],
				['dian','典'],
				['dian','殿'],
				['dian','颠'],
				['dian','甸'],
				['dian','垫'],
				['dian','奠'],
				['dian','癫'],
				['dian','淀'],
				['dian','佃'],
				['dian','滇'],
				['dian','碘'],
				['dian','惦'],
				['dian','巅'],
				['dian','掂'],
				['dian','玷'],
				['dian','癜'],
				['dian','踮'],
				['dian','點'],
				['dian','钿'],
				['dian','靛'],
				['dian','簟'],
				['dian','電'],
				['dian','阽'],
				['dian','坫'],
				['dian','齻'],
				['dian','蜔'],
				['dian','鈿'],
				['dian','磹'],
				['dian','顛'],
				['dian','蒧'],
				['dian','蹎'],
				['diao','调'],
				['diao','掉'],
				['diao','雕'],
				['diao','吊'],
				['diao','钓'],
				['diao','刁'],
				['diao','碉'],
				['diao','凋'],
				['diao','叼'],
				['diao','貂'],
				['diao','調'],
				['diao','鲷'],
				['diao','鵰'],
				['diao','铫'],
				['diao','铞'],
				['diao','訋'],
				['diao','鸼'],
				['diao','鯛'],
				['diao','釣'],
				['diao','窎'],
				['diao','颩'],
				['diao','矵'],
				['die','跌'],
				['die','爹'],
				['die','叠'],
				['die','蝶'],
				['die','谍'],
				['die','碟'],
				['die','迭'],
				['die','喋'],
				['die','牒'],
				['die','堞'],
				['die','蹀'],
				['die','耋'],
				['die','鲽'],
				['die','垤'],
				['die','绖'],
				['die','眰'],
				['die','褋'],
				['die','瓞'],
				['die','蜨'],
				['die','眣'],
				['die','耊'],
				['die','镻'],
				['die','詄'],
				['ding','定'],
				['ding','顶'],
				['ding','丁'],
				['ding','订'],
				['ding','盯'],
				['ding','钉'],
				['ding','鼎'],
				['ding','叮'],
				['ding','锭'],
				['ding','铤'],
				['ding','啶'],
				['ding','酊'],
				['ding','仃'],
				['ding','碇'],
				['ding','腚'],
				['ding','玎'],
				['ding','疔'],
				['ding','耵'],
				['ding','頂'],
				['ding','訂'],
				['ding','釘'],
				['ding','虰'],
				['ding','饤'],
				['ding','碠'],
				['ding','磸'],
				['ding','艼'],
				['ding','鐤'],
				['ding','錠'],
				['ding','鼑'],
				['diu','丢'],
				['diu','铥'],
				['dong','动'],
				['dong','东'],
				['dong','洞'],
				['dong','懂'],
				['dong','冬'],
				['dong','董'],
				['dong','冻'],
				['dong','栋'],
				['dong','咚'],
				['dong','恫'],
				['dong','侗'],
				['dong','胴'],
				['dong','胨'],
				['dong','硐'],
				['dong','氡'],
				['dong','鸫'],
				['dong','岽'],
				['dong','鶇'],
				['dong','蝀'],
				['dong','笗'],
				['dong','箽'],
				['dong','鶫'],
				['dong','菄'],
				['dong','霘'],
				['dong','衕'],
				['dong','垌'],
				['dong','詷'],
				['dou','都'],
				['dou','斗'],
				['dou','读'],
				['dou','抖'],
				['dou','豆'],
				['dou','陡'],
				['dou','兜'],
				['dou','逗'],
				['dou','窦'],
				['dou','蚪'],
				['dou','痘'],
				['dou','饾'],
				['dou','蔸'],
				['dou','餖'],
				['dou','竇'],
				['dou','閗'],
				['dou','脰'],
				['dou','篼'],
				['dou','荳'],
				['du','度'],
				['du','独'],
				['du','督'],
				['du','毒'],
				['du','杜'],
				['du','渡'],
				['du','肚'],
				['du','赌'],
				['du','堵'],
				['du','睹'],
				['du','妒'],
				['du','嘟'],
				['du','笃'],
				['du','渎'],
				['du','犊'],
				['du','镀'],
				['du','牍'],
				['du','蠹'],
				['du','芏'],
				['du','黩'],
				['du','碡'],
				['du','髑'],
				['du','讀'],
				['du','阇'],
				['du','椟'],
				['du','覩'],
				['du','讟'],
				['du','韥'],
				['du','鑟'],
				['du','篤'],
				['du','陼'],
				['du','詫'],
				['du','鍍'],
				['du','闍'],
				['du','秺'],
				['duan','断'],
				['duan','段'],
				['duan','短'],
				['duan','端'],
				['duan','锻'],
				['duan','缎'],
				['duan','椴'],
				['duan','煅'],
				['duan','鍛'],
				['duan','緞'],
				['duan','葮'],
				['dui','对'],
				['dui','队'],
				['dui','堆'],
				['dui','兑'],
				['dui','怼'],
				['dui','碓'],
				['dui','隊'],
				['dui','憝'],
				['dui','鐓'],
				['dui','錞'],
				['dui','祋'],
				['dui','鋭'],
				['dui','鎚'],
				['dui','陮'],
				['dui','鴭'],
				['dun','顿'],
				['dun','盾'],
				['dun','吨'],
				['dun','敦'],
				['dun','蹲'],
				['dun','钝'],
				['dun','遁'],
				['dun','沌'],
				['dun','墩'],
				['dun','炖'],
				['dun','盹'],
				['dun','囤'],
				['dun','趸'],
				['dun','砘'],
				['dun','礅'],
				['dun','遯'],
				['dun','蹾'],
				['dun','鈍'],
				['dun','镦'],
				['dun','躉'],
				['dun','逇'],
				['dun','頓'],
				['duo','多'],
				['duo','夺'],
				['duo','朵'],
				['duo','躲'],
				['duo','堕'],
				['duo','踱'],
				['duo','舵'],
				['duo','哆'],
				['duo','惰'],
				['duo','咄'],
				['duo','驮'],
				['duo','铎'],
				['duo','跺'],
				['duo','垛'],
				['duo','剁'],
				['duo','掇'],
				['duo','裰'],
				['duo','缍'],
				['duo','哚'],
				['duo','沲'],
				['duo','躱'],
				['duo','饳'],
				['e','恶'],
				['e','额'],
				['e','俄'],
				['e','饿'],
				['e','哦'],
				['e','鹅'],
				['e','厄'],
				['e','遏'],
				['e','愕'],
				['e','娥'],
				['e','扼'],
				['e','鄂'],
				['e','峨'],
				['e','呃'],
				['e','蛾'],
				['e','噩'],
				['e','鳄'],
				['e','讹'],
				['e','颚'],
				['e','萼'],
				['e','谔'],
				['e','婀'],
				['e','垩'],
				['e','鹗'],
				['e','锷'],
				['e','轭'],
				['e','阏'],
				['e','腭'],
				['e','屙'],
				['e','苊'],
				['e','莪'],
				['e','锇'],
				['e','蚵'],
				['e','阨'],
				['e','齾'],
				['e','鰪'],
				['e','蕚'],
				['e','額'],
				['e','訛'],
				['e','顎'],
				['e','詻'],
				['e','軛'],
				['e','阸'],
				['e','頞'],
				['e','鶚'],
				['e','鍔'],
				['e','蘁'],
				['e','礘'],
				['e','譌'],
				['e','睋'],
				['e','讍'],
				['e','餓'],
				['e','鵝'],
				['ei','诶'],
				['en','恩'],
				['en','嗯'],
				['en','唔'],
				['en','摁'],
				['en','蒽'],
				['er','而'],
				['er','二'],
				['er','儿'],
				['er','尔'],
				['er','耳'],
				['er','饵'],
				['er','贰'],
				['er','珥'],
				['er','迩'],
				['er','洱'],
				['er','铒'],
				['er','鸸'],
				['er','佴'],
				['er','粫'],
				['er','鲕'],
				['er','薾'],
				['er','邇'],
				['er','輀'],
				['er','胹'],
				['er','貮'],
				['er','餌'],
				['er','趰'],
				['fa','发'],
				['fa','法'],
				['fa','罚'],
				['fa','乏'],
				['fa','伐'],
				['fa','阀'],
				['fa','筏'],
				['fa','砝'],
				['fa','珐'],
				['fa','垡'],
				['fa','髮'],
				['fa','髪'],
				['fa','瞂'],
				['fa','蕟'],
				['fa','罰'],
				['fan','反'],
				['fan','范'],
				['fan','犯'],
				['fan','饭'],
				['fan','凡'],
				['fan','翻'],
				['fan','烦'],
				['fan','繁'],
				['fan','泛'],
				['fan','返'],
				['fan','番'],
				['fan','帆'],
				['fan','贩'],
				['fan','藩'],
				['fan','梵'],
				['fan','樊'],
				['fan','幡'],
				['fan','蕃'],
				['fan','燔'],
				['fan','矾'],
				['fan','钒'],
				['fan','畈'],
				['fan','蘩'],
				['fan','攵'],
				['fan','範'],
				['fan','蹯'],
				['fan','飯'],
				['fan','販'],
				['fan','訉'],
				['fan','軓'],
				['fan','鐇'],
				['fan','繙'],
				['fan','盕'],
				['fan','鷭'],
				['fan','蠜'],
				['fan','飰'],
				['fan','轓'],
				['fan','薠'],
				['fan','軬'],
				['fan','緐'],
				['fan','笲'],
				['fang','方'],
				['fang','放'],
				['fang','房'],
				['fang','防'],
				['fang','访'],
				['fang','仿'],
				['fang','芳'],
				['fang','妨'],
				['fang','纺'],
				['fang','坊'],
				['fang','肪'],
				['fang','舫'],
				['fang','鲂'],
				['fang','枋'],
				['fang','匚'],
				['fang','钫'],
				['fang','邡'],
				['fang','髣'],
				['fang','訪'],
				['fang','眆'],
				['fei','非'],
				['fei','飞'],
				['fei','费'],
				['fei','废'],
				['fei','菲'],
				['fei','肥'],
				['fei','肺'],
				['fei','匪'],
				['fei','啡'],
				['fei','妃'],
				['fei','沸'],
				['fei','斐'],
				['fei','狒'],
				['fei','吠'],
				['fei','诽'],
				['fei','腓'],
				['fei','绯'],
				['fei','扉'],
				['fei','翡'],
				['fei','蜚'],
				['fei','霏'],
				['fei','芾'],
				['fei','淝'],
				['fei','榧'],
				['fei','砩'],
				['fei','鲱'],
				['fei','怫'],
				['fei','痱'],
				['fei','悱'],
				['fei','镄'],
				['fei','飛'],
				['fei','費'],
				['fei','笰'],
				['fei','篚'],
				['fei','蕡'],
				['fei','鯡'],
				['fei','鐨'],
				['fei','騛'],
				['fei','髴'],
				['fei','裶'],
				['fei','蕜'],
				['fei','緋'],
				['fen','分'],
				['fen','份'],
				['fen','纷'],
				['fen','奋'],
				['fen','粉'],
				['fen','愤'],
				['fen','芬'],
				['fen','氛'],
				['fen','吩'],
				['fen','坟'],
				['fen','焚'],
				['fen','粪'],
				['fen','忿'],
				['fen','汾'],
				['fen','酚'],
				['fen','偾'],
				['fen','玢'],
				['fen','棼'],
				['fen','雰'],
				['fen','糞'],
				['fen','膹'],
				['fen','瞓'],
				['fen','鼢'],
				['fen','紛'],
				['fen','翂'],
				['fen','鲼'],
				['fen','羵'],
				['fen','鼖'],
				['fen','魵'],
				['fen','蚡'],
				['fen','鈖'],
				['fen','黺'],
				['feng','风'],
				['feng','封'],
				['feng','丰'],
				['feng','奉'],
				['feng','峰'],
				['feng','凤'],
				['feng','疯'],
				['feng','锋'],
				['feng','冯'],
				['feng','蜂'],
				['feng','逢'],
				['feng','讽'],
				['feng','枫'],
				['feng','俸'],
				['feng','烽'],
				['feng','沣'],
				['feng','酆'],
				['feng','風'],
				['feng','葑'],
				['feng','砜'],
				['feng','豐'],
				['feng','唪'],
				['feng','縫'],
				['feng','赗'],
				['feng','鄷'],
				['feng','馮'],
				['feng','鳳'],
				['feng','盽'],
				['feng','碸'],
				['feng','艂'],
				['feng','鋒'],
				['feng','豊'],
				['fiao','覅'],
				['fo','佛'],
				['fou','否'],
				['fou','缶'],
				['fou','缻'],
				['fu','服'],
				['fu','夫'],
				['fu','府'],
				['fu','复'],
				['fu','父'],
				['fu','福'],
				['fu','负'],
				['fu','富'],
				['fu','副'],
				['fu','付'],
				['fu','附'],
				['fu','妇'],
				['fu','符'],
				['fu','弗'],
				['fu','伏'],
				['fu','幅'],
				['fu','浮'],
				['fu','腐'],
				['fu','腹'],
				['fu','扶'],
				['fu','抚'],
				['fu','赋'],
				['fu','肤'],
				['fu','覆'],
				['fu','赴'],
				['fu','傅'],
				['fu','辅'],
				['fu','俘'],
				['fu','咐'],
				['fu','甫'],
				['fu','俯'],
				['fu','辐'],
				['fu','缚'],
				['fu','拂'],
				['fu','芙'],
				['fu','斧'],
				['fu','敷'],
				['fu','脯'],
				['fu','孵'],
				['fu','蝠'],
				['fu','袱'],
				['fu','阜'],
				['fu','孚'],
				['fu','腑'],
				['fu','氟'],
				['fu','釜'],
				['fu','驸'],
				['fu','匐'],
				['fu','蝮'],
				['fu','苻'],
				['fu','馥'],
				['fu','讣'],
				['fu','趺'],
				['fu','茯'],
				['fu','蚨'],
				['fu','郛'],
				['fu','呋'],
				['fu','凫'],
				['fu','涪'],
				['fu','鲋'],
				['fu','跗'],
				['fu','菔'],
				['fu','罘'],
				['fu','蜉'],
				['fu','麸'],
				['fu','拊'],
				['fu','呒'],
				['fu','莩'],
				['fu','黼'],
				['fu','桴'],
				['fu','绂'],
				['fu','祓'],
				['fu','黻'],
				['fu','膚'],
				['fu','褔'],
				['fu','複'],
				['fu','赙'],
				['fu','幞'],
				['fu','負'],
				['fu','袝'],
				['fu','輔'],
				['fu','滏'],
				['fu','绋'],
				['fu','賦'],
				['fu','鮄'],
				['fu','郙'],
				['fu','稃'],
				['fu','祔'],
				['fu','鳆'],
				['fu','鄜'],
				['fu','葍'],
				['fu','禣'],
				['fu','鈇'],
				['fu','綍'],
				['fu','艴'],
				['fu','訃'],
				['fu','釡'],
				['fu','鍑'],
				['fu','虙'],
				['fu','韨'],
				['fu','簠'],
				['fu','韍'],
				['fu','蕧'],
				['fu','豧'],
				['fu','輻'],
				['fu','綒'],
				['fu','麩'],
				['fu','縛'],
				['ga','尬'],
				['ga','嘎'],
				['ga','噶'],
				['ga','旮'],
				['ga','尕'],
				['ga','钆'],
				['ga','尜'],
				['gai','该'],
				['gai','改'],
				['gai','概'],
				['gai','盖'],
				['gai','丐'],
				['gai','钙'],
				['gai','溉'],
				['gai','赅'],
				['gai','垓'],
				['gai','蓋'],
				['gai','陔'],
				['gai','該'],
				['gai','荄'],
				['gai','戤'],
				['gai','鈣'],
				['gai','葢'],
				['gai','賅'],
				['gan','感'],
				['gan','干'],
				['gan','敢'],
				['gan','赶'],
				['gan','甘'],
				['gan','肝'],
				['gan','乾'],
				['gan','杆'],
				['gan','尴'],
				['gan','竿'],
				['gan','赣'],
				['gan','橄'],
				['gan','秆'],
				['gan','柑'],
				['gan','苷'],
				['gan','淦'],
				['gan','矸'],
				['gan','绀'],
				['gan','擀'],
				['gan','酐'],
				['gan','泔'],
				['gan','疳'],
				['gan','旰'],
				['gan','鳡'],
				['gan','坩'],
				['gan','筸'],
				['gan','簳'],
				['gan','澉'],
				['gan','鱤'],
				['gan','趕'],
				['gan','粓'],
				['gan','飦'],
				['gan','稈'],
				['gan','紺'],
				['gang','刚'],
				['gang','港'],
				['gang','钢'],
				['gang','岗'],
				['gang','纲'],
				['gang','冈'],
				['gang','缸'],
				['gang','扛'],
				['gang','杠'],
				['gang','肛'],
				['gang','罡'],
				['gang','綱'],
				['gang','鋼'],
				['gang','罁'],
				['gang','筻'],
				['gang','釭'],
				['gao','高'],
				['gao','告'],
				['gao','搞'],
				['gao','稿'],
				['gao','糕'],
				['gao','膏'],
				['gao','睾'],
				['gao','羔'],
				['gao','诰'],
				['gao','皋'],
				['gao','镐'],
				['gao','槁'],
				['gao','篙'],
				['gao','杲'],
				['gao','缟'],
				['gao','锆'],
				['gao','槔'],
				['gao','藁'],
				['gao','郜'],
				['gao','菒'],
				['gao','誥'],
				['gao','祮'],
				['gao','鷱'],
				['gao','筶'],
				['gao','縞'],
				['gao','臯'],
				['gao','禞'],
				['gao','韟'],
				['gao','祰'],
				['gao','藳'],
				['ge','个'],
				['ge','合'],
				['ge','各'],
				['ge','格'],
				['ge','革'],
				['ge','哥'],
				['ge','歌'],
				['ge','隔'],
				['ge','割'],
				['ge','阁'],
				['ge','戈'],
				['ge','葛'],
				['ge','搁'],
				['ge','胳'],
				['ge','鸽'],
				['ge','咯'],
				['ge','骼'],
				['ge','疙'],
				['ge','蛤'],
				['ge','圪'],
				['ge','铬'],
				['ge','阖'],
				['ge','嗝'],
				['ge','纥'],
				['ge','舸'],
				['ge','镉'],
				['ge','虼'],
				['ge','搿'],
				['ge','騔'],
				['ge','膈'],
				['ge','硌'],
				['ge','袼'],
				['ge','哿'],
				['ge','鬲'],
				['ge','閤'],
				['ge','鲄'],
				['ge','閣'],
				['ge','鴿'],
				['ge','詥'],
				['ge','箇'],
				['ge','鴐'],
				['ge','轕'],
				['ge','裓'],
				['ge','塥'],
				['ge','鎘'],
				['ge','紇'],
				['ge','魺'],
				['gei','给'],
				['gei','給'],
				['gen','根'],
				['gen','跟'],
				['gen','亘'],
				['gen','艮'],
				['gen','哏'],
				['gen','茛'],
				['geng','更'],
				['geng','耿'],
				['geng','梗'],
				['geng','庚'],
				['geng','哽'],
				['geng','赓'],
				['geng','羹'],
				['geng','埂'],
				['geng','鲠'],
				['geng','绠'],
				['geng','絙'],
				['geng','鯁'],
				['geng','鶊'],
				['geng','鹒'],
				['geng','縆'],
				['geng','骾'],
				['geng','賡'],
				['gong','公'],
				['gong','工'],
				['gong','共'],
				['gong','功'],
				['gong','供'],
				['gong','攻'],
				['gong','宫'],
				['gong','贡'],
				['gong','恭'],
				['gong','弓'],
				['gong','巩'],
				['gong','拱'],
				['gong','躬'],
				['gong','龚'],
				['gong','汞'],
				['gong','蚣'],
				['gong','觥'],
				['gong','肱'],
				['gong','貢'],
				['gong','珙'],
				['gong','魟'],
				['gong','鞏'],
				['gong','碽'],
				['gong','龔'],
				['gou','构'],
				['gou','够'],
				['gou','购'],
				['gou','狗'],
				['gou','沟'],
				['gou','勾'],
				['gou','钩'],
				['gou','苟'],
				['gou','垢'],
				['gou','篝'],
				['gou','媾'],
				['gou','枸'],
				['gou','诟'],
				['gou','佝'],
				['gou','笱'],
				['gou','彀'],
				['gou','岣'],
				['gou','鞲'],
				['gou','缑'],
				['gou','觏'],
				['gou','鉤'],
				['gou','袧'],
				['gou','遘'],
				['gou','購'],
				['gou','耈'],
				['gou','詬'],
				['gou','雊'],
				['gou','簼'],
				['gou','訽'],
				['gou','耇'],
				['gou','褠'],
				['gou','緱'],
				['gou','覯'],
				['gu','古'],
				['gu','故'],
				['gu','股'],
				['gu','顾'],
				['gu','固'],
				['gu','姑'],
				['gu','骨'],
				['gu','谷'],
				['gu','鼓'],
				['gu','估'],
				['gu','孤'],
				['gu','滑'],
				['gu','雇'],
				['gu','贾'],
				['gu','咕'],
				['gu','辜'],
				['gu','菇'],
				['gu','呱'],
				['gu','沽'],
				['gu','箍'],
				['gu','蛊'],
				['gu','锢'],
				['gu','鹄'],
				['gu','汩'],
				['gu','梏'],
				['gu','轱'],
				['gu','钴'],
				['gu','鸪'],
				['gu','蛄'],
				['gu','毂'],
				['gu','诂'],
				['gu','崮'],
				['gu','痼'],
				['gu','牯'],
				['gu','嘏'],
				['gu','罟'],
				['gu','瞽'],
				['gu','菰'],
				['gu','觚'],
				['gu','顧'],
				['gu','酤'],
				['gu','鲴'],
				['gu','臌'],
				['gu','鯝'],
				['gu','笟'],
				['gu','蓇'],
				['gu','鴣'],
				['gu','苽'],
				['gu','馉'],
				['gu','祻'],
				['gu','牿'],
				['gu','脵'],
				['gu','轂'],
				['gu','盬'],
				['gu','糓'],
				['gu','箛'],
				['gu','頋'],
				['gua','挂'],
				['gua','瓜'],
				['gua','寡'],
				['gua','刮'],
				['gua','卦'],
				['gua','褂'],
				['gua','剐'],
				['gua','诖'],
				['gua','胍'],
				['gua','鸹'],
				['gua','罣'],
				['gua','栝'],
				['gua','詿'],
				['gua','緺'],
				['gua','鴰'],
				['gua','騧'],
				['gua','絓'],
				['guai','怪'],
				['guai','拐'],
				['guai','乖'],
				['guan','关'],
				['guan','管'],
				['guan','观'],
				['guan','官'],
				['guan','馆'],
				['guan','惯'],
				['guan','贯'],
				['guan','冠'],
				['guan','灌'],
				['guan','罐'],
				['guan','棺'],
				['guan','鹳'],
				['guan','倌'],
				['guan','莞'],
				['guan','盥'],
				['guan','涫'],
				['guan','掼'],
				['guan','關'],
				['guan','鳏'],
				['guan','觀'],
				['guan','祼'],
				['guan','鳤'],
				['guan','矔'],
				['guan','筦'],
				['guan','雚'],
				['guan','罆'],
				['guan','貫'],
				['guan','鏆'],
				['guan','館'],
				['guan','鑵'],
				['guan','蒄'],
				['guan','輨'],
				['guan','窤'],
				['guan','錧'],
				['guang','光'],
				['guang','广'],
				['guang','逛'],
				['guang','胱'],
				['guang','咣'],
				['guang','犷'],
				['guang','銧'],
				['guang','桄'],
				['guang','硄'],
				['guang','趪'],
				['gui','规'],
				['gui','贵'],
				['gui','归'],
				['gui','鬼'],
				['gui','轨'],
				['gui','桂'],
				['gui','柜'],
				['gui','跪'],
				['gui','龟'],
				['gui','瑰'],
				['gui','诡'],
				['gui','硅'],
				['gui','闺'],
				['gui','圭'],
				['gui','皈'],
				['gui','傀'],
				['gui','刽'],
				['gui','桧'],
				['gui','鲑'],
				['gui','癸'],
				['gui','庋'],
				['gui','妫'],
				['gui','匦'],
				['gui','宄'],
				['gui','晷'],
				['gui','炅'],
				['gui','刿'],
				['gui','規'],
				['gui','鱖'],
				['gui','鳜'],
				['gui','邽'],
				['gui','貴'],
				['gui','簋'],
				['gui','螝'],
				['gui','軌'],
				['gui','龜'],
				['gui','膭'],
				['gui','鱥'],
				['gui','祪'],
				['gui','雟'],
				['gui','鴃'],
				['gui','瞆'],
				['gui','禬'],
				['gui','袿'],
				['gui','詭'],
				['gui','鬶'],
				['gui','鴂'],
				['gui','蓕'],
				['gui','赽'],
				['gui','趹'],
				['gui','瞡'],
				['gui','鮭'],
				['gui','襘'],
				['gun','滚'],
				['gun','棍'],
				['gun','衮'],
				['gun','鲧'],
				['gun','绲'],
				['gun','辊'],
				['gun','磙'],
				['gun','謴'],
				['gun','袞'],
				['gun','鮌'],
				['gun','緄'],
				['gun','鯀'],
				['gun','輥'],
				['gun','睴'],
				['guo','国'],
				['guo','过'],
				['guo','果'],
				['guo','郭'],
				['guo','锅'],
				['guo','裹'],
				['guo','涡'],
				['guo','猓'],
				['guo','過'],
				['guo','帼'],
				['guo','虢'],
				['guo','蝈'],
				['guo','聒'],
				['guo','掴'],
				['guo','馃'],
				['guo','椁'],
				['guo','崞'],
				['guo','呙'],
				['guo','埚'],
				['guo','腘'],
				['guo','蜾'],
				['guo','馘'],
				['guo','鍋'],
				['guo','膕'],
				['guo','聝'],
				['guo','綶'],
				['guo','菓'],
				['guo','粿'],
				['ha','哈'],
				['ha','铪'],
				['hai','还'],
				['hai','海'],
				['hai','孩'],
				['hai','害'],
				['hai','咳'],
				['hai','骇'],
				['hai','骸'],
				['hai','亥'],
				['hai','嗨'],
				['hai','氦'],
				['hai','還'],
				['hai','醢'],
				['hai','胲'],
				['hai','酼'],
				['hai','餀'],
				['han','汉'],
				['han','含'],
				['han','喊'],
				['han','韩'],
				['han','寒'],
				['han','汗'],
				['han','翰'],
				['han','函'],
				['han','罕'],
				['han','憾'],
				['han','涵'],
				['han','旱'],
				['han','撼'],
				['han','悍'],
				['han','捍'],
				['han','酣'],
				['han','瀚'],
				['han','憨'],
				['han','鼾'],
				['han','邯'],
				['han','焊'],
				['han','颔'],
				['han','晗'],
				['han','焓'],
				['han','蚶'],
				['han','菡'],
				['han','顸'],
				['han','邗'],
				['han','肣'],
				['han','頷'],
				['han','蔊'],
				['han','谽'],
				['han','睅'],
				['han','鋎'],
				['han','韓'],
				['han','糮'],
				['han','銲'],
				['han','魽'],
				['hang','行'],
				['hang','航'],
				['hang','杭'],
				['hang','吭'],
				['hang','夯'],
				['hang','沆'],
				['hang','颃'],
				['hang','蚢'],
				['hang','迒'],
				['hang','笐'],
				['hang','绗'],
				['hao','好'],
				['hao','号'],
				['hao','毫'],
				['hao','豪'],
				['hao','耗'],
				['hao','皓'],
				['hao','壕'],
				['hao','郝'],
				['hao','嚎'],
				['hao','昊'],
				['hao','嗥'],
				['hao','蒿'],
				['hao','蚝'],
				['hao','颢'],
				['hao','濠'],
				['hao','灏'],
				['hao','貉'],
				['hao','薅'],
				['hao','鄗'],
				['hao','嚆'],
				['hao','號'],
				['hao','蠔'],
				['hao','籇'],
				['hao','薃'],
				['he','和'],
				['he','何'],
				['he','河'],
				['he','核'],
				['he','喝'],
				['he','赫'],
				['he','荷'],
				['he','贺'],
				['he','盒'],
				['he','鹤'],
				['he','褐'],
				['he','颌'],
				['he','禾'],
				['he','劾'],
				['he','诃'],
				['he','涸'],
				['he','壑'],
				['he','阂'],
				['he','嗬'],
				['he','龢'],
				['he','菏'],
				['he','曷'],
				['he','盍'],
				['he','翮'],
				['he','饸'],
				['he','龁'],
				['he','盇'],
				['he','鞨'],
				['he','賀'],
				['he','粭'],
				['he','鶡'],
				['he','翯'],
				['he','麧'],
				['he','覈'],
				['he','鹖'],
				['he','隺'],
				['he','礉'],
				['he','蠚'],
				['he','鸖'],
				['he','頜'],
				['he','鶴'],
				['he','盉'],
				['hei','黑'],
				['hei','嘿'],
				['hei','黒'],
				['hen','很'],
				['hen','恨'],
				['hen','狠'],
				['hen','痕'],
				['hen','鞎'],
				['hen','詪'],
				['heng','横'],
				['heng','衡'],
				['heng','恒'],
				['heng','哼'],
				['heng','亨'],
				['heng','珩'],
				['heng','桁'],
				['heng','蘅'],
				['heng','鸻'],
				['heng','脝'],
				['heng','鴴'],
				['hong','红'],
				['hong','轰'],
				['hong','洪'],
				['hong','宏'],
				['hong','鸿'],
				['hong','虹'],
				['hong','哄'],
				['hong','弘'],
				['hong','烘'],
				['hong','泓'],
				['hong','荭'],
				['hong','竑'],
				['hong','闳'],
				['hong','讧'],
				['hong','訇'],
				['hong','紅'],
				['hong','薨'],
				['hong','纮'],
				['hong','蕻'],
				['hong','黉'],
				['hong','翃'],
				['hong','闀'],
				['hong','訌'],
				['hong','鋐'],
				['hong','鍧'],
				['hong','鴻'],
				['hong','紭'],
				['hong','閧'],
				['hong','軣'],
				['hong','轟'],
				['hong','紘'],
				['hong','竤'],
				['hong','谼'],
				['hou','后'],
				['hou','候'],
				['hou','厚'],
				['hou','侯'],
				['hou','喉'],
				['hou','後'],
				['hou','吼'],
				['hou','猴'],
				['hou','逅'],
				['hou','鲎'],
				['hou','糇'],
				['hou','骺'],
				['hou','篌'],
				['hou','堠'],
				['hou','睺'],
				['hou','腄'],
				['hou','瘊'],
				['hou','郈'],
				['hou','餱'],
				['hou','翭'],
				['hou','葔'],
				['hu','乎'],
				['hu','护'],
				['hu','户'],
				['hu','胡'],
				['hu','互'],
				['hu','呼'],
				['hu','忽'],
				['hu','湖'],
				['hu','虎'],
				['hu','糊'],
				['hu','狐'],
				['hu','壶'],
				['hu','沪'],
				['hu','蝴'],
				['hu','弧'],
				['hu','瑚'],
				['hu','唬'],
				['hu','葫'],
				['hu','惚'],
				['hu','浒'],
				['hu','扈'],
				['hu','琥'],
				['hu','斛'],
				['hu','鹘'],
				['hu','笏'],
				['hu','祜'],
				['hu','囫'],
				['hu','鹕'],
				['hu','槲'],
				['hu','觳'],
				['hu','怙'],
				['hu','岵'],
				['hu','唿'],
				['hu','猢'],
				['hu','煳'],
				['hu','瓠'],
				['hu','鹱'],
				['hu','醐'],
				['hu','滹'],
				['hu','護'],
				['hu','戽'],
				['hu','烀'],
				['hu','縠'],
				['hu','餬'],
				['hu','鱯'],
				['hu','鄠'],
				['hu','粐'],
				['hu','魱'],
				['hu','虖'],
				['hu','鍸'],
				['hu','鳠'],
				['hu','穫'],
				['hu','冱'],
				['hu','轷'],
				['hu','綔'],
				['hu','虝'],
				['hu','礐'],
				['hua','话'],
				['hua','化'],
				['hua','花'],
				['hua','华'],
				['hua','划'],
				['hua','画'],
				['hua','哗'],
				['hua','豁'],
				['hua','猾'],
				['hua','桦'],
				['hua','骅'],
				['hua','砉'],
				['hua','铧'],
				['hua','話'],
				['hua','華'],
				['hua','糀'],
				['hua','驊'],
				['hua','繣'],
				['hua','螖'],
				['huai','怀'],
				['huai','坏'],
				['huai','淮'],
				['huai','徊'],
				['huai','槐'],
				['huai','踝'],
				['huai','褱'],
				['huan','环'],
				['huan','欢'],
				['huan','换'],
				['huan','缓'],
				['huan','患'],
				['huan','幻'],
				['huan','唤'],
				['huan','桓'],
				['huan','焕'],
				['huan','宦'],
				['huan','痪'],
				['huan','豢'],
				['huan','奂'],
				['huan','涣'],
				['huan','寰'],
				['huan','獾'],
				['huan','浣'],
				['huan','洹'],
				['huan','圜'],
				['huan','萑'],
				['huan','鬟'],
				['huan','緩'],
				['huan','逭'],
				['huan','鹮'],
				['huan','锾'],
				['huan','漶'],
				['huan','鲩'],
				['huan','驩'],
				['huan','缳'],
				['huan','擐'],
				['huan','轘'],
				['huan','睆'],
				['huan','阛'],
				['huan','镮'],
				['huan','藧'],
				['huan','貛'],
				['huan','讙'],
				['huan','鯶'],
				['huan','瞣'],
				['huan','繯'],
				['huan','萈'],
				['huan','綄'],
				['huan','瞏'],
				['huang','黄'],
				['huang','皇'],
				['huang','荒'],
				['huang','慌'],
				['huang','晃'],
				['huang','谎'],
				['huang','煌'],
				['huang','惶'],
				['huang','凰'],
				['huang','恍'],
				['huang','幌'],
				['huang','蝗'],
				['huang','簧'],
				['huang','磺'],
				['huang','徨'],
				['huang','璜'],
				['huang','隍'],
				['huang','潢'],
				['huang','肓'],
				['huang','湟'],
				['huang','黃'],
				['huang','遑'],
				['huang','篁'],
				['huang','鳇'],
				['huang','蟥'],
				['huang','锽'],
				['huang','鰉'],
				['huang','詤'],
				['huang','鐄'],
				['huang','艎'],
				['huang','癀'],
				['huang','葟'],
				['hui','会'],
				['hui','回'],
				['hui','挥'],
				['hui','毁'],
				['hui','汇'],
				['hui','灰'],
				['hui','恢'],
				['hui','慧'],
				['hui','惠'],
				['hui','辉'],
				['hui','绘'],
				['hui','徽'],
				['hui','贿'],
				['hui','讳'],
				['hui','晦'],
				['hui','秽'],
				['hui','卉'],
				['hui','晖'],
				['hui','诲'],
				['hui','彗'],
				['hui','恚'],
				['hui','诙'],
				['hui','喙'],
				['hui','荟'],
				['hui','蛔'],
				['hui','蕙'],
				['hui','洄'],
				['hui','麾'],
				['hui','烩'],
				['hui','蟪'],
				['hui','茴'],
				['hui','虺'],
				['hui','咴'],
				['hui','隳'],
				['hui','缋'],
				['hui','翙'],
				['hui','迴'],
				['hui','阓'],
				['hui','詼'],
				['hui','翬'],
				['hui','顪'],
				['hui','篲'],
				['hui','譓'],
				['hui','鮰'],
				['hui','詯'],
				['hui','翚'],
				['hui','賄'],
				['hui','諱'],
				['hui','輝'],
				['hui','銊'],
				['hui','豗'],
				['hui','穢'],
				['hui','鐬'],
				['hui','鏸'],
				['hun','婚'],
				['hun','混'],
				['hun','魂'],
				['hun','昏'],
				['hun','浑'],
				['hun','荤'],
				['hun','诨'],
				['hun','馄'],
				['hun','阍'],
				['hun','珲'],
				['hun','溷'],
				['hun','葷'],
				['hun','繉'],
				['hun','轋'],
				['hun','睧'],
				['hun','閽'],
				['huo','或'],
				['huo','活'],
				['huo','火'],
				['huo','获'],
				['huo','货'],
				['huo','伙'],
				['huo','霍'],
				['huo','惑'],
				['huo','祸'],
				['huo','夥'],
				['huo','蠖'],
				['huo','藿'],
				['huo','嚯'],
				['huo','镬'],
				['huo','锪'],
				['huo','钬'],
				['huo','攉'],
				['huo','禍'],
				['huo','劐'],
				['huo','貨'],
				['huo','臛'],
				['huo','靃'],
				['huo','鑊'],
				['ji','机'],
				['ji','己'],
				['ji','及'],
				['ji','几'],
				['ji','计'],
				['ji','基'],
				['ji','即'],
				['ji','记'],
				['ji','济'],
				['ji','极'],
				['ji','击'],
				['ji','集'],
				['ji','级'],
				['ji','技'],
				['ji','际'],
				['ji','奇'],
				['ji','纪'],
				['ji','继'],
				['ji','急'],
				['ji','激'],
				['ji','既'],
				['ji','积'],
				['ji','吉'],
				['ji','迹'],
				['ji','疾'],
				['ji','季'],
				['ji','辑'],
				['ji','鸡'],
				['ji','剂'],
				['ji','绩'],
				['ji','籍'],
				['ji','寄'],
				['ji','挤'],
				['ji','寂'],
				['ji','祭'],
				['ji','忌'],
				['ji','肌'],
				['ji','玑'],
				['ji','饥'],
				['ji','脊'],
				['ji','稽'],
				['ji','冀'],
				['ji','藉'],
				['ji','圾'],
				['ji','姬'],
				['ji','嫉'],
				['ji','妓'],
				['ji','棘'],
				['ji','讥'],
				['ji','畸'],
				['ji','叽'],
				['ji','矶'],
				['ji','羁'],
				['ji','伎'],
				['ji','汲'],
				['ji','悸'],
				['ji','暨'],
				['ji','亟'],
				['ji','笈'],
				['ji','戟'],
				['ji','唧'],
				['ji','骥'],
				['ji','稷'],
				['ji','瘠'],
				['ji','跻'],
				['ji','嵇'],
				['ji','髻'],
				['ji','鲫'],
				['ji','岌'],
				['ji','偈'],
				['ji','蓟'],
				['ji','箕'],
				['ji','畿'],
				['ji','觊'],
				['ji','乩'],
				['ji','犄'],
				['ji','霁'],
				['ji','麂'],
				['ji','楫'],
				['ji','芨'],
				['ji','屐'],
				['ji','咭'],
				['ji','赍'],
				['ji','蒺'],
				['ji','芰'],
				['ji','嵴'],
				['ji','佶'],
				['ji','齑'],
				['ji','荠'],
				['ji','笄'],
				['ji','殛'],
				['ji','虮'],
				['ji','鲚'],
				['ji','積'],
				['ji','戢'],
				['ji','計'],
				['ji','跽'],
				['ji','墼'],
				['ji','谿'],
				['ji','繼'],
				['ji','級'],
				['ji','記'],
				['ji','洎'],
				['ji','際'],
				['ji','雞'],
				['ji','紀'],
				['ji','鹡'],
				['ji','蕺'],
				['ji','蹐'],
				['ji','罽'],
				['ji','蹟'],
				['ji','繫'],
				['ji','飢'],
				['ji','鱾'],
				['ji','穊'],
				['ji','剞'],
				['ji','跡'],
				['ji','踦'],
				['ji','韲'],
				['ji','鶺'],
				['ji','掎'],
				['ji','哜'],
				['ji','齏'],
				['ji','績'],
				['ji','羇'],
				['ji','鄿'],
				['ji','鯚'],
				['ji','臮'],
				['ji','脨'],
				['ji','虀'],
				['ji','蟿'],
				['ji','踖'],
				['ji','觭'],
				['ji','茍'],
				['ji','羈'],
				['ji','鐖'],
				['ji','訐'],
				['ji','谻'],
				['ji','穄'],
				['ji','繋'],
				['ji','蝍'],
				['ji','鍓'],
				['ji','鞿'],
				['ji','鵋'],
				['ji','稘'],
				['ji','箿'],
				['ji','緝'],
				['ji','覊'],
				['ji','鞊'],
				['ji','驥'],
				['ji','鯽'],
				['ji','磯'],
				['ji','趌'],
				['ji','禨'],
				['ji','賷'],
				['ji','鰿'],
				['ji','鱀'],
				['jia','家'],
				['jia','加'],
				['jia','价'],
				['jia','假'],
				['jia','架'],
				['jia','甲'],
				['jia','驾'],
				['jia','佳'],
				['jia','夹'],
				['jia','嘉'],
				['jia','嫁'],
				['jia','迦'],
				['jia','颊'],
				['jia','稼'],
				['jia','挟'],
				['jia','枷'],
				['jia','钾'],
				['jia','岬'],
				['jia','荚'],
				['jia','戛'],
				['jia','郏'],
				['jia','胛'],
				['jia','袈'],
				['jia','痂'],
				['jia','浃'],
				['jia','珈'],
				['jia','跏'],
				['jia','笳'],
				['jia','袷'],
				['jia','蛱'],
				['jia','葭'],
				['jia','镓'],
				['jia','铗'],
				['jia','鉀'],
				['jia','筴'],
				['jia','瘕'],
				['jia','賈'],
				['jia','恝'],
				['jia','麚'],
				['jia','頰'],
				['jia','駕'],
				['jia','豭'],
				['jia','貑'],
				['jia','莢'],
				['jia','腵'],
				['jia','郟'],
				['jian','间'],
				['jian','见'],
				['jian','建'],
				['jian','件'],
				['jian','简'],
				['jian','检'],
				['jian','坚'],
				['jian','监'],
				['jian','舰'],
				['jian','减'],
				['jian','剑'],
				['jian','渐'],
				['jian','健'],
				['jian','尖'],
				['jian','肩'],
				['jian','键'],
				['jian','箭'],
				['jian','兼'],
				['jian','践'],
				['jian','艰'],
				['jian','鉴'],
				['jian','浅'],
				['jian','歼'],
				['jian','奸'],
				['jian','剪'],
				['jian','荐'],
				['jian','贱'],
				['jian','捡'],
				['jian','碱'],
				['jian','俭'],
				['jian','煎'],
				['jian','溅'],
				['jian','拣'],
				['jian','柬'],
				['jian','谏'],
				['jian','槛'],
				['jian','茧'],
				['jian','笺'],
				['jian','僭'],
				['jian','涧'],
				['jian','睑'],
				['jian','缄'],
				['jian','锏'],
				['jian','鞯'],
				['jian','饯'],
				['jian','翦'],
				['jian','笕'],
				['jian','間'],
				['jian','謇'],
				['jian','腱'],
				['jian','見'],
				['jian','蹇'],
				['jian','戬'],
				['jian','菅'],
				['jian','硷'],
				['jian','裥'],
				['jian','犍'],
				['jian','缣'],
				['jian','篯'],
				['jian','湔'],
				['jian','趼'],
				['jian','簡'],
				['jian','毽'],
				['jian','鹣'],
				['jian','鞬'],
				['jian','囝'],
				['jian','戋'],
				['jian','蒹'],
				['jian','谫'],
				['jian','搛'],
				['jian','鹼'],
				['jian','鰜'],
				['jian','鵳'],
				['jian','鑑'],
				['jian','閒'],
				['jian','鹹'],
				['jian','枧'],
				['jian','钘'],
				['jian','鍵'],
				['jian','葌'],
				['jian','鲣'],
				['jian','監'],
				['jian','瞼'],
				['jian','踐'],
				['jian','艦'],
				['jian','鬋'],
				['jian','麉'],
				['jian','鰹'],
				['jian','薦'],
				['jian','踺'],
				['jian','鳒'],
				['jian','鋻'],
				['jian','楗'],
				['jian','瞷'],
				['jian','諓'],
				['jian','蕳'],
				['jian','礀'],
				['jian','鐧'],
				['jian','鍳'],
				['jian','趝'],
				['jian','醎'],
				['jian','銒'],
				['jian','葏'],
				['jian','繝'],
				['jian','虃'],
				['jian','蠒'],
				['jian','臶'],
				['jian','箋'],
				['jiang','将'],
				['jiang','讲'],
				['jiang','江'],
				['jiang','降'],
				['jiang','蒋'],
				['jiang','奖'],
				['jiang','疆'],
				['jiang','匠'],
				['jiang','僵'],
				['jiang','姜'],
				['jiang','浆'],
				['jiang','桨'],
				['jiang','酱'],
				['jiang','缰'],
				['jiang','绛'],
				['jiang','犟'],
				['jiang','豇'],
				['jiang','糨'],
				['jiang','薑'],
				['jiang','礓'],
				['jiang','鳉'],
				['jiang','講'],
				['jiang','耩'],
				['jiang','洚'],
				['jiang','螀'],
				['jiang','蔣'],
				['jiang','鱂'],
				['jiang','絳'],
				['jiang','謽'],
				['jiao','教'],
				['jiao','交'],
				['jiao','觉'],
				['jiao','叫'],
				['jiao','较'],
				['jiao','校'],
				['jiao','角'],
				['jiao','脚'],
				['jiao','焦'],
				['jiao','缴'],
				['jiao','娇'],
				['jiao','胶'],
				['jiao','骄'],
				['jiao','郊'],
				['jiao','轿'],
				['jiao','狡'],
				['jiao','绞'],
				['jiao','搅'],
				['jiao','嚼'],
				['jiao','浇'],
				['jiao','矫'],
				['jiao','礁'],
				['jiao','蕉'],
				['jiao','椒'],
				['jiao','窖'],
				['jiao','侥'],
				['jiao','蛟'],
				['jiao','酵'],
				['jiao','饺'],
				['jiao','跤'],
				['jiao','佼'],
				['jiao','醮'],
				['jiao','皎'],
				['jiao','铰'],
				['jiao','姣'],
				['jiao','較'],
				['jiao','鲛'],
				['jiao','峤'],
				['jiao','湫'],
				['jiao','艽'],
				['jiao','覺'],
				['jiao','鹪'],
				['jiao','敫'],
				['jiao','茭'],
				['jiao','僬'],
				['jiao','徼'],
				['jiao','矯'],
				['jiao','膠'],
				['jiao','腳'],
				['jiao','蹻'],
				['jiao','轎'],
				['jiao','噍'],
				['jiao','萩'],
				['jiao','挢'],
				['jiao','蟭'],
				['jiao','筊'],
				['jiao','膲'],
				['jiao','藠'],
				['jiao','骹'],
				['jiao','餃'],
				['jiao','簥'],
				['jiao','覐'],
				['jiao','訆'],
				['jiao','轇'],
				['jiao','繳'],
				['jiao','絞'],
				['jiao','蟜'],
				['jiao','鐎'],
				['jiao','覚'],
				['jiao','釥'],
				['jiao','驕'],
				['jiao','鷦'],
				['jie','解'],
				['jie','结'],
				['jie','接'],
				['jie','界'],
				['jie','节'],
				['jie','阶'],
				['jie','姐'],
				['jie','介'],
				['jie','借'],
				['jie','街'],
				['jie','杰'],
				['jie','皆'],
				['jie','届'],
				['jie','洁'],
				['jie','截'],
				['jie','戒'],
				['jie','揭'],
				['jie','捷'],
				['jie','劫'],
				['jie','竭'],
				['jie','诫'],
				['jie','桔'],
				['jie','诘'],
				['jie','睫'],
				['jie','偕'],
				['jie','秸'],
				['jie','嗟'],
				['jie','芥'],
				['jie','桀'],
				['jie','拮'],
				['jie','孑'],
				['jie','碣'],
				['jie','婕'],
				['jie','羯'],
				['jie','結'],
				['jie','疥'],
				['jie','蚧'],
				['jie','讦'],
				['jie','疖'],
				['jie','節'],
				['jie','階'],
				['jie','絜'],
				['jie','喈'],
				['jie','袓'],
				['jie','褯'],
				['jie','蛣'],
				['jie','鲒'],
				['jie','脻'],
				['jie','蓵'],
				['jie','稭'],
				['jie','蜐'],
				['jie','誡'],
				['jie','詰'],
				['jie','衸'],
				['jie','菨'],
				['jie','蛶'],
				['jie','飷'],
				['jin','进'],
				['jin','金'],
				['jin','今'],
				['jin','近'],
				['jin','尽'],
				['jin','仅'],
				['jin','紧'],
				['jin','禁'],
				['jin','津'],
				['jin','劲'],
				['jin','晋'],
				['jin','谨'],
				['jin','锦'],
				['jin','筋'],
				['jin','巾'],
				['jin','浸'],
				['jin','襟'],
				['jin','矜'],
				['jin','烬'],
				['jin','瑾'],
				['jin','靳'],
				['jin','噤'],
				['jin','荩'],
				['jin','觐'],
				['jin','馑'],
				['jin','缙'],
				['jin','槿'],
				['jin','妗'],
				['jin','進'],
				['jin','钅'],
				['jin','衿'],
				['jin','堇'],
				['jin','盡'],
				['jin','卺'],
				['jin','緊'],
				['jin','廑'],
				['jin','謹'],
				['jin','觔'],
				['jin','赆'],
				['jin','賮'],
				['jin','菫'],
				['jin','齽'],
				['jin','砛'],
				['jin','祲'],
				['jin','蓳'],
				['jin','錦'],
				['jing','经'],
				['jing','精'],
				['jing','京'],
				['jing','境'],
				['jing','惊'],
				['jing','警'],
				['jing','竟'],
				['jing','静'],
				['jing','景'],
				['jing','睛'],
				['jing','竞'],
				['jing','敬'],
				['jing','镜'],
				['jing','径'],
				['jing','净'],
				['jing','井'],
				['jing','晶'],
				['jing','颈'],
				['jing','靖'],
				['jing','荆'],
				['jing','鲸'],
				['jing','茎'],
				['jing','兢'],
				['jing','阱'],
				['jing','菁'],
				['jing','痉'],
				['jing','憬'],
				['jing','經'],
				['jing','胫'],
				['jing','旌'],
				['jing','泾'],
				['jing','迳'],
				['jing','儆'],
				['jing','靓'],
				['jing','鏡'],
				['jing','肼'],
				['jing','粳'],
				['jing','婧'],
				['jing','頸'],
				['jing','獍'],
				['jing','驚'],
				['jing','腈'],
				['jing','靜'],
				['jing','莖'],
				['jing','鼱'],
				['jing','刭'],
				['jing','競'],
				['jing','麖'],
				['jing','秔'],
				['jing','脛'],
				['jing','逕'],
				['jing','蟼'],
				['jing','竸'],
				['jing','荊'],
				['jing','穽'],
				['jing','鯨'],
				['jiong','炯'],
				['jiong','窘'],
				['jiong','迥'],
				['jiong','颎'],
				['jiong','扃'],
				['jiong','絅'],
				['jiong','褧'],
				['jiong','駉'],
				['jiong','蘔'],
				['jiong','綗'],
				['jiu','就'],
				['jiu','究'],
				['jiu','九'],
				['jiu','久'],
				['jiu','酒'],
				['jiu','救'],
				['jiu','旧'],
				['jiu','纠'],
				['jiu','舅'],
				['jiu','揪'],
				['jiu','疚'],
				['jiu','咎'],
				['jiu','鸠'],
				['jiu','灸'],
				['jiu','臼'],
				['jiu','啾'],
				['jiu','厩'],
				['jiu','韭'],
				['jiu','鹫'],
				['jiu','赳'],
				['jiu','柩'],
				['jiu','阄'],
				['jiu','玖'],
				['jiu','僦'],
				['jiu','桕'],
				['jiu','紤'],
				['jiu','鬏'],
				['jiu','舊'],
				['jiu','鷲'],
				['jiu','糺'],
				['jiu','鳩'],
				['jiu','韮'],
				['ju','据'],
				['ju','具'],
				['ju','局'],
				['ju','举'],
				['ju','居'],
				['ju','句'],
				['ju','剧'],
				['ju','巨'],
				['ju','距'],
				['ju','聚'],
				['ju','拒'],
				['ju','惧'],
				['ju','俱'],
				['ju','拘'],
				['ju','矩'],
				['ju','菊'],
				['ju','沮'],
				['ju','鞠'],
				['ju','锯'],
				['ju','瞿'],
				['ju','炬'],
				['ju','咀'],
				['ju','橘'],
				['ju','狙'],
				['ju','踞'],
				['ju','驹'],
				['ju','遽'],
				['ju','飓'],
				['ju','疽'],
				['ju','榘'],
				['ju','掬'],
				['ju','钜'],
				['ju','榉'],
				['ju','苣'],
				['ju','趄'],
				['ju','龃'],
				['ju','莒'],
				['ju','倨'],
				['ju','踽'],
				['ju','讵'],
				['ju','雎'],
				['ju','鞫'],
				['ju','裾'],
				['ju','苴'],
				['ju','醵'],
				['ju','窭'],
				['ju','鮈'],
				['ju','锔'],
				['ju','琚'],
				['ju','椐'],
				['ju','舉'],
				['ju','虡'],
				['ju','屦'],
				['ju','鐻'],
				['ju','犋'],
				['ju','秬'],
				['ju','鉅'],
				['ju','踘'],
				['ju','鼰'],
				['ju','鋸'],
				['ju','粷'],
				['ju','蒟'],
				['ju','臄'],
				['ju','跔'],
				['ju','筥'],
				['ju','簴'],
				['ju','艍'],
				['ju','粔'],
				['ju','罝'],
				['ju','閰'],
				['ju','駶'],
				['ju','輂'],
				['juan','卷'],
				['juan','圈'],
				['juan','捐'],
				['juan','倦'],
				['juan','绢'],
				['juan','娟'],
				['juan','眷'],
				['juan','鹃'],
				['juan','涓'],
				['juan','镌'],
				['juan','蠲'],
				['juan','锩'],
				['juan','鄄'],
				['juan','狷'],
				['juan','踡'],
				['juan','睊'],
				['juan','臇'],
				['juan','脧'],
				['juan','睠'],
				['juan','絹'],
				['juan','錈'],
				['juan','絭'],
				['juan','罥'],
				['juan','蕋'],
				['juan','鎸'],
				['Juàn','邀'],
				['Juàn','斤'],
				['Juàn','隽'],
				['Juàn','雋'],
				['jue','决'],
				['jue','绝'],
				['jue','爵'],
				['jue','掘'],
				['jue','厥'],
				['jue','诀'],
				['jue','崛'],
				['jue','倔'],
				['jue','抉'],
				['jue','攫'],
				['jue','獗'],
				['jue','撅'],
				['jue','蕨'],
				['jue','蹶'],
				['jue','噘'],
				['jue','谲'],
				['jue','橛'],
				['jue','珏'],
				['jue','噱'],
				['jue','矍'],
				['jue','镢'],
				['jue','桷'],
				['jue','劂'],
				['jue','孓'],
				['jue','絕'],
				['jue','爝'],
				['jue','钁'],
				['jue','觖'],
				['jue','觼'],
				['jue','蕝'],
				['jue','蹷'],
				['jue','誳'],
				['jue','矡'],
				['jue','繑'],
				['jue','芵'],
				['jue','訣'],
				['jue','蟨'],
				['jue','穱'],
				['jue','絶'],
				['jue','躩'],
				['jun','莙'],
				['Jūn','军'],
				['Jūn','均'],
				['Jūn','君'],
				['Jūn','菌'],
				['Jūn','俊'],
				['Jūn','址'],
				['Jūn','庞'],
				['Jūn','耕'],
				['Jūn','锐'],
				['Jūn','缝'],
				['Jūn','悔'],
				['Jūn','玲'],
				['Jūn','惟'],
				['Jūn','斥'],
				['Jūn','添'],
				['Jūn','挖'],
				['Jūn','呵'],
				['Jūn','讼'],
				['Jūn','氧'],
				['Jūn','浩'],
				['Jūn','羽'],
				['Jūn','酷'],
				['Jūn','掠'],
				['Jūn','妖'],
				['Jūn','郡'],
				['Jūn','钧'],
				['Jūn','峻'],
				['Jūn','骏'],
				['Jūn','竣'],
				['Jūn','浚'],
				['Jūn','麇'],
				['Jūn','皲'],
				['Jūn','捃'],
				['Jūn','馂'],
				['Jūn','軍'],
				['Jūn','麕'],
				['Jūn','鲪'],
				['Jūn','袀'],
				['Jūn','箘'],
				['Jūn','碅'],
				['Jūn','麏'],
				['Jūn','蜠'],
				['Jūn','覠'],
				['ka','卡'],
				['ka','咖'],
				['ka','喀'],
				['ka','咔'],
				['ka','胩'],
				['ka','佧'],
				['kai','开'],
				['kai','凯'],
				['kai','慨'],
				['kai','恺'],
				['kai','楷'],
				['kai','揩'],
				['kai','铠'],
				['kai','闿'],
				['kai','锴'],
				['kai','忾'],
				['kai','開'],
				['kai','锎'],
				['kai','剀'],
				['kai','蒈'],
				['kai','垲'],
				['kan','看'],
				['kan','刊'],
				['kan','堪'],
				['kan','砍'],
				['kan','坎'],
				['kan','勘'],
				['kan','侃'],
				['kan','瞰'],
				['kan','龛'],
				['kan','戡'],
				['kan','阚'],
				['kan','磡'],
				['kan','矙'],
				['kan','竷'],
				['kan','轗'],
				['kang','抗'],
				['kang','康'],
				['kang','慷'],
				['kang','炕'],
				['kang','亢'],
				['kang','糠'],
				['kang','闶'],
				['kang','伉'],
				['kang','钪'],
				['kang','鱇'],
				['kang','鈧'],
				['kang','躿'],
				['kao','考'],
				['kao','靠'],
				['kao','烤'],
				['kao','拷'],
				['kao','铐'],
				['kao','犒'],
				['kao','尻'],
				['kao','栲'],
				['kao','訄'],
				['kao','鲓'],
				['ke','可'],
				['ke','克'],
				['ke','科'],
				['ke','客'],
				['ke','刻'],
				['ke','课'],
				['ke','颗'],
				['ke','柯'],
				['ke','壳'],
				['ke','渴'],
				['ke','棵'],
				['ke','磕'],
				['ke','苛'],
				['ke','恪'],
				['ke','珂'],
				['ke','轲'],
				['ke','坷'],
				['ke','瞌'],
				['ke','蝌'],
				['ke','窠'],
				['ke','嗑'],
				['ke','颏'],
				['ke','缂'],
				['ke','岢'],
				['ke','髁'],
				['ke','氪'],
				['ke','稞'],
				['ke','疴'],
				['ke','溘'],
				['ke','锞'],
				['ke','課'],
				['ke','骒'],
				['ke','簻'],
				['ke','顆'],
				['ke','钶'],
				['ke','艐'],
				['ke','袔'],
				['ke','窼'],
				['ke','趷'],
				['ke','碦'],
				['ke','礊'],
				['ke','薖'],
				['ke','軻'],
				['ke','騍'],
				['ke','頦'],
				['ken','肯'],
				['ken','恳'],
				['ken','垦'],
				['ken','啃'],
				['ken','龈'],
				['ken','裉'],
				['ken','頎'],
				['ken','肻'],
				['keng','坑'],
				['keng','铿'],
				['keng','硁'],
				['kong','空'],
				['kong','控'],
				['kong','恐'],
				['kong','孔'],
				['kong','崆'],
				['kong','倥'],
				['kong','箜'],
				['kong','鞚'],
				['kong','躻'],
				['kong','鵼'],
				['kou','口'],
				['kou','扣'],
				['kou','寇'],
				['kou','叩'],
				['kou','抠'],
				['kou','蔻'],
				['kou','芤'],
				['kou','眍'],
				['kou','筘'],
				['kou','蔲'],
				['kou','鷇'],
				['kou','釦'],
				['ku','苦'],
				['ku','库'],
				['ku','哭'],
				['ku','裤'],
				['ku','枯'],
				['ku','窟'],
				['ku','骷'],
				['ku','绔'],
				['ku','堀'],
				['ku','喾'],
				['ku','袴'],
				['ku','矻'],
				['ku','刳'],
				['ku','齁'],
				['ku','跍'],
				['ku','窋'],
				['ku','褲'],
				['ku','郀'],
				['ku','秙'],
				['kua','跨'],
				['kua','夸'],
				['kua','垮'],
				['kua','挎'],
				['kua','胯'],
				['kua','侉'],
				['kua','顝'],
				['kua','誇'],
				['kua','骻'],
				['kua','銙'],
				['kuai','快'],
				['kuai','块'],
				['kuai','筷'],
				['kuai','蒯'],
				['kuai','脍'],
				['kuai','侩'],
				['kuai','狯'],
				['kuai','哙'],
				['kuai','浍'],
				['kuai','鲙'],
				['kuai','郐'],
				['kuai','鄶'],
				['kuai','駃'],
				['kuan','款'],
				['kuan','宽'],
				['kuan','髋'],
				['kuan','窾'],
				['kuang','况'],
				['kuang','狂'],
				['kuang','矿'],
				['kuang','框'],
				['kuang','旷'],
				['kuang','眶'],
				['kuang','匡'],
				['kuang','筐'],
				['kuang','诳'],
				['kuang','邝'],
				['kuang','诓'],
				['kuang','哐'],
				['kuang','圹'],
				['kuang','夼'],
				['kuang','纩'],
				['kuang','贶'],
				['kuang','礦'],
				['kuang','軠'],
				['kuang','鑛'],
				['kuang','眖'],
				['kuang','黋'],
				['kuang','邼'],
				['kuang','鵟'],
				['kuang','鄺'],
				['kui','亏'],
				['kui','溃'],
				['kui','愧'],
				['kui','魁'],
				['kui','奎'],
				['kui','窥'],
				['kui','馈'],
				['kui','盔'],
				['kui','葵'],
				['kui','匮'],
				['kui','逵'],
				['kui','睽'],
				['kui','夔'],
				['kui','喟'],
				['kui','揆'],
				['kui','馗'],
				['kui','篑'],
				['kui','聩'],
				['kui','愦'],
				['kui','暌'],
				['kui','岿'],
				['kui','喹'],
				['kui','跬'],
				['kui','悝'],
				['kui','蝰'],
				['kui','虧'],
				['kui','骙'],
				['kui','聭'],
				['kui','饋'],
				['kui','頍'],
				['kui','蒉'],
				['kui','聧'],
				['kui','籄'],
				['kui','闚'],
				['kui','聵'],
				['kui','蹞'],
				['kui','餽'],
				['kun','困'],
				['kun','昆'],
				['kun','捆'],
				['kun','坤'],
				['kun','琨'],
				['kun','锟'],
				['kun','髡'],
				['kun','阃'],
				['kun','鲲'],
				['kun','醌'],
				['kun','悃'],
				['kun','硱'],
				['kun','裈'],
				['kun','睏'],
				['kun','稛'],
				['kun','鹍'],
				['kun','髠'],
				['kun','綑'],
				['kun','裩'],
				['kun','褌'],
				['kun','蜫'],
				['kun','臗'],
				['kun','稇'],
				['kuo','括'],
				['kuo','扩'],
				['kuo','阔'],
				['kuo','廓'],
				['kuo','蛞'],
				['kuo','闊'],
				['kuo','筈'],
				['kuo','霩'],
				['kuo','秳'],
				['kuo','鞟'],
				['la','拉'],
				['la','落'],
				['la','蓝'],
				['la','啦'],
				['la','腊'],
				['la','辣'],
				['la','蜡'],
				['la','垃'],
				['la','喇'],
				['la','剌'],
				['la','邋'],
				['la','旯'],
				['la','砬'],
				['la','瘌'],
				['la','鱲'],
				['la','鞡'],
				['la','镴'],
				['la','藞'],
				['la','臘'],
				['la','辢'],
				['la','臈'],
				['la','蠟'],
				['lai','来'],
				['lai','莱'],
				['lai','赖'],
				['lai','睐'],
				['lai','籁'],
				['lai','濑'],
				['lai','癞'],
				['lai','涞'],
				['lai','徕'],
				['lai','崃'],
				['lai','赉'],
				['lai','铼'],
				['lai','賴'],
				['lai','睞'],
				['lai','萊'],
				['lai','鶆'],
				['lai','賚'],
				['lai','鵣'],
				['lan','兰'],
				['lan','烂'],
				['lan','览'],
				['lan','拦'],
				['lan','栏'],
				['lan','懒'],
				['lan','滥'],
				['lan','篮'],
				['lan','岚'],
				['lan','澜'],
				['lan','揽'],
				['lan','缆'],
				['lan','婪'],
				['lan','榄'],
				['lan','阑'],
				['lan','斓'],
				['lan','褴'],
				['lan','镧'],
				['lan','谰'],
				['lan','罱'],
				['lan','襕'],
				['lan','蘭'],
				['lan','讕'],
				['lan','糷'],
				['lan','葻'],
				['lan','藍'],
				['lan','漤'],
				['lan','籣'],
				['lan','闌'],
				['lan','繿'],
				['lan','襽'],
				['lang','浪'],
				['lang','朗'],
				['lang','郎'],
				['lang','狼'],
				['lang','廊'],
				['lang','琅'],
				['lang','螂'],
				['lang','榔'],
				['lang','啷'],
				['lang','阆'],
				['lang','锒'],
				['lang','莨'],
				['lang','蒗'],
				['lang','鎯'],
				['lang','艆'],
				['lang','郞'],
				['lang','稂'],
				['lang','脼'],
				['lang','閬'],
				['lang','羮'],
				['lao','老'],
				['lao','劳'],
				['lao','络'],
				['lao','牢'],
				['lao','捞'],
				['lao','姥'],
				['lao','佬'],
				['lao','唠'],
				['lao','烙'],
				['lao','酪'],
				['lao','潦'],
				['lao','涝'],
				['lao','崂'],
				['lao','痨'],
				['lao','醪'],
				['lao','铑'],
				['lao','顟'],
				['lao','栳'],
				['lao','粩'],
				['lao','蛯'],
				['lao','耢'],
				['lao','荖'],
				['lao','铹'],
				['lao','耮'],
				['lao','蟧'],
				['le','了'],
				['le','乐'],
				['le','勒'],
				['le','叻'],
				['le','泐'],
				['le','饹'],
				['le','仂'],
				['le','鳓'],
				['le','簕'],
				['le','韷'],
				['lei','类'],
				['lei','雷'],
				['lei','泪'],
				['lei','累'],
				['lei','垒'],
				['lei','肋'],
				['lei','蕾'],
				['lei','磊'],
				['lei','擂'],
				['lei','儡'],
				['lei','镭'],
				['lei','類'],
				['lei','诔'],
				['lei','羸'],
				['lei','嘞'],
				['lei','耒'],
				['lei','嫘'],
				['lei','缧'],
				['lei','礌'],
				['lei','埒'],
				['lei','酹'],
				['lei','蔂'],
				['lei','檑'],
				['lei','絫'],
				['lei','磥'],
				['lei','蕌'],
				['lei','礨'],
				['lei','罍'],
				['lei','鐳'],
				['lei','鑘'],
				['lei','鸓'],
				['lei','腂'],
				['lei','蠝'],
				['lei','纍'],
				['leng','冷'],
				['leng','愣'],
				['leng','棱'],
				['leng','楞'],
				['leng','塄'],
				['leng','稜'],
				['leng','睖'],
				['leng','薐'],
				['leng','輘'],
				['li','里'],
				['li','理'],
				['li','力'],
				['li','利'],
				['li','立'],
				['li','离'],
				['li','李'],
				['li','历'],
				['li','例'],
				['li','丽'],
				['li','礼'],
				['li','厉'],
				['li','励'],
				['li','黎'],
				['li','粒'],
				['li','隶'],
				['li','莉'],
				['li','璃'],
				['li','哩'],
				['li','厘'],
				['li','吏'],
				['li','栗'],
				['li','狸'],
				['li','梨'],
				['li','篱'],
				['li','犁'],
				['li','鲤'],
				['li','笠'],
				['li','漓'],
				['li','砾'],
				['li','雳'],
				['li','俐'],
				['li','沥'],
				['li','荔'],
				['li','痢'],
				['li','罹'],
				['li','戾'],
				['li','蠡'],
				['li','俚'],
				['li','蛎'],
				['li','藜'],
				['li','詈'],
				['li','俪'],
				['li','栎'],
				['li','澧'],
				['li','砺'],
				['li','蜊'],
				['li','骊'],
				['li','莅'],
				['li','锂'],
				['li','醴'],
				['li','娌'],
				['li','郦'],
				['li','嫠'],
				['li','傈'],
				['li','枥'],
				['li','跞'],
				['li','鹂'],
				['li','逦'],
				['li','鲡'],
				['li','裡'],
				['li','唳'],
				['li','粝'],
				['li','鳢'],
				['li','疠'],
				['li','呖'],
				['li','溧'],
				['li','黧'],
				['li','喱'],
				['li','缡'],
				['li','離'],
				['li','苈'],
				['li','猁'],
				['li','鱧'],
				['li','疬'],
				['li','釐'],
				['li','轹'],
				['li','篥'],
				['li','坜'],
				['li','粚'],
				['li','麗'],
				['li','裏'],
				['li','麜'],
				['li','蓠'],
				['li','醨'],
				['li','隸'],
				['li','褵'],
				['li','蒞'],
				['li','脷'],
				['li','苙'],
				['li','磿'],
				['li','裏'],
				['li','綟'],
				['li','謧'],
				['li','鯏'],
				['li','粴'],
				['li','讈'],
				['li','鋫'],
				['li','禮'],
				['li','藶'],
				['li','蠣'],
				['li','盠'],
				['li','赲'],
				['li','邌'],
				['li','鯬'],
				['li','菞'],
				['li','筣'],
				['li','籬'],
				['li','鋰'],
				['li','鱺'],
				['lia','俩'],
				['lian','联'],
				['lian','连'],
				['lian','脸'],
				['lian','练'],
				['lian','恋'],
				['lian','怜'],
				['lian','炼'],
				['lian','廉'],
				['lian','莲'],
				['lian','链'],
				['lian','帘'],
				['lian','敛'],
				['lian','琏'],
				['lian','镰'],
				['lian','涟'],
				['lian','濂'],
				['lian','殓'],
				['lian','臁'],
				['lian','鲢'],
				['lian','裢'],
				['lian','蠊'],
				['lian','奁'],
				['lian','楝'],
				['lian','裣'],
				['lian','連'],
				['lian','練'],
				['lian','潋'],
				['lian','鐮'],
				['lian','蓮'],
				['lian','臉'],
				['lian','聯'],
				['lian','簾'],
				['lian','鎌'],
				['lian','鍊'],
				['lian','鬑'],
				['lian','聫'],
				['lian','羷'],
				['lian','鏈'],
				['lian','稴'],
				['liang','两'],
				['liang','量'],
				['liang','良'],
				['liang','亮'],
				['liang','粮'],
				['liang','辆'],
				['liang','梁'],
				['liang','凉'],
				['liang','谅'],
				['liang','踉'],
				['liang','粱'],
				['liang','晾'],
				['liang','椋'],
				['liang','魉'],
				['liang','辌'],
				['liang','裲'],
				['liang','墚'],
				['liang','綡'],
				['liang','蜽'],
				['liang','糧'],
				['liang','諒'],
				['liao','料'],
				['liao','疗'],
				['liao','聊'],
				['liao','辽'],
				['liao','僚'],
				['liao','廖'],
				['liao','寥'],
				['liao','撩'],
				['liao','缭'],
				['liao','撂'],
				['liao','燎'],
				['liao','瞭'],
				['liao','镣'],
				['liao','寮'],
				['liao','獠'],
				['liao','嘹'],
				['liao','鹩'],
				['liao','钌'],
				['liao','尥'],
				['liao','蓼'],
				['liao','豂'],
				['liao','镠'],
				['liao','膫'],
				['liao','蹽'],
				['liao','繚'],
				['liao','鏐'],
				['liao','鐐'],
				['liao','膋'],
				['liao','竂'],
				['lie','列'],
				['lie','烈'],
				['lie','裂'],
				['lie','猎'],
				['lie','劣'],
				['lie','咧'],
				['lie','洌'],
				['lie','冽'],
				['lie','趔'],
				['lie','鬣'],
				['lie','捩'],
				['lie','躐'],
				['lie','鬛'],
				['lie','蛚'],
				['lie','鴷'],
				['lie','睙'],
				['lie','茢'],
				['lie','脟'],
				['lin','林'],
				['lin','临'],
				['lin','邻'],
				['lin','淋'],
				['lin','琳'],
				['lin','麟'],
				['lin','鳞'],
				['lin','磷'],
				['lin','凛'],
				['lin','赁'],
				['lin','霖'],
				['lin','吝'],
				['lin','躏'],
				['lin','粼'],
				['lin','嶙'],
				['lin','懔'],
				['lin','遴'],
				['lin','蔺'],
				['lin','廪'],
				['lin','啉'],
				['lin','檩'],
				['lin','臨'],
				['lin','辚'],
				['lin','膦'],
				['lin','箖'],
				['lin','鄰'],
				['lin','麐'],
				['lin','粦'],
				['lin','驎'],
				['lin','瞵'],
				['lin','鱗'],
				['lin','鏻'],
				['lin','藺'],
				['lin','轥'],
				['lin','隣'],
				['lin','魿'],
				['ling','领'],
				['ling','令'],
				['ling','另'],
				['ling','灵'],
				['ling','零'],
				['ling','龄'],
				['ling','凌'],
				['ling','陵'],
				['ling','铃'],
				['ling','岭'],
				['ling','菱'],
				['ling','伶'],
				['ling','绫'],
				['ling','拎'],
				['ling','翎'],
				['ling','聆'],
				['ling','羚'],
				['ling','苓'],
				['ling','蛉'],
				['ling','呤'],
				['ling','棂'],
				['ling','囹'],
				['ling','鲮'],
				['ling','泠'],
				['ling','瓴'],
				['ling','酃'],
				['ling','靈'],
				['ling','鸰'],
				['ling','領'],
				['ling','袊'],
				['ling','齡'],
				['ling','舲'],
				['ling','笭'],
				['ling','跉'],
				['ling','柃'],
				['ling','齢'],
				['ling','鈴'],
				['ling','秢'],
				['ling','祾'],
				['ling','詅'],
				['ling','醽'],
				['ling','霝'],
				['ling','鴒'],
				['liu','流'],
				['liu','六'],
				['liu','留'],
				['liu','陆'],
				['liu','刘'],
				['liu','柳'],
				['liu','溜'],
				['liu','瘤'],
				['liu','榴'],
				['liu','硫'],
				['liu','浏'],
				['liu','琉'],
				['liu','绺'],
				['liu','遛'],
				['liu','馏'],
				['liu','镏'],
				['liu','鎏'],
				['liu','熘'],
				['liu','鹨'],
				['liu','旒'],
				['liu','锍'],
				['liu','蹓'],
				['liu','骝'],
				['liu','鰡'],
				['liu','飗'],
				['liu','罶'],
				['liu','駵'],
				['liu','霤'],
				['liu','羀'],
				['liu','綹'],
				['liu','飀'],
				['liu','鬸'],
				['liu','鹠'],
				['liu','藰'],
				['liu','驑'],
				['long','龙'],
				['long','弄'],
				['long','隆'],
				['long','笼'],
				['long','垄'],
				['long','拢'],
				['long','胧'],
				['long','聋'],
				['long','咙'],
				['long','陇'],
				['long','珑'],
				['long','窿'],
				['long','茏'],
				['long','泷'],
				['long','垅'],
				['long','砻'],
				['long','眬'],
				['long','栊'],
				['long','龍'],
				['long','衖'],
				['long','癃'],
				['long','鑨'],
				['long','籠'],
				['long','襱'],
				['long','霳'],
				['long','躘'],
				['long','驡'],
				['long','礱'],
				['long','蠪'],
				['long','篭'],
				['long','龓'],
				['long','聾'],
				['lou','楼'],
				['lou','漏'],
				['lou','陋'],
				['lou','搂'],
				['lou','喽'],
				['lou','髅'],
				['lou','篓'],
				['lou','娄'],
				['lou','镂'],
				['lou','偻'],
				['lou','瘘'],
				['lou','蝼'],
				['lou','嵝'],
				['lou','蒌'],
				['lou','耧'],
				['lou','謱'],
				['lou','簍'],
				['lou','鏤'],
				['lou','蔞'],
				['lou','髏'],
				['lou','鞻'],
				['lu','路'],
				['lu','露'],
				['lu','鲁'],
				['lu','录'],
				['lu','卢'],
				['lu','炉'],
				['lu','鹿'],
				['lu','禄'],
				['lu','虏'],
				['lu','芦'],
				['lu','碌'],
				['lu','庐'],
				['lu','颅'],
				['lu','噜'],
				['lu','赂'],
				['lu','戮'],
				['lu','掳'],
				['lu','麓'],
				['lu','漉'],
				['lu','卤'],
				['lu','璐'],
				['lu','辘'],
				['lu','箓'],
				['lu','撸'],
				['lu','橹'],
				['lu','潞'],
				['lu','鲈'],
				['lu','鹭'],
				['lu','鸬'],
				['lu','胪'],
				['lu','泸'],
				['lu','渌'],
				['lu','舻'],
				['lu','轳'],
				['lu','逯'],
				['lu','栌'],
				['lu','辂'],
				['lu','氇'],
				['lu','垆'],
				['lu','顱'],
				['lu','陸'],
				['lu','簏'],
				['lu','錄'],
				['lu','鈩'],
				['lu','蕗'],
				['lu','菉'],
				['lu','蘆'],
				['lu','镥'],
				['lu','盝'],
				['lu','纑'],
				['lu','蓾'],
				['lu','録'],
				['lu','鴼'],
				['lu','祿'],
				['lu','簬'],
				['lu','粶'],
				['lu','騼'],
				['lu','膟'],
				['lu','艫'],
				['lu','騄'],
				['lu','睩'],
				['lu','稑'],
				['lu','醁'],
				['lu','臚'],
				['lu','賂'],
				['lu','虜'],
				['lu','籙'],
				['lu','艣'],
				['lu','鷺'],
				['lu','磠'],
				['lu','鑪'],
				['lu:','律'],
				['lu:','率'],
				['lu:','虑'],
				['lu:','旅'],
				['lu:','绿'],
				['lu:','吕'],
				['lu:','履'],
				['lu:','屡'],
				['lu:','驴'],
				['lu:','侣'],
				['lu:','缕'],
				['lu:','氯'],
				['lu:','铝'],
				['lu:','滤'],
				['lu:','榈'],
				['lu:','捋'],
				['lu:','闾'],
				['lu:','褛'],
				['lu:','膂'],
				['luan','乱'],
				['luan','卵'],
				['luan','挛'],
				['luan','峦'],
				['luan','鸾'],
				['luan','銮'],
				['luan','孪'],
				['luan','栾'],
				['luan','滦'],
				['luan','脔'],
				['luan','娈'],
				['luan','釠'],
				['luan','鑾'],
				['luan','羉'],
				['lue','略'],
				['lue','锊'],
				['lun','论'],
				['lun','伦'],
				['lun','轮'],
				['lun','仑'],
				['lun','沦'],
				['lun','纶'],
				['lun','抡'],
				['lun','囵'],
				['lun','論'],
				['lun','輪'],
				['lun','菕'],
				['lun','鯩'],
				['lun','腀'],
				['luo','罗'],
				['luo','洛'],
				['luo','逻'],
				['luo','裸'],
				['luo','螺'],
				['luo','萝'],
				['luo','骆'],
				['luo','锣'],
				['luo','骡'],
				['luo','摞'],
				['luo','椤'],
				['luo','箩'],
				['luo','珞'],
				['luo','雒'],
				['luo','倮'],
				['luo','猡'],
				['luo','荦'],
				['luo','絡'],
				['luo','瘰'],
				['luo','泺'],
				['luo','蠃'],
				['luo','茖'],
				['luo','镙'],
				['luo','騾'],
				['luo','蓏'],
				['luo','砢'],
				['luo','驘'],
				['luo','脶'],
				['luo','羅'],
				['luo','蘿'],
				['luo','鸁'],
				['luo','躶'],
				['luo','鏍'],
				['luo','笿'],
				['luo','邏'],
				['lv','葎'],
				['lv','絽'],
				['lv','綠'],
				['lv','閭'],
				['lv','藘'],
				['lv','稆'],
				['lv','郘'],
				['lv','縷'],
				['lv','鑢'],
				['lv','驢'],
				['lv','瞜'],
				['lv','繂'],
				['lv','褸'],
				['lv','馿'],
				['ma','么'],
				['ma','马'],
				['ma','吗'],
				['ma','妈'],
				['ma','麻'],
				['ma','玛'],
				['ma','码'],
				['ma','骂'],
				['ma','嘛'],
				['ma','蚂'],
				['ma','蟆'],
				['ma','嬷'],
				['ma','麼'],
				['ma','犸'],
				['ma','杩'],
				['ma','唛'],
				['ma','馬'],
				['ma','碼'],
				['ma','螞'],
				['ma','祃'],
				['ma','罵'],
				['ma','駡'],
				['ma','禡'],
				['ma','蔴'],
				['mai','买'],
				['mai','卖'],
				['mai','麦'],
				['mai','脉'],
				['mai','埋'],
				['mai','迈'],
				['mai','霾'],
				['mai','劢'],
				['mai','脈'],
				['mai','麥'],
				['mai','買'],
				['mai','賣'],
				['mai','貍'],
				['mai','荬'],
				['mai','邁'],
				['mai','蕒'],
				['man','满'],
				['man','慢'],
				['man','曼'],
				['man','漫'],
				['man','蛮'],
				['man','瞒'],
				['man','蔓'],
				['man','幔'],
				['man','馒'],
				['man','谩'],
				['man','鳗'],
				['man','螨'],
				['man','缦'],
				['man','熳'],
				['man','鞔'],
				['man','颟'],
				['man','鬘'],
				['man','墁'],
				['man','镘'],
				['man','蠻'],
				['man','饅'],
				['man','鰻'],
				['man','蔄'],
				['man','瞞'],
				['mang','忙'],
				['mang','茫'],
				['mang','盲'],
				['mang','芒'],
				['mang','莽'],
				['mang','氓'],
				['mang','蟒'],
				['mang','邙'],
				['mang','硭'],
				['mang','铓'],
				['mang','莾'],
				['mang','龒'],
				['mang','駹'],
				['mang','蠎'],
				['mang','漭'],
				['mang','笀'],
				['mang','茻'],
				['mao','毛'],
				['mao','贸'],
				['mao','冒'],
				['mao','矛'],
				['mao','貌'],
				['mao','猫'],
				['mao','帽'],
				['mao','茂'],
				['mao','茅'],
				['mao','髦'],
				['mao','锚'],
				['mao','卯'],
				['mao','袤'],
				['mao','懋'],
				['mao','耄'],
				['mao','瑁'],
				['mao','峁'],
				['mao','牦'],
				['mao','铆'],
				['mao','蟊'],
				['mao','昴'],
				['mao','旄'],
				['mao','瞀'],
				['mao','蝥'],
				['mao','茆'],
				['mao','貓'],
				['mao','鄚'],
				['mao','泖'],
				['mao','髳'],
				['mao','鶜'],
				['mao','貿'],
				['mao','錨'],
				['mao','鄮'],
				['mao','芼'],
				['mei','没'],
				['mei','美'],
				['mei','每'],
				['mei','梅'],
				['mei','妹'],
				['mei','眉'],
				['mei','媒'],
				['mei','煤'],
				['mei','枚'],
				['mei','媚'],
				['mei','玫'],
				['mei','昧'],
				['mei','霉'],
				['mei','魅'],
				['mei','酶'],
				['mei','湄'],
				['mei','嵋'],
				['mei','寐'],
				['mei','镁'],
				['mei','楣'],
				['mei','莓'],
				['mei','袂'],
				['mei','浼'],
				['mei','镅'],
				['mei','鹛'],
				['mei','郿'],
				['mei','眊'],
				['mei','黴'],
				['mei','跊'],
				['mei','猸'],
				['mei','篃'],
				['mei','矀'],
				['mei','蘪'],
				['mei','鋂'],
				['mei','鬽'],
				['mei','祙'],
				['mei','禖'],
				['mei','鎇'],
				['mei','羙'],
				['men','们'],
				['men','门'],
				['men','闷'],
				['men','扪'],
				['men','懑'],
				['men','焖'],
				['men','钔'],
				['men','門'],
				['men','穈'],
				['men','虋'],
				['meng','梦'],
				['meng','蒙'],
				['meng','猛'],
				['meng','盟'],
				['meng','孟'],
				['meng','朦'],
				['meng','萌'],
				['meng','懵'],
				['meng','檬'],
				['meng','锰'],
				['meng','虻'],
				['meng','蠓'],
				['meng','蜢'],
				['meng','勐'],
				['meng','矇'],
				['meng','艨'],
				['meng','艋'],
				['meng','瞢'],
				['meng','礞'],
				['meng','甍'],
				['meng','鄳'],
				['meng','矒'],
				['mi','米'],
				['mi','密'],
				['mi','秘'],
				['mi','迷'],
				['mi','弥'],
				['mi','蜜'],
				['mi','谜'],
				['mi','觅'],
				['mi','眯'],
				['mi','靡'],
				['mi','咪'],
				['mi','糜'],
				['mi','谧'],
				['mi','宓'],
				['mi','猕'],
				['mi','麋'],
				['mi','弭'],
				['mi','汨'],
				['mi','嘧'],
				['mi','醚'],
				['mi','幂'],
				['mi','縻'],
				['mi','蘼'],
				['mi','祢'],
				['mi','敉'],
				['mi','芈'],
				['mi','瞇'],
				['mi','脒'],
				['mi','粎'],
				['mi','醿'],
				['mi','蒾'],
				['mi','葞'],
				['mi','麊'],
				['mi','醾'],
				['mi','蓂'],
				['mi','覓'],
				['mi','詸'],
				['mi','鼏'],
				['mi','罙'],
				['mi','鸍'],
				['mi','覛'],
				['mi','禰'],
				['mi','籋'],
				['mian','面'],
				['mian','免'],
				['mian','棉'],
				['mian','眠'],
				['mian','勉'],
				['mian','绵'],
				['mian','缅'],
				['mian','冕'],
				['mian','娩'],
				['mian','腼'],
				['mian','湎'],
				['mian','沔'],
				['mian','靦'],
				['mian','眄'],
				['mian','靣'],
				['mian','綿'],
				['mian','矊'],
				['mian','緜'],
				['mian','麵'],
				['miao','描'],
				['miao','妙'],
				['miao','庙'],
				['miao','苗'],
				['miao','秒'],
				['miao','瞄'],
				['miao','渺'],
				['miao','缪'],
				['miao','藐'],
				['miao','缈'],
				['miao','喵'],
				['miao','淼'],
				['miao','邈'],
				['miao','眇'],
				['miao','杪'],
				['miao','鹋'],
				['miao','緲'],
				['miao','繆'],
				['miao','鶓'],
				['mie','灭'],
				['mie','蔑'],
				['mie','咩'],
				['mie','篾'],
				['mie','乜'],
				['mie','蠛'],
				['min','民'],
				['min','敏'],
				['min','悯'],
				['min','闽'],
				['min','闵'],
				['min','抿'],
				['min','皿'],
				['min','泯'],
				['min','岷'],
				['min','黾'],
				['min','愍'],
				['min','缗'],
				['min','珉'],
				['min','玟'],
				['min','緡'],
				['min','鳘'],
				['min','苠'],
				['min','簢'],
				['min','鈱'],
				['min','盿'],
				['min','賯'],
				['min','緍'],
				['min','閔'],
				['min','鰵'],
				['ming','明'],
				['ming','名'],
				['ming','命'],
				['ming','鸣'],
				['ming','铭'],
				['ming','冥'],
				['ming','茗'],
				['ming','瞑'],
				['ming','溟'],
				['ming','酩'],
				['ming','螟'],
				['ming','暝'],
				['ming','鳴'],
				['ming','眳'],
				['ming','銘'],
				['ming','眀'],
				['ming','鄍'],
				['ming','詺'],
				['miu','谬'],
				['miu','謬'],
				['mo','模'],
				['mo','莫'],
				['mo','默'],
				['mo','摩'],
				['mo','末'],
				['mo','魔'],
				['mo','摸'],
				['mo','墨'],
				['mo','磨'],
				['mo','漠'],
				['mo','膜'],
				['mo','抹'],
				['mo','陌'],
				['mo','沫'],
				['mo','寞'],
				['mo','麽'],
				['mo','摹'],
				['mo','蓦'],
				['mo','谟'],
				['mo','蘑'],
				['mo','茉'],
				['mo','馍'],
				['mo','殁'],
				['mo','秣'],
				['mo','镆'],
				['mo','貘'],
				['mo','嫫'],
				['mo','貊'],
				['mo','麿'],
				['mo','耱'],
				['mo','驀'],
				['mo','瘼'],
				['mo','糢'],
				['mo','瞐'],
				['mo','眜'],
				['mo','藦'],
				['mo','謨'],
				['mo','艒'],
				['mo','粖'],
				['mo','饝'],
				['mo','謩'],
				['mou','某'],
				['mou','谋'],
				['mou','牟'],
				['mou','眸'],
				['mou','鍪'],
				['mou','蛑'],
				['mou','哞'],
				['mou','侔'],
				['mou','鴾'],
				['mou','麰'],
				['mou','鞪'],
				['mou','謀'],
				['mu','目'],
				['mu','母'],
				['mu','木'],
				['mu','姆'],
				['mu','幕'],
				['mu','牧'],
				['mu','穆'],
				['mu','墓'],
				['mu','慕'],
				['mu','亩'],
				['mu','募'],
				['mu','暮'],
				['mu','牡'],
				['mu','睦'],
				['mu','拇'],
				['mu','沐'],
				['mu','苜'],
				['mu','钼'],
				['mu','仫'],
				['mu','雮'],
				['mu','坶'],
				['mu','砪'],
				['mu','霂'],
				['na','那'],
				['na','拿'],
				['na','哪'],
				['na','纳'],
				['na','呐'],
				['na','钠'],
				['na','捺'],
				['na','衲'],
				['na','肭'],
				['na','納'],
				['na','镎'],
				['na','靹'],
				['na','袦'],
				['na','鈉'],
				['na','蒘'],
				['na','篛'],
				['na','誽'],
				['na','秅'],
				['nai','乃'],
				['nai','奶'],
				['nai','耐'],
				['nai','奈'],
				['nai','氖'],
				['nai','艿'],
				['nai','鼐'],
				['nai','萘'],
				['nai','柰'],
				['nai','迺'],
				['nai','螚'],
				['nai','釢'],
				['nai','褦'],
				['nan','难'],
				['nan','南'],
				['nan','男'],
				['nan','喃'],
				['nan','楠'],
				['nan','囡'],
				['nan','赧'],
				['nan','難'],
				['nan','蝻'],
				['nan','腩'],
				['nang','囊'],
				['nang','囔'],
				['nang','馕'],
				['nang','曩'],
				['nang','攮'],
				['nao','脑'],
				['nao','闹'],
				['nao','恼'],
				['nao','挠'],
				['nao','瑙'],
				['nao','淖'],
				['nao','呶'],
				['nao','孬'],
				['nao','腦'],
				['nao','蛲'],
				['nao','垴'],
				['nao','硇'],
				['nao','铙'],
				['nao','猱'],
				['nao','臑'],
				['nao','蟯'],
				['nao','脳'],
				['nao','鬧'],
				['ne','呢'],
				['ne','讷'],
				['ne','眲'],
				['nei','内'],
				['nei','馁'],
				['nei','鮾'],
				['nei','餒'],
				['nei','鯘'],
				['nen','嫩'],
				['neng','能'],
				['neus','莻'],
				['ngag','鈪'],
				['ni','你'],
				['ni','尼'],
				['ni','泥'],
				['ni','拟'],
				['ni','逆'],
				['ni','妮'],
				['ni','匿'],
				['ni','腻'],
				['ni','倪'],
				['ni','溺'],
				['ni','霓'],
				['ni','昵'],
				['ni','睨'],
				['ni','伲'],
				['ni','怩'],
				['ni','旎'],
				['ni','猊'],
				['ni','鲵'],
				['ni','铌'],
				['ni','膩'],
				['ni','坭'],
				['ni','麑'],
				['ni','薿'],
				['ni','蜺'],
				['ni','貎'],
				['ni','齯'],
				['ni','聻'],
				['nian','年'],
				['nian','念'],
				['nian','粘'],
				['nian','廿'],
				['nian','捻'],
				['nian','拈'],
				['nian','黏'],
				['nian','碾'],
				['nian','撵'],
				['nian','鲇'],
				['nian','蔫'],
				['nian','辇'],
				['nian','鲶'],
				['nian','埝'],
				['nian','蹨'],
				['nian','輾'],
				['nian','秊'],
				['nian','跈'],
				['nian','輦'],
				['nian','鮎'],
				['niang','娘'],
				['niang','酿'],
				['niang','釀'],
				['niao','鸟'],
				['niao','袅'],
				['niao','茑'],
				['niao','脲'],
				['niao','嬲'],
				['niao','蔦'],
				['nie','涅'],
				['nie','捏'],
				['nie','聂'],
				['nie','孽'],
				['nie','蹑'],
				['nie','嗫'],
				['nie','啮'],
				['nie','镍'],
				['nie','颞'],
				['nie','臬'],
				['nie','镊'],
				['nie','蘖'],
				['nie','陧'],
				['nie','菍'],
				['nie','齧'],
				['nie','糵'],
				['nie','顳'],
				['nie','苶'],
				['nie','鑷'],
				['nie','糱'],
				['nie','蠥'],
				['nie','鋷'],
				['nie','聶'],
				['nin','您'],
				['ning','宁'],
				['ning','凝'],
				['ning','拧'],
				['ning','狞'],
				['ning','泞'],
				['ning','柠'],
				['ning','咛'],
				['ning','佞'],
				['ning','甯'],
				['ning','聍'],
				['ning','矃'],
				['ning','聹'],
				['niu','牛'],
				['niu','纽'],
				['niu','扭'],
				['niu','钮'],
				['niu','妞'],
				['niu','忸'],
				['niu','狃'],
				['niu','紐'],
				['niu','鈕'],
				['niu','靵'],
				['nong','农'],
				['nong','浓'],
				['nong','侬'],
				['nong','脓'],
				['nong','哝'],
				['nong','膿'],
				['nong','秾'],
				['nong','農'],
				['nong','醲'],
				['nong','襛'],
				['nou','耨'],
				['nou','羺'],
				['nu','努'],
				['nu','怒'],
				['nu','奴'],
				['nu','弩'],
				['nu','驽'],
				['nu','孥'],
				['nu','胬'],
				['nu','砮'],
				['nu:','女'],
				['nu:','钕'],
				['nu:','衄'],
				['nu:','恧'],
				['nuan','暖'],
				['nue','虐'],
				['nue','疟'],
				['nuo','诺'],
				['nuo','娜'],
				['nuo','挪'],
				['nuo','懦'],
				['nuo','傩'],
				['nuo','糯'],
				['nuo','锘'],
				['nuo','搦'],
				['nuo','耎'],
				['nuo','稬'],
				['nv','衂'],
				['o','噢'],
				['o','喔'],
				['o','筽'],
				['ou','区'],
				['ou','欧'],
				['ou','偶'],
				['ou','呕'],
				['ou','殴'],
				['ou','鸥'],
				['ou','藕'],
				['ou','讴'],
				['ou','耦'],
				['ou','沤'],
				['ou','瓯'],
				['ou','怄'],
				['ou','蕅'],
				['ou','鏂'],
				['pa','怕'],
				['pa','爬'],
				['pa','帕'],
				['pa','啪'],
				['pa','趴'],
				['pa','琶'],
				['pa','葩'],
				['pa','杷'],
				['pa','筢'],
				['pai','派'],
				['pai','排'],
				['pai','拍'],
				['pai','牌'],
				['pai','徘'],
				['pai','湃'],
				['pai','俳'],
				['pai','哌'],
				['pai','簰'],
				['pan','判'],
				['pan','盘'],
				['pan','叛'],
				['pan','潘'],
				['pan','盼'],
				['pan','攀'],
				['pan','畔'],
				['pan','拚'],
				['pan','磐'],
				['pan','蹒'],
				['pan','蟠'],
				['pan','袢'],
				['pan','泮'],
				['pan','襻'],
				['pan','鋬'],
				['pan','盤'],
				['pan','闆'],
				['pan','眫'],
				['pan','膰'],
				['pan','蹣'],
				['pan','盻'],
				['pan','踫'],
				['pan','眅'],
				['pang','旁'],
				['pang','胖'],
				['pang','彷'],
				['pang','乓'],
				['pang','螃'],
				['pang','滂'],
				['pang','雱'],
				['pang','耪'],
				['pang','逄'],
				['pang','鳑'],
				['pang','趽'],
				['pang','鰟'],
				['pang','蠭'],
				['pang','龎'],
				['pao','跑'],
				['pao','炮'],
				['pao','抛'],
				['pao','泡'],
				['pao','袍'],
				['pao','咆'],
				['pao','疱'],
				['pao','庖'],
				['pao','龅'],
				['pao','匏'],
				['pao','狍'],
				['pao','脬'],
				['pao','砲'],
				['pao','窌'],
				['pao','麅'],
				['pao','礮'],
				['pao','鞄'],
				['pao','褜'],
				['pao','萢'],
				['pei','配'],
				['pei','培'],
				['pei','佩'],
				['pei','陪'],
				['pei','赔'],
				['pei','胚'],
				['pei','沛'],
				['pei','裴'],
				['pei','呸'],
				['pei','辔'],
				['pei','旆'],
				['pei','帔'],
				['pei','锫'],
				['pei','霈'],
				['pei','醅'],
				['pei','茷'],
				['pei','肧'],
				['pen','喷'],
				['pen','盆'],
				['pen','湓'],
				['pen','翸'],
				['peng','朋'],
				['peng','碰'],
				['peng','彭'],
				['peng','鹏'],
				['peng','捧'],
				['peng','蓬'],
				['peng','膨'],
				['peng','棚'],
				['peng','篷'],
				['peng','砰'],
				['peng','抨'],
				['peng','烹'],
				['peng','澎'],
				['peng','怦'],
				['peng','嘭'],
				['peng','硼'],
				['peng','蟛'],
				['peng','堋'],
				['peng','軿'],
				['peng','閛'],
				['peng','纄'],
				['peng','芃'],
				['peng','髼'],
				['peng','韼'],
				['peng','磞'],
				['peng','鵬'],
				['peng','輧'],
				['peng','鬅'],
				['pi','批'],
				['pi','皮'],
				['pi','疲'],
				['pi','匹'],
				['pi','披'],
				['pi','脾'],
				['pi','屁'],
				['pi','譬'],
				['pi','劈'],
				['pi','僻'],
				['pi','啤'],
				['pi','霹'],
				['pi','毗'],
				['pi','癖'],
				['pi','琵'],
				['pi','丕'],
				['pi','坯'],
				['pi','媲'],
				['pi','痞'],
				['pi','噼'],
				['pi','陂'],
				['pi','砒'],
				['pi','纰'],
				['pi','枇'],
				['pi','圮'],
				['pi','罴'],
				['pi','鼙'],
				['pi','铍'],
				['pi','庀'],
				['pi','蚍'],
				['pi','邳'],
				['pi','甓'],
				['pi','淠'],
				['pi','貔'],
				['pi','蜱'],
				['pi','芘'],
				['pi','鲏'],
				['pi','郫'],
				['pi','埤'],
				['pi','陴'],
				['pi','仳'],
				['pi','魾'],
				['pi','鮍'],
				['pi','闢'],
				['pi','腗'],
				['pi','擗'],
				['pi','錃'],
				['pi','鷿'],
				['pi','秠'],
				['pi','篺'],
				['pi','隦'],
				['pian','片'],
				['pian','篇'],
				['pian','偏'],
				['pian','骗'],
				['pian','翩'],
				['pian','谝'],
				['pian','骈'],
				['pian','胼'],
				['pian','蹁'],
				['pian','騙'],
				['pian','犏'],
				['pian','魸'],
				['pian','骿'],
				['piao','票'],
				['piao','飘'],
				['piao','漂'],
				['piao','嫖'],
				['piao','瞟'],
				['piao','瓢'],
				['piao','剽'],
				['piao','缥'],
				['piao','嘌'],
				['piao','骠'],
				['piao','殍'],
				['piao','螵'],
				['piao','磦'],
				['piao','醥'],
				['piao','縹'],
				['piao','魒'],
				['pie','瞥'],
				['pie','撇'],
				['pie','苤'],
				['pie','氕'],
				['pin','品'],
				['pin','贫'],
				['pin','频'],
				['pin','拼'],
				['pin','聘'],
				['pin','苹'],
				['pin','嫔'],
				['pin','颦'],
				['pin','姘'],
				['pin','牝'],
				['pin','頻'],
				['pin','蘋'],
				['pin','貧'],
				['pin','榀'],
				['ping','平'],
				['ping','评'],
				['ping','凭'],
				['ping','瓶'],
				['ping','萍'],
				['ping','坪'],
				['ping','乒'],
				['ping','鲆'],
				['ping','枰'],
				['ping','娉'],
				['ping','俜'],
				['ping','評'],
				['ping','聠'],
				['ping','蛢'],
				['ping','蓱'],
				['ping','缾'],
				['po','破'],
				['po','迫'],
				['po','婆'],
				['po','坡'],
				['po','颇'],
				['po','朴'],
				['po','泼'],
				['po','魄'],
				['po','珀'],
				['po','叵'],
				['po','鄱'],
				['po','粕'],
				['po','笸'],
				['po','钋'],
				['po','皤'],
				['po','攴'],
				['po','頗'],
				['po','钷'],
				['po','釙'],
				['po','酦'],
				['po','醱'],
				['po','蒪'],
				['pou','剖'],
				['pou','掊'],
				['pou','裒'],
				['pu','普'],
				['pu','扑'],
				['pu','铺'],
				['pu','仆'],
				['pu','葡'],
				['pu','浦'],
				['pu','菩'],
				['pu','谱'],
				['pu','蒲'],
				['pu','埔'],
				['pu','曝'],
				['pu','噗'],
				['pu','圃'],
				['pu','溥'],
				['pu','璞'],
				['pu','匍'],
				['pu','濮'],
				['pu','莆'],
				['pu','蹼'],
				['pu','氆'],
				['pu','镨'],
				['pu','酺'],
				['pu','舖'],
				['pu','镤'],
				['pu','譜'],
				['pu','襆'],
				['pu','穙'],
				['pu','纀'],
				['pu','菐'],
				['pu','贌'],
				['pu','鋪'],
				['qi','起'],
				['qi','其'],
				['qi','只'],
				['qi','气'],
				['qi','期'],
				['qi','器'],
				['qi','企'],
				['qi','七'],
				['qi','齐'],
				['qi','妻'],
				['qi','弃'],
				['qi','汽'],
				['qi','启'],
				['qi','骑'],
				['qi','旗'],
				['qi','欺'],
				['qi','岂'],
				['qi','契'],
				['qi','歧'],
				['qi','棋'],
				['qi','溪'],
				['qi','漆'],
				['qi','戚'],
				['qi','凄'],
				['qi','乞'],
				['qi','祈'],
				['qi','泣'],
				['qi','栖'],
				['qi','迄'],
				['qi','崎'],
				['qi','鳍'],
				['qi','琪'],
				['qi','缉'],
				['qi','砌'],
				['qi','琦'],
				['qi','祁'],
				['qi','祺'],
				['qi','绮'],
				['qi','耆'],
				['qi','脐'],
				['qi','杞'],
				['qi','岐'],
				['qi','淇'],
				['qi','憩'],
				['qi','沏'],
				['qi','芪'],
				['qi','嘁'],
				['qi','讫'],
				['qi','麒'],
				['qi','碛'],
				['qi','畦'],
				['qi','槭'],
				['qi','葺'],
				['qi','蛴'],
				['qi','圻'],
				['qi','颀'],
				['qi','祇'],
				['qi','蕲'],
				['qi','綦'],
				['qi','亓'],
				['qi','萋'],
				['qi','芑'],
				['qi','桤'],
				['qi','碶'],
				['qi','骐'],
				['qi','萁'],
				['qi','綮'],
				['qi','臍'],
				['qi','碁'],
				['qi','柒'],
				['qi','汔'],
				['qi','蜞'],
				['qi','碕'],
				['qi','饑'],
				['qi','鯕'],
				['qi','鲯'],
				['qi','跂'],
				['qi','磜'],
				['qi','郪'],
				['qi','豈'],
				['qi','齊'],
				['qi','諆'],
				['qi','軙'],
				['qi','騎'],
				['qi','麡'],
				['qi','禥'],
				['qi','屺'],
				['qi','鬐'],
				['qi','鬿'],
				['qi','磩'],
				['qi','簯'],
				['qi','鼜'],
				['qi','蚑'],
				['qi','竒'],
				['qi','纃'],
				['qi','艩'],
				['qi','粸'],
				['qi','邔'],
				['qi','鶈'],
				['qi','螧'],
				['qia','恰'],
				['qia','洽'],
				['qia','掐'],
				['qia','髂'],
				['qia','葜'],
				['qia','硈'],
				['qia','跒'],
				['qian','前'],
				['qian','千'],
				['qian','钱'],
				['qian','潜'],
				['qian','签'],
				['qian','迁'],
				['qian','牵'],
				['qian','遣'],
				['qian','欠'],
				['qian','纤'],
				['qian','歉'],
				['qian','谦'],
				['qian','谴'],
				['qian','倩'],
				['qian','铅'],
				['qian','嵌'],
				['qian','虔'],
				['qian','茜'],
				['qian','钳'],
				['qian','堑'],
				['qian','黔'],
				['qian','荨'],
				['qian','骞'],
				['qian','掮'],
				['qian','钤'],
				['qian','阡'],
				['qian','芡'],
				['qian','愆'],
				['qian','箝'],
				['qian','仟'],
				['qian','悭'],
				['qian','錢'],
				['qian','佥'],
				['qian','慊'],
				['qian','芊'],
				['qian','肷'],
				['qian','扦'],
				['qian','缱'],
				['qian','钎'],
				['qian','褰'],
				['qian','搴'],
				['qian','籤'],
				['qian','椠'],
				['qian','蒨'],
				['qian','葴'],
				['qian','遷'],
				['qian','鍼'],
				['qian','軡'],
				['qian','竏'],
				['qian','鹐'],
				['qian','蚈'],
				['qian','藖'],
				['qian','騫'],
				['qian','韆'],
				['qian','篟'],
				['qian','箞'],
				['qian','簽'],
				['qian','鉛'],
				['qian','鰬'],
				['qian','鈆'],
				['qian','蜸'],
				['qian','岍'],
				['qiang','强'],
				['qiang','枪'],
				['qiang','墙'],
				['qiang','抢'],
				['qiang','腔'],
				['qiang','跄'],
				['qiang','呛'],
				['qiang','羌'],
				['qiang','蔷'],
				['qiang','锵'],
				['qiang','戕'],
				['qiang','鎗'],
				['qiang','襁'],
				['qiang','羟'],
				['qiang','樯'],
				['qiang','炝'],
				['qiang','嫱'],
				['qiang','蜣'],
				['qiang','戗'],
				['qiang','锖'],
				['qiang','镪'],
				['qiang','錆'],
				['qiang','鏘'],
				['qiang','蹌'],
				['qiang','薔'],
				['qiang','篬'],
				['qiang','蹡'],
				['qiang','鸧'],
				['qiang','艢'],
				['qiao','巧'],
				['qiao','桥'],
				['qiao','悄'],
				['qiao','乔'],
				['qiao','瞧'],
				['qiao','敲'],
				['qiao','雀'],
				['qiao','侨'],
				['qiao','俏'],
				['qiao','峭'],
				['qiao','翘'],
				['qiao','窍'],
				['qiao','鞘'],
				['qiao','樵'],
				['qiao','憔'],
				['qiao','撬'],
				['qiao','跷'],
				['qiao','橇'],
				['qiao','锹'],
				['qiao','诮'],
				['qiao','荞'],
				['qiao','谯'],
				['qiao','菬'],
				['qiao','愀'],
				['qiao','鞒'],
				['qiao','硗'],
				['qiao','竅'],
				['qiao','劁'],
				['qiao','鍬'],
				['qiao','翹'],
				['qiao','硚'],
				['qiao','碻'],
				['qiao','睄'],
				['qiao','缲'],
				['qiao','礄'],
				['qiao','陗'],
				['qiao','鐰'],
				['qiao','鄡'],
				['qiao','趬'],
				['qiao','磽'],
				['qiao','踃'],
				['qiao','藮'],
				['qie','且'],
				['qie','切'],
				['qie','窃'],
				['qie','怯'],
				['qie','伽'],
				['qie','茄'],
				['qie','妾'],
				['qie','惬'],
				['qie','锲'],
				['qie','挈'],
				['qie','箧'],
				['qie','郤'],
				['qie','郄'],
				['qie','詧'],
				['qie','聺'],
				['qie','踥'],
				['qie','稧'],
				['qie','鐑'],
				['qie','鍥'],
				['qin','亲'],
				['qin','侵'],
				['qin','秦'],
				['qin','勤'],
				['qin','琴'],
				['qin','钦'],
				['qin','禽'],
				['qin','寝'],
				['qin','擒'],
				['qin','沁'],
				['qin','芹'],
				['qin','噙'],
				['qin','芩'],
				['qin','嗪'],
				['qin','揿'],
				['qin','衾'],
				['qin','吣'],
				['qin','锓'],
				['qin','檎'],
				['qin','螓'],
				['qin','骎'],
				['qin','駸'],
				['qin','親'],
				['qin','鮼'],
				['qin','矝'],
				['qin','蠄'],
				['qin','鈙'],
				['qin','綅'],
				['qin','笉'],
				['qin','菳'],
				['qin','菣'],
				['qin','菦'],
				['qin','鈂'],
				['qing','情'],
				['qing','清'],
				['qing','请'],
				['qing','轻'],
				['qing','青'],
				['qing','倾'],
				['qing','庆'],
				['qing','晴'],
				['qing','卿'],
				['qing','氢'],
				['qing','顷'],
				['qing','擎'],
				['qing','蜻'],
				['qing','氰'],
				['qing','罄'],
				['qing','磬'],
				['qing','輕'],
				['qing','請'],
				['qing','檠'],
				['qing','黥'],
				['qing','鲭'],
				['qing','箐'],
				['qing','頃'],
				['qing','謦'],
				['qing','鶄'],
				['qing','苘'],
				['qing','碃'],
				['qing','圊'],
				['qing','鯖'],
				['qiong','穷'],
				['qiong','琼'],
				['qiong','穹'],
				['qiong','邛'],
				['qiong','蛩'],
				['qiong','跫'],
				['qiong','銎'],
				['qiong','筇'],
				['qiong','茕'],
				['qiong','藭'],
				['qiong','窮'],
				['qiong','蛬'],
				['qiong','藑'],
				['qiong','睘'],
				['qiong','赹'],
				['qiu','求'],
				['qiu','球'],
				['qiu','秋'],
				['qiu','丘'],
				['qiu','邱'],
				['qiu','囚'],
				['qiu','裘'],
				['qiu','酋'],
				['qiu','蚯'],
				['qiu','虬'],
				['qiu','鳅'],
				['qiu','泅'],
				['qiu','遒'],
				['qiu','俅'],
				['qiu','楸'],
				['qiu','逑'],
				['qiu','鰽'],
				['qiu','巯'],
				['qiu','糗'],
				['qiu','犰'],
				['qiu','蝤'],
				['qiu','赇'],
				['qiu','趥'],
				['qiu','虯'],
				['qiu','絿'],
				['qiu','蛷'],
				['qiu','銶'],
				['qiu','鞧'],
				['qiu','鞦'],
				['qiu','訅'],
				['qiu','秌'],
				['qiu','肍'],
				['qiu','鰌'],
				['qiu','鹙'],
				['qiu','穐'],
				['qiu','觓'],
				['qiu','龝'],
				['qu','去'],
				['qu','取'],
				['qu','趣'],
				['qu','曲'],
				['qu','趋'],
				['qu','驱'],
				['qu','屈'],
				['qu','躯'],
				['qu','渠'],
				['qu','娶'],
				['qu','龋'],
				['qu','觑'],
				['qu','岖'],
				['qu','蛐'],
				['qu','祛'],
				['qu','蛆'],
				['qu','衢'],
				['qu','蠼'],
				['qu','劬'],
				['qu','阒'],
				['qu','诎'],
				['qu','朐'],
				['qu','癯'],
				['qu','黢'],
				['qu','蕖'],
				['qu','鸲'],
				['qu','璩'],
				['qu','蘧'],
				['qu','麴'],
				['qu','磲'],
				['qu','鼩'],
				['qu','菃'],
				['qu','袪'],
				['qu','趨'],
				['qu','胠'],
				['qu','驅'],
				['qu','臒'],
				['qu','跼'],
				['qu','麹'],
				['qu','氍'],
				['qu','麯'],
				['qu','胊'],
				['qu','闃'],
				['qu','蠷'],
				['qu','臞'],
				['qu','軀'],
				['qu','閴'],
				['qu','鰸'],
				['qu','齲'],
				['qu','葋'],
				['quan','全'],
				['quan','权'],
				['quan','券'],
				['quan','劝'],
				['quan','泉'],
				['quan','拳'],
				['quan','犬'],
				['quan','痊'],
				['quan','蜷'],
				['quan','诠'],
				['quan','荃'],
				['quan','绻'],
				['quan','颧'],
				['quan','铨'],
				['quan','醛'],
				['quan','鬈'],
				['quan','筌'],
				['quan','悛'],
				['quan','畎'],
				['quan','鳈'],
				['quan','鰁'],
				['quan','詮'],
				['quan','譔'],
				['quan','絟'],
				['quan','辁'],
				['quan','腃'],
				['quan','硂'],
				['quan','葲'],
				['quan','銓'],
				['quan','顴'],
				['que','却'],
				['que','确'],
				['que','缺'],
				['que','阙'],
				['que','鹊'],
				['que','瘸'],
				['que','榷'],
				['que','阕'],
				['que','炔'],
				['que','確'],
				['que','悫'],
				['que','鵲'],
				['que','闕'],
				['qun','群'],
				['qun','裙'],
				['qun','逡'],
				['qun','踆'],
				['ran','然'],
				['ran','染'],
				['ran','燃'],
				['ran','冉'],
				['ran','髯'],
				['ran','苒'],
				['ran','蚺'],
				['ran','髥'],
				['ran','袇'],
				['ran','肰'],
				['rang','让'],
				['rang','嚷'],
				['rang','壤'],
				['rang','攘'],
				['rang','瓤'],
				['rang','穰'],
				['rang','禳'],
				['rang','讓'],
				['rao','扰'],
				['rao','绕'],
				['rao','饶'],
				['rao','荛'],
				['rao','桡'],
				['rao','娆'],
				['rao','繞'],
				['rao','饒'],
				['rao','遶'],
				['re','热'],
				['re','惹'],
				['re','喏'],
				['ren','人'],
				['ren','任'],
				['ren','认'],
				['ren','忍'],
				['ren','仁'],
				['ren','刃'],
				['ren','韧'],
				['ren','妊'],
				['ren','壬'],
				['ren','纫'],
				['ren','饪'],
				['ren','稔'],
				['ren','恁'],
				['ren','仞'],
				['ren','荏'],
				['ren','衽'],
				['ren','認'],
				['ren','轫'],
				['ren','葚'],
				['ren','訒'],
				['ren','韌'],
				['ren','袵'],
				['ren','鵀'],
				['ren','讱'],
				['ren','靭'],
				['ren','餁'],
				['ren','纴'],
				['ren','魜'],
				['ren','秹'],
				['ren','絍'],
				['ren','靱'],
				['ren','飪'],
				['ren','紝'],
				['ren','軔'],
				['reng','仍'],
				['reng','扔'],
				['reng','礽'],
				['ri','日'],
				['ri','鈤'],
				['rong','容'],
				['rong','荣'],
				['rong','融'],
				['rong','溶'],
				['rong','绒'],
				['rong','蓉'],
				['rong','熔'],
				['rong','戎'],
				['rong','镕'],
				['rong','茸'],
				['rong','冗'],
				['rong','榕'],
				['rong','嵘'],
				['rong','肜'],
				['rong','蝾'],
				['rong','絨'],
				['rong','穃'],
				['rong','鎔'],
				['rong','褣'],
				['rong','駥'],
				['rong','狨'],
				['rou','肉'],
				['rou','柔'],
				['rou','揉'],
				['rou','蹂'],
				['rou','鞣'],
				['rou','糅'],
				['rou','腬'],
				['rou','葇'],
				['ru','如'],
				['ru','入'],
				['ru','乳'],
				['ru','辱'],
				['ru','儒'],
				['ru','汝'],
				['ru','褥'],
				['ru','蠕'],
				['ru','茹'],
				['ru','孺'],
				['ru','嚅'],
				['ru','濡'],
				['ru','缛'],
				['ru','洳'],
				['ru','薷'],
				['ru','溽'],
				['ru','蓐'],
				['ru','铷'],
				['ru','襦'],
				['ru','颥'],
				['ru','鳰'],
				['ru','顬'],
				['ru','込'],
				['ru','袽'],
				['ru','鴽'],
				['ruan','软'],
				['ruan','阮'],
				['ruan','朊'],
				['ruan','軟'],
				['ruan','緛'],
				['rui','瑞'],
				['rui','蕊'],
				['rui','睿'],
				['rui','芮'],
				['rui','蚋'],
				['rui','蕤'],
				['rui','枘'],
				['rui','銳'],
				['rui','緌'],
				['rui','蘂'],
				['run','润'],
				['run','闰'],
				['run','閏'],
				['ruo','若'],
				['ruo','弱'],
				['ruo','偌'],
				['ruo','箬'],
				['ruo','鶸'],
				['ruo','蒻'],
				['sa','萨'],
				['sa','撒'],
				['sa','洒'],
				['sa','卅'],
				['sa','飒'],
				['sa','仨'],
				['sa','靸'],
				['sa','脎'],
				['sai','赛'],
				['sai','塞'],
				['sai','腮'],
				['sai','鳃'],
				['sai','噻'],
				['sai','簺'],
				['san','三'],
				['san','散'],
				['san','伞'],
				['san','叁'],
				['san','馓'],
				['san','糁'],
				['san','毵'],
				['san','彡'],
				['san','鬖'],
				['san','糂'],
				['san','繖'],
				['sang','丧'],
				['sang','桑'],
				['sang','嗓'],
				['sang','搡'],
				['sang','颡'],
				['sang','磉'],
				['sao','扫'],
				['sao','艘'],
				['sao','嫂'],
				['sao','骚'],
				['sao','搔'],
				['sao','臊'],
				['sao','缫'],
				['sao','埽'],
				['sao','瘙'],
				['sao','鰠'],
				['sao','騷'],
				['sao','鳋'],
				['sao','矂'],
				['sao','髞'],
				['se','色'],
				['se','瑟'],
				['se','涩'],
				['se','啬'],
				['se','穑'],
				['se','铯'],
				['se','繬'],
				['se','穯'],
				['se','譅'],
				['se','鎍'],
				['se','銫'],
				['sen','森'],
				['seng','僧'],
				['seng','鬙'],
				['sha','杀'],
				['sha','沙'],
				['sha','傻'],
				['sha','啥'],
				['sha','莎'],
				['sha','厦'],
				['sha','煞'],
				['sha','纱'],
				['sha','砂'],
				['sha','杉'],
				['sha','霎'],
				['sha','鲨'],
				['sha','铩'],
				['sha','裟'],
				['sha','痧'],
				['sha','歃'],
				['sha','唼'],
				['sha','閷'],
				['sha','紗'],
				['sha','翜'],
				['sha','閯'],
				['sha','繺'],
				['sha','繌'],
				['sha','鯋'],
				['sha','翣'],
				['sha','蔱'],
				['sha','鯊'],
				['shai','晒'],
				['shai','筛'],
				['shai','篩'],
				['shai','酾'],
				['shan','山'],
				['shan','善'],
				['shan','闪'],
				['shan','扇'],
				['shan','衫'],
				['shan','陕'],
				['shan','擅'],
				['shan','珊'],
				['shan','删'],
				['shan','煽'],
				['shan','膳'],
				['shan','讪'],
				['shan','汕'],
				['shan','姗'],
				['shan','跚'],
				['shan','赡'],
				['shan','缮'],
				['shan','蟮'],
				['shan','鳝'],
				['shan','苫'],
				['shan','嬗'],
				['shan','潸'],
				['shan','鄯'],
				['shan','舢'],
				['shan','疝'],
				['shan','芟'],
				['shan','膻'],
				['shan','钐'],
				['shan','剡'],
				['shan','纔'],
				['shan','埏'],
				['shan','睒'],
				['shan','羶'],
				['shan','釤'],
				['shan','閃'],
				['shan','骟'],
				['shan','笧'],
				['shan','鱣'],
				['shan','訕'],
				['shan','繕'],
				['shan','陝'],
				['shan','鯅'],
				['shan','羴'],
				['shang','上'],
				['shang','商'],
				['shang','伤'],
				['shang','尚'],
				['shang','赏'],
				['shang','晌'],
				['shang','熵'],
				['shang','墒'],
				['shang','觞'],
				['shang','绱'],
				['shang','殇'],
				['shang','垧'],
				['shang','謪'],
				['shang','賞'],
				['shao','少'],
				['shao','烧'],
				['shao','召'],
				['shao','绍'],
				['shao','稍'],
				['shao','哨'],
				['shao','梢'],
				['shao','邵'],
				['shao','勺'],
				['shao','韶'],
				['shao','捎'],
				['shao','芍'],
				['shao','艄'],
				['shao','劭'],
				['shao','紹'],
				['shao','筲'],
				['shao','蛸'],
				['shao','潲'],
				['shao','袑'],
				['she','社'],
				['she','设'],
				['she','射'],
				['she','折'],
				['she','涉'],
				['she','舍'],
				['she','摄'],
				['she','蛇'],
				['she','舌'],
				['she','奢'],
				['she','慑'],
				['she','赦'],
				['she','麝'],
				['she','赊'],
				['she','佘'],
				['she','歙'],
				['she','厍'],
				['she','猞'],
				['she','設'],
				['she','蔎'],
				['she','滠'],
				['she','揲'],
				['she','輋'],
				['she','騇'],
				['she','蛥'],
				['shei','谁'],
				['shen','什'],
				['shen','身'],
				['shen','神'],
				['shen','深'],
				['shen','甚'],
				['shen','审'],
				['shen','申'],
				['shen','伸'],
				['shen','慎'],
				['shen','肾'],
				['shen','渗'],
				['shen','绅'],
				['shen','呻'],
				['shen','婶'],
				['shen','娠'],
				['shen','蜃'],
				['shen','莘'],
				['shen','砷'],
				['shen','诜'],
				['shen','腎'],
				['shen','哂'],
				['shen','胂'],
				['shen','渖'],
				['shen','谂'],
				['shen','椹'],
				['shen','矧'],
				['shen','谉'],
				['shen','鲹'],
				['shen','蓡'],
				['shen','瞫'],
				['shen','詵'],
				['shen','蔘'],
				['shen','鯵'],
				['shen','籶'],
				['shen','鰰'],
				['shen','魫'],
				['shen','紳'],
				['shen','祳'],
				['shen','葠'],
				['shen','邥'],
				['shen','鯓'],
				['shen','眘'],
				['sheng','生'],
				['sheng','声'],
				['sheng','省'],
				['sheng','胜'],
				['sheng','升'],
				['sheng','圣'],
				['sheng','剩'],
				['sheng','牲'],
				['sheng','绳'],
				['sheng','甥'],
				['sheng','笙'],
				['sheng','嵊'],
				['sheng','晟'],
				['sheng','渑'],
				['sheng','眚'],
				['sheng','聲'],
				['sheng','聖'],
				['sheng','賸'],
				['sheng','陞'],
				['sheng','蕂'],
				['sheng','苼'],
				['sheng','鼪'],
				['sheng','鍟'],
				['sheng','繩'],
				['shi','是'],
				['shi','时'],
				['shi','事'],
				['shi','实'],
				['shi','十'],
				['shi','使'],
				['shi','世'],
				['shi','市'],
				['shi','式'],
				['shi','师'],
				['shi','识'],
				['shi','士'],
				['shi','失'],
				['shi','始'],
				['shi','示'],
				['shi','视'],
				['shi','史'],
				['shi','势'],
				['shi','施'],
				['shi','试'],
				['shi','适'],
				['shi','食'],
				['shi','室'],
				['shi','释'],
				['shi','诗'],
				['shi','氏'],
				['shi','尸'],
				['shi','驶'],
				['shi','饰'],
				['shi','湿'],
				['shi','侍'],
				['shi','拾'],
				['shi','誓'],
				['shi','逝'],
				['shi','狮'],
				['shi','蚀'],
				['shi','矢'],
				['shi','轼'],
				['shi','嗜'],
				['shi','仕'],
				['shi','恃'],
				['shi','噬'],
				['shi','屎'],
				['shi','拭'],
				['shi','柿'],
				['shi','虱'],
				['shi','谥'],
				['shi','舐'],
				['shi','視'],
				['shi','弑'],
				['shi','豕'],
				['shi','螫'],
				['shi','筮'],
				['shi','適'],
				['shi','炻'],
				['shi','鲥'],
				['shi','莳'],
				['shi','蓍'],
				['shi','釋'],
				['shi','識'],
				['shi','祏'],
				['shi','試'],
				['shi','铈'],
				['shi','諡'],
				['shi','埘'],
				['shi','絁'],
				['shi','贳'],
				['shi','辻'],
				['shi','邿'],
				['shi','眎'],
				['shi','詩'],
				['shi','鍦'],
				['shi','葹'],
				['shi','鉐'],
				['shi','籂'],
				['shi','鉃'],
				['shi','駛'],
				['shi','鳾'],
				['shi','蝕'],
				['shi','襫'],
				['shi','鈰'],
				['shi','鰤'],
				['shi','鶳'],
				['shi','鉂'],
				['shi','鼫'],
				['shi','魳'],
				['shi','蝨'],
				['shi','遈'],
				['shi','飾'],
				['shi','鉽'],
				['shou','手'],
				['shou','受'],
				['shou','收'],
				['shou','首'],
				['shou','守'],
				['shou','授'],
				['shou','售'],
				['shou','兽'],
				['shou','寿'],
				['shou','瘦'],
				['shou','狩'],
				['shou','绶'],
				['shou','艏'],
				['shou','鏉'],
				['shou','醻'],
				['shou','膄'],
				['shu','数'],
				['shu','书'],
				['shu','术'],
				['shu','属'],
				['shu','述'],
				['shu','树'],
				['shu','输'],
				['shu','束'],
				['shu','熟'],
				['shu','殊'],
				['shu','署'],
				['shu','叔'],
				['shu','舒'],
				['shu','鼠'],
				['shu','疏'],
				['shu','恕'],
				['shu','竖'],
				['shu','蜀'],
				['shu','枢'],
				['shu','淑'],
				['shu','梳'],
				['shu','蔬'],
				['shu','暑'],
				['shu','倏'],
				['shu','赎'],
				['shu','墅'],
				['shu','庶'],
				['shu','抒'],
				['shu','薯'],
				['shu','曙'],
				['shu','漱'],
				['shu','孰'],
				['shu','戍'],
				['shu','塾'],
				['shu','纾'],
				['shu','黍'],
				['shu','術'],
				['shu','菽'],
				['shu','澍'],
				['shu','秫'],
				['shu','殳'],
				['shu','姝'],
				['shu','腧'],
				['shu','沭'],
				['shu','摅'],
				['shu','輸'],
				['shu','豎'],
				['shu','毹'],
				['shu','裋'],
				['shu','紓'],
				['shu','籔'],
				['shu','蒁'],
				['shu','鵨'],
				['shu','綀'],
				['shu','鶐'],
				['shua','刷'],
				['shua','耍'],
				['shua','唰'],
				['shuai','帅'],
				['shuai','摔'],
				['shuai','甩'],
				['shuai','蟀'],
				['shuai','缞'],
				['shuan','栓'],
				['shuan','拴'],
				['shuan','闩'],
				['shuan','涮'],
				['shuan','腨'],
				['shuang','双'],
				['shuang','爽'],
				['shuang','霜'],
				['shuang','孀'],
				['shuang','雙'],
				['shuang','騻'],
				['shuang','艭'],
				['shuang','鸘'],
				['shuang','驦'],
				['shui','说'],
				['shui','水'],
				['shui','税'],
				['shui','睡'],
				['shui','稅'],
				['shui','誰'],
				['shui','脽'],
				['shui','説'],
				['shun','顺'],
				['shun','瞬'],
				['shun','舜'],
				['shun','吮'],
				['shun','順'],
				['shun','蕣'],
				['shuo','硕'],
				['shuo','烁'],
				['shuo','朔'],
				['shuo','說'],
				['shuo','槊'],
				['shuo','铄'],
				['shuo','蒴'],
				['shuo','妁'],
				['shuo','搠'],
				['shuo','矟'],
				['shuo','碩'],
				['si','斯'],
				['si','四'],
				['si','司'],
				['si','思'],
				['si','死'],
				['si','似'],
				['si','私'],
				['si','丝'],
				['si','寺'],
				['si','饲'],
				['si','撕'],
				['si','肆'],
				['si','嗣'],
				['si','嘶'],
				['si','厮'],
				['si','祀'],
				['si','巳'],
				['si','俟'],
				['si','咝'],
				['si','蛳'],
				['si','泗'],
				['si','笥'],
				['si','锶'],
				['si','姒'],
				['si','驷'],
				['si','鸶'],
				['si','汜'],
				['si','缌'],
				['si','耜'],
				['si','澌'],
				['si','鷉'],
				['si','絲'],
				['si','兕'],
				['si','覗'],
				['si','鼶'],
				['si','飔'],
				['si','禠'],
				['si','飼'],
				['si','鷥'],
				['si','蕼'],
				['si','竢'],
				['si','禩'],
				['si','緦'],
				['si','禗'],
				['so','螦'],
				['song','送'],
				['song','松'],
				['song','宋'],
				['song','耸'],
				['song','颂'],
				['song','诵'],
				['song','嵩'],
				['song','悚'],
				['song','怂'],
				['song','淞'],
				['song','凇'],
				['song','竦'],
				['song','崧'],
				['song','菘'],
				['song','鬆'],
				['song','頌'],
				['song','訟'],
				['song','駷'],
				['sou','搜'],
				['sou','嗽'],
				['sou','嗖'],
				['sou','叟'],
				['sou','擞'],
				['sou','飕'],
				['sou','馊'],
				['sou','薮'],
				['sou','嗾'],
				['sou','锼'],
				['sou','溲'],
				['sou','螋'],
				['sou','瞍'],
				['sou','蒐'],
				['sou','颼'],
				['sou','鎪'],
				['sou','蓃'],
				['su','苏'],
				['su','诉'],
				['su','速'],
				['su','素'],
				['su','俗'],
				['su','肃'],
				['su','宿'],
				['su','塑'],
				['su','稣'],
				['su','溯'],
				['su','粟'],
				['su','酥'],
				['su','愫'],
				['su','簌'],
				['su','谡'],
				['su','夙'],
				['su','窣'],
				['su','嗉'],
				['su','僳'],
				['su','涑'],
				['su','觫'],
				['su','蔌'],
				['su','蘇'],
				['su','骕'],
				['su','餗'],
				['su','訴'],
				['su','肅'],
				['su','遡'],
				['su','驌'],
				['su','藗'],
				['su','鷫'],
				['su','粛'],
				['su','鯂'],
				['su','膆'],
				['su','蘓'],
				['suan','算'],
				['suan','酸'],
				['suan','蒜'],
				['suan','狻'],
				['suan','笇'],
				['suan','筭'],
				['suan','祘'],
				['sui','随'],
				['sui','虽'],
				['sui','岁'],
				['sui','碎'],
				['sui','尿'],
				['sui','遂'],
				['sui','绥'],
				['sui','髓'],
				['sui','隋'],
				['sui','隧'],
				['sui','穗'],
				['sui','祟'],
				['sui','邃'],
				['sui','睢'],
				['sui','燧'],
				['sui','濉'],
				['sui','谇'],
				['sui','荽'],
				['sui','雖'],
				['sui','隨'],
				['sui','繸'],
				['sui','眭'],
				['sui','譢'],
				['sui','繐'],
				['sui','鞖'],
				['sui','禭'],
				['sui','簑'],
				['sui','綏'],
				['sui','鐩'],
				['sui','穟'],
				['Suī','宅'],
				['Suī','葰'],
				['sun','损'],
				['sun','孙'],
				['sun','笋'],
				['sun','荪'],
				['sun','隼'],
				['sun','飧'],
				['sun','榫'],
				['sun','狲'],
				['sun','筍'],
				['sun','蕵'],
				['suo','所'],
				['suo','索'],
				['suo','缩'],
				['suo','锁'],
				['suo','梭'],
				['suo','琐'],
				['suo','嗦'],
				['suo','唆'],
				['suo','娑'],
				['suo','蓑'],
				['suo','挲'],
				['suo','嗍'],
				['suo','羧'],
				['suo','睃'],
				['suo','唢'],
				['suo','縮'],
				['suo','桫'],
				['suo','鮻'],
				['suo','髿'],
				['suo','鎖'],
				['suo','鏁'],
				['suo','逤'],
				['ta','他'],
				['ta','她'],
				['ta','它'],
				['ta','塔'],
				['ta','踏'],
				['ta','拓'],
				['ta','塌'],
				['ta','榻'],
				['ta','嗒'],
				['ta','蹋'],
				['ta','沓'],
				['ta','鳎'],
				['ta','獭'],
				['ta','挞'],
				['ta','闼'],
				['ta','遢'],
				['ta','趿'],
				['ta','漯'],
				['ta','铊'],
				['ta','祂'],
				['ta','鰨'],
				['ta','阘'],
				['ta','溻'],
				['ta','遝'],
				['ta','闒'],
				['ta','禢'],
				['ta','蹹'],
				['ta','誻'],
				['tai','太'],
				['tai','台'],
				['tai','态'],
				['tai','泰'],
				['tai','抬'],
				['tai','胎'],
				['tai','汰'],
				['tai','苔'],
				['tai','钛'],
				['tai','肽'],
				['tai','呔'],
				['tai','跆'],
				['tai','薹'],
				['tai','邰'],
				['tai','炱'],
				['tai','骀'],
				['tai','酞'],
				['tai','鲐'],
				['tai','鈦'],
				['tai','臺'],
				['tai','粏'],
				['tai','菭'],
				['tai','舦'],
				['tai','颱'],
				['tan','谈'],
				['tan','探'],
				['tan','坦'],
				['tan','叹'],
				['tan','坛'],
				['tan','贪'],
				['tan','滩'],
				['tan','谭'],
				['tan','摊'],
				['tan','炭'],
				['tan','毯'],
				['tan','碳'],
				['tan','潭'],
				['tan','瘫'],
				['tan','痰'],
				['tan','檀'],
				['tan','坍'],
				['tan','袒'],
				['tan','忐'],
				['tan','昙'],
				['tan','覃'],
				['tan','郯'],
				['tan','钽'],
				['tan','锬'],
				['tan','貪'],
				['tan','镡'],
				['tan','談'],
				['tan','襢'],
				['tan','醰'],
				['tan','罈'],
				['tan','藫'],
				['tan','蕁'],
				['tang','唐'],
				['tang','堂'],
				['tang','躺'],
				['tang','汤'],
				['tang','糖'],
				['tang','倘'],
				['tang','塘'],
				['tang','趟'],
				['tang','棠'],
				['tang','膛'],
				['tang','烫'],
				['tang','淌'],
				['tang','搪'],
				['tang','螳'],
				['tang','傥'],
				['tang','瑭'],
				['tang','蹚'],
				['tang','帑'],
				['tang','羰'],
				['tang','醣'],
				['tang','樘'],
				['tang','溏'],
				['tang','镗'],
				['tang','螗'],
				['tang','矘'],
				['tang','鼞'],
				['tang','耥'],
				['tang','闛'],
				['tang','鞺'],
				['tang','磄'],
				['tang','鎲'],
				['tao','讨'],
				['tao','逃'],
				['tao','套'],
				['tao','陶'],
				['tao','桃'],
				['tao','涛'],
				['tao','萄'],
				['tao','掏'],
				['tao','淘'],
				['tao','滔'],
				['tao','韬'],
				['tao','饕'],
				['tao','啕'],
				['tao','绦'],
				['tao','洮'],
				['tao','鼗'],
				['tao','討'],
				['tao','迯'],
				['tao','绹'],
				['tao','絛'],
				['tao','鋾'],
				['tao','蜪'],
				['tao','醄'],
				['tao','飸'],
				['tao','謟'],
				['tao','祹'],
				['tao','駣'],
				['tao','縚'],
				['tao','縧'],
				['tao','騊'],
				['te','特'],
				['te','忒'],
				['te','忑'],
				['te','慝'],
				['te','螣'],
				['te','铽'],
				['te','脦'],
				['te','貣'],
				['teng','腾'],
				['teng','疼'],
				['teng','藤'],
				['teng','滕'],
				['teng','誊'],
				['teng','膯'],
				['teng','邆'],
				['teng','籐'],
				['teng','縢'],
				['teng','鰧'],
				['teng','騰'],
				['teng','謄'],
				['ti','体'],
				['ti','题'],
				['ti','替'],
				['ti','梯'],
				['ti','踢'],
				['ti','蹄'],
				['ti','惕'],
				['ti','啼'],
				['ti','涕'],
				['ti','屉'],
				['ti','剔'],
				['ti','剃'],
				['ti','衹'],
				['ti','嚏'],
				['ti','體'],
				['ti','倜'],
				['ti','悌'],
				['ti','題'],
				['ti','逖'],
				['ti','锑'],
				['ti','裼'],
				['ti','缇'],
				['ti','绨'],
				['ti','鹈'],
				['ti','醍'],
				['ti','荑'],
				['ti','鳀'],
				['ti','薙'],
				['ti','虒'],
				['ti','遆'],
				['ti','穉'],
				['ti','趧'],
				['ti','綈'],
				['ti','鶗'],
				['ti','軆'],
				['ti','騠'],
				['ti','緹'],
				['ti','磃'],
				['ti','逷'],
				['ti','蕛'],
				['ti','鬀'],
				['tian','天'],
				['tian','田'],
				['tian','填'],
				['tian','甜'],
				['tian','舔'],
				['tian','恬'],
				['tian','腆'],
				['tian','忝'],
				['tian','阗'],
				['tian','瞋'],
				['tian','殄'],
				['tian','畋'],
				['tian','餂'],
				['tian','铦'],
				['tian','鍩'],
				['tian','觍'],
				['tian','鴫'],
				['tian','掭'],
				['tian','黇'],
				['tian','覥'],
				['tian','紾'],
				['tian','碵'],
				['tiao','条'],
				['tiao','跳'],
				['tiao','挑'],
				['tiao','眺'],
				['tiao','迢'],
				['tiao','窕'],
				['tiao','笤'],
				['tiao','龆'],
				['tiao','佻'],
				['tiao','苕'],
				['tiao','粜'],
				['tiao','祧'],
				['tiao','蓚'],
				['tiao','髫'],
				['tiao','鲦'],
				['tiao','蜩'],
				['tiao','頫'],
				['tiao','鞗'],
				['tiao','鰷'],
				['tiao','蓨'],
				['tiao','鯈'],
				['tie','铁'],
				['tie','贴'],
				['tie','帖'],
				['tie','餮'],
				['tie','萜'],
				['tie','貼'],
				['tie','鐵'],
				['tie','驖'],
				['ting','听'],
				['ting','停'],
				['ting','庭'],
				['ting','厅'],
				['ting','艇'],
				['ting','挺'],
				['ting','廷'],
				['ting','亭'],
				['ting','霆'],
				['ting','汀'],
				['ting','婷'],
				['ting','町'],
				['ting','蜓'],
				['ting','烃'],
				['ting','葶'],
				['ting','聽'],
				['ting','梃'],
				['ting','閮'],
				['ting','莛'],
				['ting','綎'],
				['ting','鞓'],
				['ting','聴'],
				['ting','颋'],
				['ting','鼮'],
				['tong','同'],
				['tong','通'],
				['tong','统'],
				['tong','痛'],
				['tong','童'],
				['tong','铜'],
				['tong','筒'],
				['tong','桶'],
				['tong','桐'],
				['tong','佟'],
				['tong','捅'],
				['tong','瞳'],
				['tong','酮'],
				['tong','潼'],
				['tong','彤'],
				['tong','恸'],
				['tong','僮'],
				['tong','峒'],
				['tong','嗵'],
				['tong','統'],
				['tong','砼'],
				['tong','仝'],
				['tong','茼'],
				['tong','粡'],
				['tong','秱'],
				['tong','蚒'],
				['tong','綂'],
				['tong','穜'],
				['tong','餇'],
				['tong','眮'],
				['tong','銅'],
				['tong','鲖'],
				['tou','头'],
				['tou','投'],
				['tou','透'],
				['tou','偷'],
				['tou','骰'],
				['tou','頭'],
				['tou','钭'],
				['tou','紏'],
				['tou','鍮'],
				['tou','飳'],
				['tou','緰'],
				['tu','图'],
				['tu','突'],
				['tu','土'],
				['tu','徒'],
				['tu','途'],
				['tu','吐'],
				['tu','涂'],
				['tu','屠'],
				['tu','兔'],
				['tu','秃'],
				['tu','凸'],
				['tu','荼'],
				['tu','钍'],
				['tu','菟'],
				['tu','堍'],
				['tu','鋵'],
				['tu','禿'],
				['tu','酴'],
				['tu','鶟'],
				['tu','鍎'],
				['tu','葖'],
				['tu','稌'],
				['tu','莵'],
				['tu','鷵'],
				['tuan','团'],
				['tuan','湍'],
				['tuan','抟'],
				['tuan','疃'],
				['tuan','彖'],
				['tuan','蓴'],
				['tuan','糰'],
				['tuan','貒'],
				['tuan','鱄'],
				['tui','推'],
				['tui','退'],
				['tui','腿'],
				['tui','颓'],
				['tui','蜕'],
				['tui','煺'],
				['tui','脮'],
				['tui','魋'],
				['tui','隤'],
				['tui','藬'],
				['tui','頹'],
				['tui','蛻'],
				['tui','蓷'],
				['tui','蹆'],
				['tui','頺'],
				['tun','吞'],
				['tun','屯'],
				['tun','臀'],
				['tun','豚'],
				['tun','褪'],
				['tun','饨'],
				['tun','鲀'],
				['tun','暾'],
				['tun','飩'],
				['tun','臋'],
				['tun','軘'],
				['tuo','托'],
				['tuo','脱'],
				['tuo','拖'],
				['tuo','妥'],
				['tuo','陀'],
				['tuo','驼'],
				['tuo','唾'],
				['tuo','椭'],
				['tuo','鸵'],
				['tuo','沱'],
				['tuo','橐'],
				['tuo','佗'],
				['tuo','砣'],
				['tuo','跎'],
				['tuo','乇'],
				['tuo','鼍'],
				['tuo','坨'],
				['tuo','庹'],
				['tuo','箨'],
				['tuo','脫'],
				['tuo','酡'],
				['tuo','柝'],
				['tuo','柁'],
				['tuo','陁'],
				['tuo','鮀'],
				['tuo','託'],
				['tuo','袉'],
				['tuo','讬'],
				['tuo','袥'],
				['tuo','跅'],
				['tuo','駝'],
				['tuo','軃'],
				['tuo','迆'],
				['tuo','籜'],
				['tuo','駞'],
				['tuo','舃'],
				['tuo','萚'],
				['tuo','飥'],
				['tuo','鼉'],
				['tuo','砤'],
				['tuo','鱓'],
				['uu','躼'],
				['uu','艈'],
				['uu','蓞'],
				['uu','聣'],
				['uu','虲'],
				['wa','瓦'],
				['wa','娃'],
				['wa','哇'],
				['wa','蛙'],
				['wa','袜'],
				['wa','洼'],
				['wa','娲'],
				['wa','佤'],
				['wa','靺'],
				['wa','腽'],
				['wa','韈'],
				['wa','窪'],
				['wa','窊'],
				['wa','譁'],
				['wa','襪'],
				['wai','外'],
				['wai','歪'],
				['wan','完'],
				['wan','万'],
				['wan','晚'],
				['wan','湾'],
				['wan','玩'],
				['wan','弯'],
				['wan','碗'],
				['wan','顽'],
				['wan','挽'],
				['wan','丸'],
				['wan','宛'],
				['wan','腕'],
				['wan','婉'],
				['wan','皖'],
				['wan','惋'],
				['wan','蜿'],
				['wan','畹'],
				['wan','豌'],
				['wan','烷'],
				['wan','绾'],
				['wan','琬'],
				['wan','剜'],
				['wan','芄'],
				['wan','菀'],
				['wan','纨'],
				['wan','脘'],
				['wan','萬'],
				['wan','輓'],
				['wan','翫'],
				['wan','鄤'],
				['wan','蚖'],
				['wan','盌'],
				['wan','睕'],
				['wan','鍐'],
				['wan','脕'],
				['wan','貦'],
				['wan','輐'],
				['wan','関'],
				['wang','王'],
				['wang','望'],
				['wang','往'],
				['wang','网'],
				['wang','亡'],
				['wang','忘'],
				['wang','汪'],
				['wang','妄'],
				['wang','旺'],
				['wang','枉'],
				['wang','惘'],
				['wang','罔'],
				['wang','魍'],
				['wang','網'],
				['wang','辋'],
				['wang','尢'],
				['wang','菵'],
				['wang','盳'],
				['wang','迋'],
				['wang','蝄'],
				['wei','为'],
				['wei','位'],
				['wei','未'],
				['wei','委'],
				['wei','维'],
				['wei','围'],
				['wei','威'],
				['wei','微'],
				['wei','卫'],
				['wei','危'],
				['wei','味'],
				['wei','谓'],
				['wei','唯'],
				['wei','伟'],
				['wei','违'],
				['wei','尾'],
				['wei','伪'],
				['wei','慰'],
				['wei','魏'],
				['wei','韦'],
				['wei','胃'],
				['wei','喂'],
				['wei','畏'],
				['wei','尉'],
				['wei','纬'],
				['wei','萎'],
				['wei','蔚'],
				['wei','薇'],
				['wei','巍'],
				['wei','苇'],
				['wei','帷'],
				['wei','桅'],
				['wei','渭'],
				['wei','偎'],
				['wei','娓'],
				['wei','猥'],
				['wei','圩'],
				['wei','诿'],
				['wei','猬'],
				['wei','闱'],
				['wei','痿'],
				['wei','崴'],
				['wei','潍'],
				['wei','煨'],
				['wei','炜'],
				['wei','隗'],
				['wei','玮'],
				['wei','逶'],
				['wei','韪'],
				['wei','嵬'],
				['wei','艉'],
				['wei','鳚'],
				['wei','帏'],
				['wei','維'],
				['wei','鲔'],
				['wei','涠'],
				['wei','衛'],
				['wei','謂'],
				['wei','葳'],
				['wei','鮠'],
				['wei','隈'],
				['wei','霨'],
				['wei','鍏'],
				['wei','洧'],
				['wei','鄬'],
				['wei','違'],
				['wei','頠'],
				['wei','蝟'],
				['wei','苿'],
				['wei','薳'],
				['wei','緯'],
				['wei','觿'],
				['wei','讏'],
				['wei','葦'],
				['wei','硙'],
				['wei','沩'],
				['wei','鏏'],
				['wei','軎'],
				['wei','骫'],
				['wei','餵'],
				['wei','觽'],
				['wei','磈'],
				['wei','磑'],
				['wei','鳂'],
				['wei','葨'],
				['wei','藯'],
				['wei','硊'],
				['wei','趡'],
				['wei','躛'],
				['wei','骩'],
				['wei','蔿'],
				['wei','錗'],
				['wei','諉'],
				['wei','蜼'],
				['wei','鮇'],
				['wei','鮪'],
				['wen','问'],
				['wen','文'],
				['wen','闻'],
				['wen','温'],
				['wen','稳'],
				['wen','纹'],
				['wen','吻'],
				['wen','蚊'],
				['wen','雯'],
				['wen','紊'],
				['wen','瘟'],
				['wen','汶'],
				['wen','刎'],
				['wen','阌'],
				['wen','蕰'],
				['wen','璺'],
				['wen','穩'],
				['wen','藴'],
				['wen','顐'],
				['wen','聞'],
				['wen','鳁'],
				['wen','闅'],
				['wen','缊'],
				['wen','辒'],
				['wen','縕'],
				['wen','閺'],
				['wen','鼤'],
				['wen','薀'],
				['wen','紋'],
				['wen','閿'],
				['wen','緼'],
				['wen','脗'],
				['wen','螡'],
				['wen','魰'],
				['weng','翁'],
				['weng','嗡'],
				['weng','瓮'],
				['weng','蕹'],
				['weng','聬'],
				['weng','蓊'],
				['weng','鹟'],
				['weng','罋'],
				['weng','鶲'],
				['weng','齆'],
				['wo','我'],
				['wo','握'],
				['wo','沃'],
				['wo','卧'],
				['wo','窝'],
				['wo','倭'],
				['wo','蜗'],
				['wo','斡'],
				['wo','渥'],
				['wo','龌'],
				['wo','幄'],
				['wo','莴'],
				['wo','硪'],
				['wo','肟'],
				['wo','臥'],
				['wo','窩'],
				['wo','馧'],
				['wo','艧'],
				['wo','萵'],
				['wo','蝸'],
				['wo','雘'],
				['wu','无'],
				['wu','物'],
				['wu','务'],
				['wu','五'],
				['wu','武'],
				['wu','误'],
				['wu','屋'],
				['wu','午'],
				['wu','吴'],
				['wu','舞'],
				['wu','乌'],
				['wu','污'],
				['wu','伍'],
				['wu','吾'],
				['wu','悟'],
				['wu','雾'],
				['wu','巫'],
				['wu','勿'],
				['wu','呜'],
				['wu','侮'],
				['wu','晤'],
				['wu','兀'],
				['wu','毋'],
				['wu','诬'],
				['wu','捂'],
				['wu','坞'],
				['wu','芜'],
				['wu','梧'],
				['wu','鹉'],
				['wu','戊'],
				['wu','蜈'],
				['wu','妩'],
				['wu','庑'],
				['wu','鋈'],
				['wu','骛'],
				['wu','钨'],
				['wu','忤'],
				['wu','鹜'],
				['wu','婺'],
				['wu','邬'],
				['wu','浯'],
				['wu','寤'],
				['wu','焐'],
				['wu','迕'],
				['wu','芴'],
				['wu','杌'],
				['wu','誤'],
				['wu','怃'],
				['wu','仵'],
				['wu','牾'],
				['wu','鼯'],
				['wu','圬'],
				['wu','靰'],
				['wu','鹀'],
				['wu','霚'],
				['wu','痦'],
				['wu','禑'],
				['wu','霧'],
				['wu','铻'],
				['wu','茣'],
				['wu','祦'],
				['wu','鴮'],
				['wu','阢'],
				['wu','膴'],
				['wu','趶'],
				['wu','碔'],
				['wu','莁'],
				['wu','螐'],
				['wu','鰞'],
				['wu','鶩'],
				['xi','西'],
				['xi','系'],
				['xi','息'],
				['xi','希'],
				['xi','细'],
				['xi','喜'],
				['xi','习'],
				['xi','席'],
				['xi','吸'],
				['xi','戏'],
				['xi','析'],
				['xi','袭'],
				['xi','洗'],
				['xi','悉'],
				['xi','惜'],
				['xi','稀'],
				['xi','牺'],
				['xi','夕'],
				['xi','锡'],
				['xi','禧'],
				['xi','晰'],
				['xi','膝'],
				['xi','昔'],
				['xi','媳'],
				['xi','隙'],
				['xi','嘻'],
				['xi','熙'],
				['xi','兮'],
				['xi','熄'],
				['xi','嬉'],
				['xi','奚'],
				['xi','徙'],
				['xi','犀'],
				['xi','螅'],
				['xi','曦'],
				['xi','玺'],
				['xi','褶'],
				['xi','蟋'],
				['xi','熹'],
				['xi','蹊'],
				['xi','羲'],
				['xi','汐'],
				['xi','烯'],
				['xi','蜥'],
				['xi','皙'],
				['xi','醯'],
				['xi','唏'],
				['xi','淅'],
				['xi','檄'],
				['xi','僖'],
				['xi','硒'],
				['xi','阋'],
				['xi','窸'],
				['xi','翕'],
				['xi','浠'],
				['xi','饩'],
				['xi','矽'],
				['xi','細'],
				['xi','舾'],
				['xi','穸'],
				['xi','習'],
				['xi','欷'],
				['xi','葸'],
				['xi','屣'],
				['xi','隰'],
				['xi','樨'],
				['xi','觋'],
				['xi','粞'],
				['xi','菥'],
				['xi','豨'],
				['xi','禊'],
				['xi','舄'],
				['xi','鼷'],
				['xi','訢'],
				['xi','蓰'],
				['xi','郋'],
				['xi','鸂'],
				['xi','錫'],
				['xi','襲'],
				['xi','豀'],
				['xi','衋'],
				['xi','鳛'],
				['xi','騽'],
				['xi','霫'],
				['xi','糦'],
				['xi','鏭'],
				['xi','銑'],
				['xi','绤'],
				['xi','钑'],
				['xi','蓆'],
				['xi','薂'],
				['xi','睎'],
				['xi','鎴'],
				['xi','礂'],
				['xi','鑴'],
				['xi','饻'],
				['xi','翖'],
				['xi','闟'],
				['xi','餼'],
				['xi','蟢'],
				['xia','下'],
				['xia','夏'],
				['xia','侠'],
				['xia','吓'],
				['xia','辖'],
				['xia','峡'],
				['xia','狭'],
				['xia','霞'],
				['xia','瞎'],
				['xia','暇'],
				['xia','虾'],
				['xia','匣'],
				['xia','瑕'],
				['xia','遐'],
				['xia','呷'],
				['xia','黠'],
				['xia','罅'],
				['xia','郃'],
				['xia','狎'],
				['xia','硖'],
				['xia','柙'],
				['xia','鰕'],
				['xia','蝦'],
				['xia','祫'],
				['xia','陜'],
				['xia','鎋'],
				['xia','翈'],
				['xia','睱'],
				['xia','縀'],
				['xian','现'],
				['xian','先'],
				['xian','线'],
				['xian','显'],
				['xian','限'],
				['xian','险'],
				['xian','县'],
				['xian','鲜'],
				['xian','献'],
				['xian','仙'],
				['xian','陷'],
				['xian','宪'],
				['xian','闲'],
				['xian','嫌'],
				['xian','贤'],
				['xian','腺'],
				['xian','弦'],
				['xian','衔'],
				['xian','掀'],
				['xian','咸'],
				['xian','羡'],
				['xian','舷'],
				['xian','娴'],
				['xian','馅'],
				['xian','涎'],
				['xian','痫'],
				['xian','酰'],
				['xian','藓'],
				['xian','暹'],
				['xian','锨'],
				['xian','霰'],
				['xian','铣'],
				['xian','苋'],
				['xian','岘'],
				['xian','跹'],
				['xian','蚬'],
				['xian','跣'],
				['xian','冼'],
				['xian','籼'],
				['xian','燹'],
				['xian','氙'],
				['xian','線'],
				['xian','顯'],
				['xian','祆'],
				['xian','莶'],
				['xian','鹇'],
				['xian','險'],
				['xian','鮮'],
				['xian','粯'],
				['xian','纖'],
				['xian','筅'],
				['xian','猃'],
				['xian','縿'],
				['xian','軐'],
				['xian','賢'],
				['xian','韱'],
				['xian','鱻'],
				['xian','鷳'],
				['xian','県'],
				['xian','睍'],
				['xian','蜆'],
				['xian','蚿'],
				['xian','鷼'],
				['xian','縣'],
				['xian','銜'],
				['xian','閑'],
				['xian','顕'],
				['xian','鷴'],
				['xiang','想'],
				['xiang','向'],
				['xiang','相'],
				['xiang','像'],
				['xiang','象'],
				['xiang','响'],
				['xiang','项'],
				['xiang','香'],
				['xiang','乡'],
				['xiang','享'],
				['xiang','详'],
				['xiang','箱'],
				['xiang','祥'],
				['xiang','翔'],
				['xiang','湘'],
				['xiang','巷'],
				['xiang','厢'],
				['xiang','橡'],
				['xiang','镶'],
				['xiang','襄'],
				['xiang','饷'],
				['xiang','骧'],
				['xiang','庠'],
				['xiang','飨'],
				['xiang','芗'],
				['xiang','缃'],
				['xiang','響'],
				['xiang','項'],
				['xiang','詳'],
				['xiang','鲞'],
				['xiang','鄉'],
				['xiang','葙'],
				['xiang','蟓'],
				['xiang','纕'],
				['xiang','餉'],
				['xiang','饟'],
				['xiang','蘘'],
				['xiang','緗'],
				['xiang','鑲'],
				['xiang','鯗'],
				['xiang','鱌'],
				['xiao','小'],
				['xiao','笑'],
				['xiao','消'],
				['xiao','效'],
				['xiao','销'],
				['xiao','晓'],
				['xiao','削'],
				['xiao','萧'],
				['xiao','肖'],
				['xiao','孝'],
				['xiao','啸'],
				['xiao','哮'],
				['xiao','嚣'],
				['xiao','硝'],
				['xiao','霄'],
				['xiao','宵'],
				['xiao','潇'],
				['xiao','淆'],
				['xiao','逍'],
				['xiao','箫'],
				['xiao','枭'],
				['xiao','筱'],
				['xiao','骁'],
				['xiao','魈'],
				['xiao','枵'],
				['xiao','绡'],
				['xiao','哓'],
				['xiao','鸮'],
				['xiao','篠'],
				['xiao','翛'],
				['xiao','膮'],
				['xiao','笹'],
				['xiao','銷'],
				['xiao','箾'],
				['xiao','謏'],
				['xiao','詨'],
				['xiao','虈'],
				['xiao','硣'],
				['xiao','簫'],
				['xiao','簘'],
				['xiao','蟂'],
				['xiao','鴵'],
				['xiao','鵁'],
				['xiao','颵'],
				['xiao','鴞'],
				['xie','些'],
				['xie','写'],
				['xie','血'],
				['xie','协'],
				['xie','谢'],
				['xie','胁'],
				['xie','邪'],
				['xie','鞋'],
				['xie','械'],
				['xie','斜'],
				['xie','歇'],
				['xie','携'],
				['xie','泄'],
				['xie','谐'],
				['xie','卸'],
				['xie','屑'],
				['xie','懈'],
				['xie','泻'],
				['xie','蟹'],
				['xie','亵'],
				['xie','颉'],
				['xie','楔'],
				['xie','蝎'],
				['xie','榭'],
				['xie','燮'],
				['xie','邂'],
				['xie','撷'],
				['xie','缬'],
				['xie','勰'],
				['xie','獬'],
				['xie','骱'],
				['xie','瀣'],
				['xie','謝'],
				['xie','榍'],
				['xie','渫'],
				['xie','廨'],
				['xie','躞'],
				['xie','绁'],
				['xie','薤'],
				['xie','龤'],
				['xie','脅'],
				['xie','韰'],
				['xie','藛'],
				['xie','薢'],
				['xie','齛'],
				['xie','缷'],
				['xie','蠏'],
				['xie','諧'],
				['xie','綊'],
				['xie','蠍'],
				['xie','蝑'],
				['xin','心'],
				['xin','新'],
				['xin','信'],
				['xin','辛'],
				['xin','欣'],
				['xin','薪'],
				['xin','芯'],
				['xin','馨'],
				['xin','衅'],
				['xin','锌'],
				['xin','鑫'],
				['xin','忻'],
				['xin','歆'],
				['xin','昕'],
				['xin','囟'],
				['xin','鋅'],
				['xin','舋'],
				['xing','性'],
				['xing','形'],
				['xing','兴'],
				['xing','星'],
				['xing','型'],
				['xing','幸'],
				['xing','醒'],
				['xing','刑'],
				['xing','姓'],
				['xing','猩'],
				['xing','腥'],
				['xing','杏'],
				['xing','惺'],
				['xing','邢'],
				['xing','悻'],
				['xing','硎'],
				['xing','饧'],
				['xing','荥'],
				['xing','擤'],
				['xing','陉'],
				['xing','荇'],
				['xing','興'],
				['xing','铏'],
				['xing','篂'],
				['xing','骍'],
				['xing','臖'],
				['xing','莕'],
				['xiong','雄'],
				['xiong','兄'],
				['xiong','胸'],
				['xiong','凶'],
				['xiong','熊'],
				['xiong','匈'],
				['xiong','汹'],
				['xiong','芎'],
				['xiong','詾'],
				['xiong','诇'],
				['xiong','詗'],
				['xiu','修'],
				['xiu','休'],
				['xiu','秀'],
				['xiu','袖'],
				['xiu','羞'],
				['xiu','绣'],
				['xiu','朽'],
				['xiu','嗅'],
				['xiu','锈'],
				['xiu','馐'],
				['xiu','咻'],
				['xiu','岫'],
				['xiu','溴'],
				['xiu','庥'],
				['xiu','鸺'],
				['xiu','髹'],
				['xiu','貅'],
				['xiu','髤'],
				['xiu','脙'],
				['xiu','鵂'],
				['xiu','饈'],
				['xiu','銝'],
				['xiu','螑'],
				['xiu','鏽'],
				['xu','许'],
				['xu','需'],
				['xu','须'],
				['xu','续'],
				['xu','序'],
				['xu','虚'],
				['xu','徐'],
				['xu','绪'],
				['xu','叙'],
				['xu','蓄'],
				['xu','吁'],
				['xu','旭'],
				['xu','墟'],
				['xu','嘘'],
				['xu','婿'],
				['xu','絮'],
				['xu','恤'],
				['xu','顼'],
				['xu','胥'],
				['xu','栩'],
				['xu','诩'],
				['xu','戌'],
				['xu','煦'],
				['xu','蓿'],
				['xu','酗'],
				['xu','虛'],
				['xu','勖'],
				['xu','溆'],
				['xu','須'],
				['xu','繻'],
				['xu','盱'],
				['xu','續'],
				['xu','魆'],
				['xu','許'],
				['xu','藚'],
				['xu','谞'],
				['xu','洫'],
				['xu','糈'],
				['xu','緒'],
				['xu','鄦'],
				['xu','諝'],
				['xu','譃'],
				['xu','虗'],
				['xu','魖'],
				['xu','醑'],
				['xu','聟'],
				['xu','訏'],
				['xu','鬚'],
				['xu','盨'],
				['xu','稸'],
				['xu','蕦'],
				['xu','頊'],
				['xu','訹'],
				['xu','盢'],
				['xuan','选'],
				['xuan','宣'],
				['xuan','旋'],
				['xuan','玄'],
				['xuan','悬'],
				['xuan','璇'],
				['xuan','轩'],
				['xuan','喧'],
				['xuan','眩'],
				['xuan','炫'],
				['xuan','渲'],
				['xuan','漩'],
				['xuan','绚'],
				['xuan','暄'],
				['xuan','癣'],
				['xuan','煊'],
				['xuan','萱'],
				['xuan','谖'],
				['xuan','铉'],
				['xuan','楦'],
				['xuan','泫'],
				['xuan','選'],
				['xuan','揎'],
				['xuan','儇'],
				['xuan','镟'],
				['xuan','譞'],
				['xuan','蕿'],
				['xuan','蜁'],
				['xuan','翧'],
				['xuan','衒'],
				['xuan','絃'],
				['xuan','軒'],
				['xuan','袨'],
				['xuan','矎'],
				['xuan','縼'],
				['xuan','禤'],
				['xuan','痃'],
				['xuan','蔙'],
				['xuan','駨'],
				['xuan','諠'],
				['xuan','睻'],
				['xue','学'],
				['xue','雪'],
				['xue','穴'],
				['xue','薛'],
				['xue','靴'],
				['xue','谑'],
				['xue','鳕'],
				['xue','泶'],
				['xue','袕'],
				['xue','鸴'],
				['xue','鷽'],
				['xun','寻'],
				['xun','训'],
				['xun','迅'],
				['xun','讯'],
				['xun','巡'],
				['xun','询'],
				['xun','逊'],
				['xun','循'],
				['xun','旬'],
				['xun','勋'],
				['xun','驯'],
				['xun','熏'],
				['xun','殉'],
				['xun','荀'],
				['xun','汛'],
				['xun','薰'],
				['xun','徇'],
				['xun','醺'],
				['xun','洵'],
				['xun','峋'],
				['xun','鲟'],
				['xun','浔'],
				['xun','恂'],
				['xun','巽'],
				['xun','獯'],
				['xun','埙'],
				['xun','窨'],
				['xun','曛'],
				['xun','訓'],
				['xun','鄩'],
				['xun','蕈'],
				['xun','詢'],
				['xun','訊'],
				['xun','纁'],
				['xun','郇'],
				['xun','訙'],
				['xun','臐'],
				['xun','紃'],
				['xun','遜'],
				['xun','矄'],
				['xun','稄'],
				['xun','蟳'],
				['xun','顨'],
				['xun','馴'],
				['ya','亚'],
				['ya','压'],
				['ya','呀'],
				['ya','牙'],
				['ya','雅'],
				['ya','押'],
				['ya','鸭'],
				['ya','讶'],
				['ya','哑'],
				['ya','鸦'],
				['ya','涯'],
				['ya','丫'],
				['ya','芽'],
				['ya','衙'],
				['ya','娅'],
				['ya','轧'],
				['ya','伢'],
				['ya','蚜'],
				['ya','桠'],
				['ya','氩'],
				['ya','琊'],
				['ya','垭'],
				['ya','迓'],
				['ya','睚'],
				['ya','砑'],
				['ya','揠'],
				['ya','岈'],
				['ya','錏'],
				['ya','痖'],
				['ya','铔'],
				['ya','鴨'],
				['ya','訝'],
				['ya','鴉'],
				['ya','顔'],
				['ya','穵'],
				['ya','輵'],
				['ya','襾'],
				['ya','窫'],
				['yai','崖'],
				['yan','眼'],
				['yan','言'],
				['yan','研'],
				['yan','验'],
				['yan','严'],
				['yan','演'],
				['yan','烟'],
				['yan','延'],
				['yan','沿'],
				['yan','炎'],
				['yan','岩'],
				['yan','掩'],
				['yan','颜'],
				['yan','厌'],
				['yan','燕'],
				['yan','盐'],
				['yan','咽'],
				['yan','艳'],
				['yan','宴'],
				['yan','焰'],
				['yan','殷'],
				['yan','衍'],
				['yan','阎'],
				['yan','焉'],
				['yan','雁'],
				['yan','彦'],
				['yan','淹'],
				['yan','檐'],
				['yan','奄'],
				['yan','堰'],
				['yan','嫣'],
				['yan','俨'],
				['yan','谚'],
				['yan','蜒'],
				['yan','晏'],
				['yan','阉'],
				['yan','筵'],
				['yan','湮'],
				['yan','砚'],
				['yan','偃'],
				['yan','腌'],
				['yan','胭'],
				['yan','魇'],
				['yan','妍'],
				['yan','赝'],
				['yan','唁'],
				['yan','兖'],
				['yan','鼹'],
				['yan','驗'],
				['yan','琰'],
				['yan','餍'],
				['yan','菸'],
				['yan','厣'],
				['yan','恹'],
				['yan','鄢'],
				['yan','焱'],
				['yan','闫'],
				['yan','滟'],
				['yan','郾'],
				['yan','酽'],
				['yan','罨'],
				['yan','谳'],
				['yan','崦'],
				['yan','硏'],
				['yan','龂'],
				['yan','顏'],
				['yan','鹽'],
				['yan','顩'],
				['yan','蝘'],
				['yan','鷃'],
				['yan','綖'],
				['yan','軅'],
				['yan','菴'],
				['yan','讞'],
				['yan','麣'],
				['yan','顑'],
				['yan','簷'],
				['yan','閻'],
				['yan','艷'],
				['yan','莚'],
				['yan','硽'],
				['yan','酀'],
				['yan','龑'],
				['yan','醃'],
				['yan','黡'],
				['yan','醼'],
				['yan','齗'],
				['yan','饜'],
				['yan','讌'],
				['yan','贋'],
				['yan','艶'],
				['yan','鶠'],
				['yan','黶'],
				['yan','訮'],
				['yan','諺'],
				['yan','閹'],
				['yan','鬳'],
				['yang','样'],
				['yang','阳'],
				['yang','养'],
				['yang','央'],
				['yang','洋'],
				['yang','杨'],
				['yang','扬'],
				['yang','羊'],
				['yang','仰'],
				['yang','痒'],
				['yang','疡'],
				['yang','佯'],
				['yang','鞅'],
				['yang','秧'],
				['yang','殃'],
				['yang','漾'],
				['yang','恙'],
				['yang','鸯'],
				['yang','怏'],
				['yang','炀'],
				['yang','陽'],
				['yang','泱'],
				['yang','烊'],
				['yang','養'],
				['yang','蛘'],
				['yang','徉'],
				['yang','飏'],
				['yang','駚'],
				['yang','諹'],
				['yang','鴦'],
				['yang','钖'],
				['yang','輰'],
				['yang','眏'],
				['yang','颺'],
				['yang','羕'],
				['yang','鴹'],
				['yang','紻'],
				['yang','詇'],
				['yang','鍚'],
				['yao','要'],
				['yao','约'],
				['yao','药'],
				['yao','摇'],
				['yao','腰'],
				['yao','咬'],
				['yao','遥'],
				['yao','耀'],
				['yao','姚'],
				['yao','窑'],
				['yao','钥'],
				['yao','谣'],
				['yao','瑶'],
				['yao','夭'],
				['yao','尧'],
				['yao','肴'],
				['yao','吆'],
				['yao','藥'],
				['yao','鹞'],
				['yao','曜'],
				['yao','杳'],
				['yao','舀'],
				['yao','幺'],
				['yao','窈'],
				['yao','爻'],
				['yao','徭'],
				['yao','崾'],
				['yao','繇'],
				['yao','崤'],
				['yao','鳐'],
				['yao','踰'],
				['yao','轺'],
				['yao','珧'],
				['yao','籥'],
				['yao','艞'],
				['yao','靿'],
				['yao','飖'],
				['yao','遙'],
				['yao','葯'],
				['yao','闄'],
				['yao','窔'],
				['yao','矅'],
				['yao','窅'],
				['yao','蕘'],
				['yao','餆'],
				['yao','祅'],
				['yao','餚'],
				['yao','窯'],
				['yao','蓔'],
				['yao','謡'],
				['yao','箹'],
				['yao','謠'],
				['yao','鑰'],
				['yao','訞'],
				['yao','颻'],
				['ye','也'],
				['ye','业'],
				['ye','夜'],
				['ye','野'],
				['ye','叶'],
				['ye','爷'],
				['ye','页'],
				['ye','耶'],
				['ye','液'],
				['ye','冶'],
				['ye','拽'],
				['ye','曳'],
				['ye','谒'],
				['ye','腋'],
				['ye','掖'],
				['ye','椰'],
				['ye','噎'],
				['ye','邺'],
				['ye','晔'],
				['ye','烨'],
				['ye','靥'],
				['ye','揶'],
				['ye','葉'],
				['ye','頁'],
				['ye','虵'],
				['ye','鐷'],
				['ye','緤'],
				['ye','鍱'],
				['yen','膶'],
				['yi','一'],
				['yi','以'],
				['yi','意'],
				['yi','已'],
				['yi','义'],
				['yi','议'],
				['yi','易'],
				['yi','医'],
				['yi','依'],
				['yi','益'],
				['yi','疑'],
				['yi','异'],
				['yi','衣'],
				['yi','伊'],
				['yi','艺'],
				['yi','移'],
				['yi','亦'],
				['yi','遗'],
				['yi','亿'],
				['yi','译'],
				['yi','役'],
				['yi','仪'],
				['yi','宜'],
				['yi','翼'],
				['yi','忆'],
				['yi','椅'],
				['yi','抑'],
				['yi','疫'],
				['yi','乙'],
				['yi','毅'],
				['yi','矣'],
				['yi','谊'],
				['yi','姨'],
				['yi','夷'],
				['yi','逸'],
				['yi','溢'],
				['yi','蚁'],
				['yi','怡'],
				['yi','倚'],
				['yi','裔'],
				['yi','懿'],
				['yi','颐'],
				['yi','绎'],
				['yi','奕'],
				['yi','咦'],
				['yi','邑'],
				['yi','胰'],
				['yi','诣'],
				['yi','贻'],
				['yi','揖'],
				['yi','彝'],
				['yi','醫'],
				['yi','驿'],
				['yi','漪'],
				['yi','翌'],
				['yi','臆'],
				['yi','沂'],
				['yi','佚'],
				['yi','屹'],
				['yi','轶'],
				['yi','熠'],
				['yi','弋'],
				['yi','诒'],
				['yi','弈'],
				['yi','翊'],
				['yi','呓'],
				['yi','噫'],
				['yi','蜴'],
				['yi','壹'],
				['yi','薏'],
				['yi','迤'],
				['yi','刈'],
				['yi','咿'],
				['yi','铱'],
				['yi','旖'],
				['yi','羿'],
				['yi','苡'],
				['yi','缢'],
				['yi','翳'],
				['yi','镒'],
				['yi','饴'],
				['yi','峄'],
				['yi','痍'],
				['yi','悒'],
				['yi','肄'],
				['yi','猗'],
				['yi','挹'],
				['yi','欹'],
				['yi','钇'],
				['yi','锜'],
				['yi','癔'],
				['yi','仡'],
				['yi','義'],
				['yi','祎'],
				['yi','眙'],
				['yi','遺'],
				['yi','議'],
				['yi','怿'],
				['yi','嶷'],
				['yi','圯'],
				['yi','佾'],
				['yi','瘗'],
				['yi','讉'],
				['yi','埸'],
				['yi','劓'],
				['yi','黟'],
				['yi','迻'],
				['yi','袆'],
				['yi','鎰'],
				['yi','螘'],
				['yi','顗'],
				['yi','袣'],
				['yi','镱'],
				['yi','殪'],
				['yi','頉'],
				['yi','齮'],
				['yi','睪'],
				['yi','齸'],
				['yi','詣'],
				['yi','藙'],
				['yi','蟻'],
				['yi','蓺'],
				['yi','笖'],
				['yi','鮨'],
				['yi','鷧'],
				['yi','絏'],
				['yi','藝'],
				['yi','譯'],
				['yi','艤'],
				['yi','蛡'],
				['yi','貤'],
				['yi','舣'],
				['yi','鶃'],
				['yi','褹'],
				['yi','阤'],
				['yi','簃'],
				['yi','肊'],
				['yi','訑'],
				['yi','頤'],
				['yi','詒'],
				['yi','裛'],
				['yi','耴'],
				['yi','輗'],
				['yi','釔'],
				['yi','饐'],
				['yi','豷'],
				['yi','礒'],
				['yi','蛜'],
				['yi','鶍'],
				['yi','鷁'],
				['yi','鷖'],
				['yi','苅'],
				['yi','謚'],
				['yi','鐿'],
				['yi','箷'],
				['yi','袘'],
				['yi','讛'],
				['yi','貽'],
				['yi','贀'],
				['yi','鴺'],
				['yi','羠'],
				['yi','驛'],
				['yi','螠'],
				['yin','因'],
				['yin','引'],
				['yin','音'],
				['yin','印'],
				['yin','银'],
				['yin','阴'],
				['yin','隐'],
				['yin','饮'],
				['yin','姻'],
				['yin','吟'],
				['yin','淫'],
				['yin','荫'],
				['yin','茵'],
				['yin','尹'],
				['yin','瘾'],
				['yin','寅'],
				['yin','胤'],
				['yin','蚓'],
				['yin','垠'],
				['yin','氤'],
				['yin','喑'],
				['yin','陰'],
				['yin','夤'],
				['yin','縯'],
				['yin','飲'],
				['yin','隱'],
				['yin','鄞'],
				['yin','狺'],
				['yin','洇'],
				['yin','吲'],
				['yin','堙'],
				['yin','铟'],
				['yin','霪'],
				['yin','鮣'],
				['yin','銀'],
				['yin','蟫'],
				['yin','骃'],
				['yin','粌'],
				['yin','訚'],
				['yin','禋'],
				['yin','秵'],
				['yin','絪'],
				['yin','闉'],
				['yin','駰'],
				['yin','鷣'],
				['yin','茚'],
				['yin','裀'],
				['yin','隂'],
				['yin','霠'],
				['yin','齦'],
				['yin','銦'],
				['yin','飮'],
				['yin','訔'],
				['yin','酳'],
				['yin','誾'],
				['yin','碒'],
				['yin','蒑'],
				['yin','訡'],
				['yin','陻'],
				['yin','釿'],
				['yin','靷'],
				['ying','应'],
				['ying','英'],
				['ying','影'],
				['ying','营'],
				['ying','迎'],
				['ying','硬'],
				['ying','映'],
				['ying','赢'],
				['ying','鹰'],
				['ying','盈'],
				['ying','婴'],
				['ying','颖'],
				['ying','莹'],
				['ying','蝇'],
				['ying','樱'],
				['ying','莺'],
				['ying','瑛'],
				['ying','荧'],
				['ying','萤'],
				['ying','鹦'],
				['ying','膺'],
				['ying','颍'],
				['ying','瀛'],
				['ying','萦'],
				['ying','缨'],
				['ying','嘤'],
				['ying','罂'],
				['ying','嬴'],
				['ying','滢'],
				['ying','郢'],
				['ying','楹'],
				['ying','潆'],
				['ying','瘿'],
				['ying','茔'],
				['ying','璎'],
				['ying','撄'],
				['ying','媵'],
				['ying','蓥'],
				['ying','蝿'],
				['ying','贏'],
				['ying','韺'],
				['ying','鷹'],
				['ying','锳'],
				['ying','軈'],
				['ying','譻'],
				['ying','縈'],
				['ying','藀'],
				['ying','蠅'],
				['ying','罃'],
				['ying','螢'],
				['yo','哟'],
				['yo','唷'],
				['yong','用'],
				['yong','永'],
				['yong','拥'],
				['yong','勇'],
				['yong','涌'],
				['yong','庸'],
				['yong','佣'],
				['yong','泳'],
				['yong','雍'],
				['yong','咏'],
				['yong','踊'],
				['yong','恿'],
				['yong','蛹'],
				['yong','甬'],
				['yong','俑'],
				['yong','臃'],
				['yong','邕'],
				['yong','慵'],
				['yong','痈'],
				['yong','饔'],
				['yong','壅'],
				['yong','镛'],
				['yong','喁'],
				['yong','墉'],
				['yong','鳙'],
				['yong','鯒'],
				['yong','鄘'],
				['yong','禜'],
				['yong','詠'],
				['yong','踴'],
				['yong','鰫'],
				['yong','鲬'],
				['yong','筩'],
				['you','有'],
				['you','又'],
				['you','由'],
				['you','友'],
				['you','游'],
				['you','优'],
				['you','右'],
				['you','油'],
				['you','尤'],
				['you','犹'],
				['you','忧'],
				['you','幼'],
				['you','幽'],
				['you','邮'],
				['you','诱'],
				['you','悠'],
				['you','佑'],
				['you','釉'],
				['you','铀'],
				['you','攸'],
				['you','黝'],
				['you','疣'],
				['you','酉'],
				['you','祐'],
				['you','柚'],
				['you','囿'],
				['you','鱿'],
				['you','宥'],
				['you','猷'],
				['you','蚴'],
				['you','莜'],
				['you','莠'],
				['you','莸'],
				['you','牖'],
				['you','呦'],
				['you','鼬'],
				['you','卣'],
				['you','蝣'],
				['you','蚰'],
				['you','侑'],
				['you','脩'],
				['you','輶'],
				['you','铕'],
				['you','鲉'],
				['you','莤'],
				['you','誘'],
				['you','羑'],
				['you','逌'],
				['you','遊'],
				['you','蜏'],
				['you','鈾'],
				['you','麀'],
				['you','蚘'],
				['you','郵'],
				['you','鮋'],
				['you','脜'],
				['you','褏'],
				['you','耰'],
				['you','褎'],
				['you','苃'],
				['you','羗'],
				['you','蓧'],
				['yu','于'],
				['yu','与'],
				['yu','语'],
				['yu','育'],
				['yu','预'],
				['yu','余'],
				['yu','域'],
				['yu','鱼'],
				['yu','遇'],
				['yu','予'],
				['yu','雨'],
				['yu','玉'],
				['yu','欲'],
				['yu','宇'],
				['yu','愈'],
				['yu','御'],
				['yu','狱'],
				['yu','誉'],
				['yu','渔'],
				['yu','愚'],
				['yu','郁'],
				['yu','豫'],
				['yu','愉'],
				['yu','裕'],
				['yu','於'],
				['yu','寓'],
				['yu','喻'],
				['yu','浴'],
				['yu','舆'],
				['yu','娱'],
				['yu','谕'],
				['yu','逾'],
				['yu','屿'],
				['yu','瑜'],
				['yu','禹'],
				['yu','虞'],
				['yu','迂'],
				['yu','毓'],
				['yu','禺'],
				['yu','俞'],
				['yu','榆'],
				['yu','隅'],
				['yu','驭'],
				['yu','聿'],
				['yu','淤'],
				['yu','渝'],
				['yu','煜'],
				['yu','欤'],
				['yu','谀'],
				['yu','盂'],
				['yu','瘀'],
				['yu','馀'],
				['yu','芋'],
				['yu','峪'],
				['yu','與'],
				['yu','熨'],
				['yu','钰'],
				['yu','觎'],
				['yu','腴'],
				['yu','臾'],
				['yu','昱'],
				['yu','阈'],
				['yu','揄'],
				['yu','妪'],
				['yu','鹬'],
				['yu','龉'],
				['yu','俣'],
				['yu','竽'],
				['yu','庾'],
				['yu','畲'],
				['yu','饫'],
				['yu','萸'],
				['yu','纡'],
				['yu','鬻'],
				['yu','鹆'],
				['yu','圄'],
				['yu','蜮'],
				['yu','窳'],
				['yu','嵛'],
				['yu','肀'],
				['yu','窬'],
				['yu','伛'],
				['yu','預'],
				['yu','穀'],
				['yu','燠'],
				['yu','語'],
				['yu','顒'],
				['yu','蝓'],
				['yu','餘'],
				['yu','颙'],
				['yu','圉'],
				['yu','雩'],
				['yu','遹'],
				['yu','狳'],
				['yu','鬱'],
				['yu','舁'],
				['yu','瘐'],
				['yu','蓣'],
				['yu','鴥'],
				['yu','妤'],
				['yu','貐'],
				['yu','魚'],
				['yu','矞'],
				['yu','隃'],
				['yu','禦'],
				['yu','鋊'],
				['yu','頨'],
				['yu','蝺'],
				['yu','鴪'],
				['yu','邘'],
				['yu','穻'],
				['yu','轝'],
				['yu','礜'],
				['yu','譽'],
				['yu','軉'],
				['yu','鴧'],
				['yu','籲'],
				['yu','藇'],
				['yu','翑'],
				['yu','諛'],
				['yu','輿'],
				['yu','鈺'],
				['yu','閾'],
				['yu','籞'],
				['yu','隩'],
				['yu','貗'],
				['yu','魣'],
				['yu','鐍'],
				['yu','驈'],
				['yu','鱮'],
				['yu','荢'],
				['yu','衘'],
				['yu','袬'],
				['yu','盓'],
				['yu','祤'],
				['yu','蘌'],
				['yu','釪'],
				['yu','錥'],
				['yu','礇'],
				['yu','羭'],
				['yu','蕷'],
				['yu','鳿'],
				['yuan','原'],
				['yuan','员'],
				['yuan','院'],
				['yuan','元'],
				['yuan','远'],
				['yuan','愿'],
				['yuan','源'],
				['yuan','园'],
				['yuan','圆'],
				['yuan','援'],
				['yuan','缘'],
				['yuan','怨'],
				['yuan','袁'],
				['yuan','渊'],
				['yuan','猿'],
				['yuan','冤'],
				['yuan','苑'],
				['yuan','垣'],
				['yuan','辕'],
				['yuan','鸳'],
				['yuan','媛'],
				['yuan','沅'],
				['yuan','爰'],
				['yuan','鸢'],
				['yuan','鼋'],
				['yuan','芫'],
				['yuan','螈'],
				['yuan','掾'],
				['yuan','塬'],
				['yuan','瑗'],
				['yuan','遠'],
				['yuan','眢'],
				['yuan','垸'],
				['yuan','橼'],
				['yuan','緣'],
				['yuan','鈨'],
				['yuan','箢'],
				['yuan','贠'],
				['yuan','鹓'],
				['yuan','願'],
				['yuan','蝝'],
				['yuan','蜎'],
				['yuan','肙'],
				['yuan','鶢'],
				['yuan','黿'],
				['yuan','縁'],
				['yuan','葾'],
				['yuan','褑'],
				['yuan','薗'],
				['yuan','鋺'],
				['yuan','禐'],
				['yuan','鳶'],
				['yuan','鼘'],
				['yuan','笎'],
				['yue','月'],
				['yue','越'],
				['yue','阅'],
				['yue','跃'],
				['yue','曰'],
				['yue','岳'],
				['yue','悦'],
				['yue','粤'],
				['yue','樾'],
				['yue','刖'],
				['yue','钺'],
				['yue','約'],
				['yue','閱'],
				['yue','龠'],
				['yue','哕'],
				['yue','瀹'],
				['yue','躍'],
				['yue','籆'],
				['yue','趯'],
				['yue','軏'],
				['yue','粵'],
				['yue','礿'],
				['yue','躒'],
				['yun','运'],
				['yun','云'],
				['yun','允'],
				['yun','孕'],
				['yun','晕'],
				['yun','韵'],
				['yun','蕴'],
				['yun','匀'],
				['yun','芸'],
				['yun','陨'],
				['yun','酝'],
				['yun','鋆'],
				['yun','耘'],
				['yun','纭'],
				['yun','氲'],
				['yun','愠'],
				['yun','筠'],
				['yun','殒'],
				['yun','恽'],
				['yun','郧'],
				['yun','郓'],
				['yun','昀'],
				['yun','韫'],
				['yun','運'],
				['yun','狁'],
				['yun','赟'],
				['yun','贇'],
				['yun','雲'],
				['yun','韻'],
				['yun','蒷'],
				['yun','腪'],
				['yun','筼'],
				['yun','頵'],
				['yun','緷'],
				['yun','醖'],
				['yun','韗'],
				['yun','眃'],
				['yun','繧'],
				['yun','蒕'],
				['yun','縜'],
				['yun','馻'],
				['yun','霣'],
				['yun','賱'],
				['yun','醞'],
				['yun','餫'],
				['za','杂'],
				['za','咱'],
				['za','扎'],
				['za','砸'],
				['za','咂'],
				['za','匝'],
				['za','雜'],
				['za','臜'],
				['za','襍'],
				['za','紮'],
				['za','臢'],
				['za','鉔'],
				['za','雑'],
				['za','韴'],
				['zai','在'],
				['zai','再'],
				['zai','载'],
				['zai','灾'],
				['zai','宰'],
				['zai','栽'],
				['zai','哉'],
				['zai','崽'],
				['zai','甾'],
				['zai','載'],
				['zai','縡'],
				['zai','酨'],
				['zai','賳'],
				['zan','赞'],
				['zan','暂'],
				['zan','瓒'],
				['zan','簪'],
				['zan','錾'],
				['zan','糌'],
				['zan','昝'],
				['zan','趱'],
				['zan','拶'],
				['zan','篸'],
				['zan','讚'],
				['zan','酂'],
				['zan','酇'],
				['zan','蹔'],
				['zan','贊'],
				['zan','饡'],
				['zang','脏'],
				['zang','葬'],
				['zang','赃'],
				['zang','臧'],
				['zang','奘'],
				['zang','臟'],
				['zang','驵'],
				['zang','髒'],
				['zang','臓'],
				['zao','造'],
				['zao','早'],
				['zao','遭'],
				['zao','糟'],
				['zao','躁'],
				['zao','燥'],
				['zao','藻'],
				['zao','灶'],
				['zao','澡'],
				['zao','凿'],
				['zao','噪'],
				['zao','枣'],
				['zao','皂'],
				['zao','蚤'],
				['zao','唣'],
				['zao','譟'],
				['zao','趮'],
				['zao','艁'],
				['zao','蹧'],
				['zao','鑿'],
				['zao','竈'],
				['ze','则'],
				['ze','责'],
				['ze','泽'],
				['ze','择'],
				['ze','咋'],
				['ze','啧'],
				['ze','仄'],
				['ze','赜'],
				['ze','笮'],
				['ze','昃'],
				['ze','舴'],
				['ze','帻'],
				['ze','迮'],
				['ze','責'],
				['ze','箦'],
				['ze','礋'],
				['ze','齰'],
				['ze','蠌'],
				['zei','贼'],
				['zei','鲗'],
				['zei','蠈'],
				['zei','賊'],
				['zen','怎'],
				['zen','谮'],
				['zeng','增'],
				['zeng','赠'],
				['zeng','憎'],
				['zeng','锃'],
				['zeng','甑'],
				['zeng','缯'],
				['zeng','罾'],
				['zeng','矰'],
				['zeng','贈'],
				['zeng','譄'],
				['zeng','鄫'],
				['zeng','繒'],
				['zeng','磳'],
				['zha','炸'],
				['zha','眨'],
				['zha','诈'],
				['zha','乍'],
				['zha','栅'],
				['zha','渣'],
				['zha','札'],
				['zha','闸'],
				['zha','榨'],
				['zha','蚱'],
				['zha','铡'],
				['zha','咤'],
				['zha','吒'],
				['zha','馇'],
				['zha','砟'],
				['zha','醡'],
				['zha','霅'],
				['zha','揸'],
				['zha','蹅'],
				['zha','鲝'],
				['zha','鲊'],
				['zha','痄'],
				['zha','齄'],
				['zha','苲'],
				['zha','哳'],
				['zha','詐'],
				['zha','譗'],
				['zhai','债'],
				['zhai','摘'],
				['zhai','窄'],
				['zhai','寨'],
				['zhai','斋'],
				['zhai','砦'],
				['zhai','瘵'],
				['zhai','齋'],
				['zhan','战'],
				['zhan','展'],
				['zhan','站'],
				['zhan','占'],
				['zhan','颤'],
				['zhan','沾'],
				['zhan','斩'],
				['zhan','詹'],
				['zhan','盏'],
				['zhan','瞻'],
				['zhan','崭'],
				['zhan','绽'],
				['zhan','湛'],
				['zhan','栈'],
				['zhan','毡'],
				['zhan','辗'],
				['zhan','蘸'],
				['zhan','旃'],
				['zhan','谵'],
				['zhan','觇'],
				['zhan','霑'],
				['zhan','讝'],
				['zhan','饘'],
				['zhan','鹯'],
				['zhan','邅'],
				['zhan','鳣'],
				['zhan','驙'],
				['zhan','搌'],
				['zhan','飐'],
				['zhan','盞'],
				['zhan','譫'],
				['zhan','鸇'],
				['zhan','薝'],
				['zhan','菚'],
				['zhan','醆'],
				['zhan','詀'],
				['zhang','张'],
				['zhang','章'],
				['zhang','掌'],
				['zhang','障'],
				['zhang','丈'],
				['zhang','帐'],
				['zhang','仗'],
				['zhang','涨'],
				['zhang','胀'],
				['zhang','账'],
				['zhang','杖'],
				['zhang','彰'],
				['zhang','璋'],
				['zhang','漳'],
				['zhang','樟'],
				['zhang','蟑'],
				['zhang','瘴'],
				['zhang','嶂'],
				['zhang','鄣'],
				['zhang','獐'],
				['zhang','幛'],
				['zhang','脹'],
				['zhang','嫜'],
				['zhang','鐣'],
				['zhang','仉'],
				['zhang','餦'],
				['zhang','瞕'],
				['zhang','鞝'],
				['zhang','麞'],
				['zhang','粻'],
				['zhang','鏱'],
				['zhang','蔁'],
				['zhang','騿'],
				['zhao','着'],
				['zhao','照'],
				['zhao','找'],
				['zhao','招'],
				['zhao','赵'],
				['zhao','罩'],
				['zhao','昭'],
				['zhao','兆'],
				['zhao','沼'],
				['zhao','诏'],
				['zhao','肇'],
				['zhao','钊'],
				['zhao','笊'],
				['zhao','鼂'],
				['zhao','鮡'],
				['zhao','謿'],
				['zhao','趙'],
				['zhao','駋'],
				['zhe','这'],
				['zhe','者'],
				['zhe','哲'],
				['zhe','浙'],
				['zhe','遮'],
				['zhe','辄'],
				['zhe','蔗'],
				['zhe','這'],
				['zhe','蛰'],
				['zhe','摺'],
				['zhe','蜇'],
				['zhe','赭'],
				['zhe','谪'],
				['zhe','鹧'],
				['zhe','柘'],
				['zhe','磔'],
				['zhe','锗'],
				['zhe','謺'],
				['zhe','袩'],
				['zhe','粍'],
				['zhe','詟'],
				['zhe','輒'],
				['zhe','蟅'],
				['zhen','真'],
				['zhen','阵'],
				['zhen','镇'],
				['zhen','震'],
				['zhen','针'],
				['zhen','振'],
				['zhen','珍'],
				['zhen','诊'],
				['zhen','侦'],
				['zhen','圳'],
				['zhen','贞'],
				['zhen','枕'],
				['zhen','朕'],
				['zhen','斟'],
				['zhen','疹'],
				['zhen','臻'],
				['zhen','桢'],
				['zhen','祯'],
				['zhen','甄'],
				['zhen','赈'],
				['zhen','箴'],
				['zhen','砧'],
				['zhen','缜'],
				['zhen','榛'],
				['zhen','稹'],
				['zhen','鸩'],
				['zhen','診'],
				['zhen','針'],
				['zhen','畛'],
				['zhen','胗'],
				['zhen','轸'],
				['zhen','溱'],
				['zhen','鎮'],
				['zhen','蓁'],
				['zhen','鉁'],
				['zhen','禛'],
				['zhen','陣'],
				['zhen','鎭'],
				['zhen','鱵'],
				['zhen','絼'],
				['zhen','貞'],
				['zhen','禎'],
				['zhen','紖'],
				['zhen','眹'],
				['zhen','袗'],
				['zhen','覙'],
				['zhen','眕'],
				['zhen','眞'],
				['zheng','正'],
				['zheng','政'],
				['zheng','争'],
				['zheng','证'],
				['zheng','整'],
				['zheng','征'],
				['zheng','郑'],
				['zheng','症'],
				['zheng','挣'],
				['zheng','睁'],
				['zheng','怔'],
				['zheng','蒸'],
				['zheng','拯'],
				['zheng','铮'],
				['zheng','筝'],
				['zheng','帧'],
				['zheng','狰'],
				['zheng','徵'],
				['zheng','诤'],
				['zheng','峥'],
				['zheng','證'],
				['zheng','钲'],
				['zheng','箏'],
				['zheng','篜'],
				['zheng','証'],
				['zhi','之'],
				['zhi','知'],
				['zhi','制'],
				['zhi','直'],
				['zhi','指'],
				['zhi','至'],
				['zhi','治'],
				['zhi','质'],
				['zhi','支'],
				['zhi','致'],
				['zhi','志'],
				['zhi','织'],
				['zhi','止'],
				['zhi','值'],
				['zhi','职'],
				['zhi','置'],
				['zhi','执'],
				['zhi','智'],
				['zhi','纸'],
				['zhi','植'],
				['zhi','殖'],
				['zhi','枝'],
				['zhi','旨'],
				['zhi','秩'],
				['zhi','芝'],
				['zhi','脂'],
				['zhi','滞'],
				['zhi','肢'],
				['zhi','汁'],
				['zhi','掷'],
				['zhi','侄'],
				['zhi','稚'],
				['zhi','帜'],
				['zhi','挚'],
				['zhi','蜘'],
				['zhi','峙'],
				['zhi','趾'],
				['zhi','窒'],
				['zhi','炙'],
				['zhi','痔'],
				['zhi','痣'],
				['zhi','咫'],
				['zhi','祗'],
				['zhi','蛭'],
				['zhi','郅'],
				['zhi','酯'],
				['zhi','芷'],
				['zhi','祉'],
				['zhi','觯'],
				['zhi','雉'],
				['zhi','栉'],
				['zhi','桎'],
				['zhi','胝'],
				['zhi','質'],
				['zhi','枳'],
				['zhi','卮'],
				['zhi','鸷'],
				['zhi','帙'],
				['zhi','贽'],
				['zhi','栀'],
				['zhi','踯'],
				['zhi','摭'],
				['zhi','陟'],
				['zhi','織'],
				['zhi','骘'],
				['zhi','彘'],
				['zhi','跖'],
				['zhi','轾'],
				['zhi','踬'],
				['zhi','製'],
				['zhi','忮'],
				['zhi','阯'],
				['zhi','蹠'],
				['zhi','黹'],
				['zhi','絷'],
				['zhi','誌'],
				['zhi','埴'],
				['zhi','铚'],
				['zhi','隻'],
				['zhi','紙'],
				['zhi','職'],
				['zhi','轵'],
				['zhi','觝'],
				['zhi','袠'],
				['zhi','鉄'],
				['zhi','茝'],
				['zhi','胵'],
				['zhi','芖'],
				['zhi','絺'],
				['zhi','紩'],
				['zhi','鑕'],
				['zhi','禃'],
				['zhi','翐'],
				['zhi','秲'],
				['zhi','緻'],
				['zhi','騺'],
				['zhi','鴲'],
				['zhi','胑'],
				['zhi','衼'],
				['zhi','跱'],
				['zhi','膱'],
				['zhi','藢'],
				['zhi','鋕'],
				['zhi','禔'],
				['zhi','鼅'],
				['zhi','袟'],
				['zhi','锧'],
				['zhi','鳷'],
				['zhi','稙'],
				['zhi','覟'],
				['zhi','贄'],
				['zhi','驇'],
				['zhi','禵'],
				['zhi','隲'],
				['zhi','鴙'],
				['zhong','中'],
				['zhong','种'],
				['zhong','众'],
				['zhong','终'],
				['zhong','钟'],
				['zhong','忠'],
				['zhong','仲'],
				['zhong','肿'],
				['zhong','衷'],
				['zhong','冢'],
				['zhong','踵'],
				['zhong','盅'],
				['zhong','锺'],
				['zhong','種'],
				['zhong','忪'],
				['zhong','腫'],
				['zhong','螽'],
				['zhong','舯'],
				['zhong','眾'],
				['zhong','終'],
				['zhong','鐘'],
				['zhong','諥'],
				['zhong','螤'],
				['zhong','蹱'],
				['zhong','茽'],
				['zhong','鈡'],
				['zhou','周'],
				['zhou','洲'],
				['zhou','州'],
				['zhou','宙'],
				['zhou','皱'],
				['zhou','骤'],
				['zhou','舟'],
				['zhou','咒'],
				['zhou','轴'],
				['zhou','昼'],
				['zhou','肘'],
				['zhou','粥'],
				['zhou','帚'],
				['zhou','胄'],
				['zhou','纣'],
				['zhou','绉'],
				['zhou','妯'],
				['zhou','啁'],
				['zhou','籀'],
				['zhou','週'],
				['zhou','赒'],
				['zhou','诪'],
				['zhou','辀'],
				['zhou','軸'],
				['zhou','驟'],
				['zhou','酎'],
				['zhou','盩'],
				['zhou','輈'],
				['zhou','輖'],
				['zhou','諏'],
				['zhou','籒'],
				['zhou','譸'],
				['zhou','紂'],
				['zhou','箒'],
				['zhou','荮'],
				['zhu','主'],
				['zhu','住'],
				['zhu','注'],
				['zhu','助'],
				['zhu','著'],
				['zhu','逐'],
				['zhu','诸'],
				['zhu','朱'],
				['zhu','筑'],
				['zhu','珠'],
				['zhu','驻'],
				['zhu','竹'],
				['zhu','祝'],
				['zhu','柱'],
				['zhu','猪'],
				['zhu','嘱'],
				['zhu','株'],
				['zhu','铸'],
				['zhu','烛'],
				['zhu','煮'],
				['zhu','蛛'],
				['zhu','贮'],
				['zhu','诛'],
				['zhu','瞩'],
				['zhu','竺'],
				['zhu','铢'],
				['zhu','侏'],
				['zhu','蛀'],
				['zhu','伫'],
				['zhu','拄'],
				['zhu','渚'],
				['zhu','杼'],
				['zhu','箸'],
				['zhu','潴'],
				['zhu','炷'],
				['zhu','苎'],
				['zhu','茱'],
				['zhu','洙'],
				['zhu','躅'],
				['zhu','橥'],
				['zhu','翥'],
				['zhu','槠'],
				['zhu','舳'],
				['zhu','纻'],
				['zhu','麈'],
				['zhu','諸'],
				['zhu','邾'],
				['zhu','硃'],
				['zhu','蠋'],
				['zhu','豬'],
				['zhu','貯'],
				['zhu','跓'],
				['zhu','麆'],
				['zhu','疰'],
				['zhu','钃'],
				['zhu','詝'],
				['zhu','築'],
				['zhu','苧'],
				['zhu','竚'],
				['zhu','藷'],
				['zhu','紵'],
				['zhu','蠾'],
				['zhu','矚'],
				['zhu','鸀'],
				['zhu','鑄'],
				['zhu','駐'],
				['zhu','茿'],
				['zhu','鼄'],
				['zhu','羜'],
				['zhu','砫'],
				['zhu','秼'],
				['zhu','笜'],
				['zhu','蠩'],
				['zhu','註'],
				['zhu','祩'],
				['zhua','抓'],
				['zhua','爪'],
				['zhua','挝'],
				['zhua','膼'],
				['zhua','髽'],
				['zhuai','睉'],
				['zhuan','转'],
				['zhuan','专'],
				['zhuan','赚'],
				['zhuan','撰'],
				['zhuan','砖'],
				['zhuan','篆'],
				['zhuan','馔'],
				['zhuan','啭'],
				['zhuan','颛'],
				['zhuan','轉'],
				['zhuan','転'],
				['zhuan','蒃'],
				['zhuan','顓'],
				['zhuan','磚'],
				['zhuan','耑'],
				['zhuan','鄟'],
				['zhuan','簨'],
				['zhuan','饌'],
				['zhuang','装'],
				['zhuang','状'],
				['zhuang','庄'],
				['zhuang','壮'],
				['zhuang','撞'],
				['zhuang','桩'],
				['zhuang','妆'],
				['zhuang','戆'],
				['zhuang','裝'],
				['zhuang','莊'],
				['zhuang','荘'],
				['zhuang','粧'],
				['zhui','追'],
				['zhui','坠'],
				['zhui','缀'],
				['zhui','锥'],
				['zhui','赘'],
				['zhui','惴'],
				['zhui','骓'],
				['zhui','缒'],
				['zhui','隹'],
				['zhui','贅'],
				['zhui','醊'],
				['zhui','錐'],
				['zhui','礈'],
				['zhui','錣'],
				['zhui','縋'],
				['zhui','諈'],
				['zhui','笍'],
				['zhui','綴'],
				['zhui','餟'],
				['zhun','准'],
				['zhun','谆'],
				['zhun','肫'],
				['zhun','窀'],
				['zhun','諄'],
				['zhun','迍'],
				['zhuo','桌'],
				['zhuo','捉'],
				['zhuo','卓'],
				['zhuo','拙'],
				['zhuo','灼'],
				['zhuo','琢'],
				['zhuo','酌'],
				['zhuo','浊'],
				['zhuo','啄'],
				['zhuo','镯'],
				['zhuo','涿'],
				['zhuo','茁'],
				['zhuo','擢'],
				['zhuo','斫'],
				['zhuo','濯'],
				['zhuo','浞'],
				['zhuo','诼'],
				['zhuo','倬'],
				['zhuo','棹'],
				['zhuo','鷟'],
				['zhuo','諑'],
				['zhuo','鐲'],
				['zhuo','蠗'],
				['zi','子'],
				['zi','自'],
				['zi','资'],
				['zi','字'],
				['zi','仔'],
				['zi','紫'],
				['zi','兹'],
				['zi','姿'],
				['zi','滋'],
				['zi','姊'],
				['zi','咨'],
				['zi','吱'],
				['zi','孜'],
				['zi','籽'],
				['zi','淄'],
				['zi','梓'],
				['zi','渍'],
				['zi','恣'],
				['zi','辎'],
				['zi','谘'],
				['zi','滓'],
				['zi','龇'],
				['zi','髭'],
				['zi','孳'],
				['zi','笫'],
				['zi','缁'],
				['zi','粢'],
				['zi','眦'],
				['zi','赀'],
				['zi','锱'],
				['zi','訾'],
				['zi','嵫'],
				['zi','秭'],
				['zi','資'],
				['zi','趑'],
				['zi','鲻'],
				['zi','茲'],
				['zi','鯔'],
				['zi','菑'],
				['zi','眥'],
				['zi','觜'],
				['zi','趦'],
				['zi','茊'],
				['zi','薋'],
				['zi','葘'],
				['zi','胔'],
				['zi','胾'],
				['zi','秶'],
				['zi','諮'],
				['zi','镃'],
				['zi','貲'],
				['zi','禌'],
				['zi','鄑'],
				['zi','齜'],
				['zong','总'],
				['zong','宗'],
				['zong','纵'],
				['zong','综'],
				['zong','踪'],
				['zong','棕'],
				['zong','鬃'],
				['zong','粽'],
				['zong','偬'],
				['zong','總'],
				['zong','綜'],
				['zong','鯮'],
				['zong','縱'],
				['zong','蹤'],
				['zong','腙'],
				['zong','蓯'],
				['zong','磫'],
				['zong','縂'],
				['zong','糉'],
				['zong','踨'],
				['zong','蓗'],
				['zong','鬉'],
				['zong','騣'],
				['zong','緵'],
				['zong','骔'],
				['zou','走'],
				['zou','奏'],
				['zou','邹'],
				['zou','揍'],
				['zou','诌'],
				['zou','诹'],
				['zou','鲰'],
				['zou','驺'],
				['zou','鄹'],
				['zou','陬'],
				['zou','菆'],
				['zou','齺'],
				['zou','緅'],
				['zou','鯐'],
				['zu','组'],
				['zu','足'],
				['zu','族'],
				['zu','祖'],
				['zu','阻'],
				['zu','租'],
				['zu','诅'],
				['zu','俎'],
				['zu','菹'],
				['zu','镞'],
				['zu','組'],
				['zu','紣'],
				['zu','詛'],
				['zu','箤'],
				['zu','稡'],
				['zuan', ['钻','纂','攥','缵','躜','纘','鑽','繤','纉','賺','躦']],
				['zui', ['最','罪','嘴','醉','蕞','纗']],
				['zun', ['尊','遵','樽','鳟','撙','鶎','鐏','鷷','罇','銌','繜']],
				['zuo',['作','做','坐','左','座','昨','佐','唑','酢','祚','柞','胙','怍','阼','稓','筰','蓙']],
				['cyb1', '半機器人一號']
			];

			var patterns, regex, rule, replacement, i, j, $li, $ul, $menu;


			for ( i = 0; i < patterns.length; i++ ) {
				rule = patterns[i];
				regex = new RegExp( rule[0] + '$' );

				// Last item in the rules.
				// It can also be a function, because the replace
				// method can have a function as the second argument.
				replacement = rule.slice( -1 )[0];

				// Input string match test
				if ( regex.test( input ) ) {

					// Add replacement to list
					if(typeof(replacement) === "object") {
						$menu = $('.ime-autocomplete');

						if(!$menu.length) {
							$menu = $('<div class="ime-autocomplete"></div>');
							$ul = $('<ul></ul>');
							$ul.appendTo($menu);
							$('body').append($menu);
						} else {
							$ul = $('ul', $menu);
							$ul.empty();
						}

						for(j = 0; j < replacement.length; j++) {
							$li = $('<li></li>');
							$li.appendTo($ul).text(replacement[j]);
						}
					}
					console.log(rule);
					// Context test required?
					if ( rule.length === 3 ) {
						if ( new RegExp( rule[1] + '$' ).test( context ) ) {
							return input.replace( regex, replacement );
						}
					} else {
						// No context test required. Just replace.
						return input.replace( regex, replacement );
					}
				}
			}
		}
	};

	$.ime.register( cn );
}( jQuery ) );
